...
bool MatmulKernelRun(
    GM_ADDR deviceA, GM_ADDR deviceB, GM_ADDR deviceMxScaleA, GM_ADDR deviceMxScaleB, GM_ADDR deviceScale,
    GM_ADDR devicePerTokenScale, GM_ADDR deviceD, uint8_t*& deviceWorkspace, uint32_t m, uint32_t n, uint32_t k,
    aclrtStream stream)
{
    ...
    using MatmulKernel =
        Gemm::Kernel::MxMatmulPerTokenPerChannelTla<BlockMmad, BlockEpilogue, BlockScheduler, workspaceStages>;

    using MatmulAdapter = Gemm::Device::DeviceGemm<MatmulKernel>;

    GemmCoord problemShape{m, n, k};
    typename MatmulKernel::Arguments arguments{
        problemShape,        aicCoreNum,          deviceA,        layoutA,        deviceB,     layoutB,
        deviceMxScaleA,      layoutMxScaleA,      deviceMxScaleB, layoutMxScaleB, deviceScale, layoutScale,
        devicePerTokenScale, layoutPerTokenScale, deviceD,        layoutD};

    MatmulAdapter matmulOp;
    if (matmulOp.CanImplement(arguments) != Status::kSuccess) {
        std::cerr << "Matmul shape cannot be implemented." << std::endl;
        return false;
    }
...
static const std::string kDataRoot = "../../examples/ascend950_fp4_mx_quant_matmul/data";

int Run(const Options& options)
{
    if (options.problemShape.m() == 0 || options.problemShape.n() == 0 || options.problemShape.k() == 0) {
        std::cerr << "M, N and K must be positive." << std::endl;
        return EXIT_FAILURE;
    }
    if (options.problemShape.n() % 2 != 0 || options.problemShape.k() % 2 != 0) {
        std::cerr << "N and K must be even for packed FP4." << std::endl;
        return EXIT_FAILURE;
    }
    ...
    bool launched = MatmulKernelRun<float>(
        deviceA, deviceB, deviceMxScaleA, deviceMxScaleB, deviceScale, devicePerTokenScale, deviceD, deviceWorkspace, m,
        n, k, stream);
    ...
    ACL_CHECK(aclrtResetDevice(options.deviceId));
    ACL_CHECK(aclFinalize());
    return errorIndices.empty() ? EXIT_SUCCESS : EXIT_FAILURE;
}
