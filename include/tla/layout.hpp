...
auto MakeLayoutFromTag(LayoutTag const& tag)
{
    static_assert(std::is_same_v<LayoutTag, Catlass::layout::RowMajor> ||
        ...
        "Catlass::layout::ColumnMajor or Catlass::layout::zN or Catlass::layout::nZ or"
    ...
    } else if constexpr (std::is_same_v<LayoutTag, Catlass::layout::VectorLayout>) {
        return MakeLayout(MakeShape(tag.shape(0)), MakeStride(Int<1>{}));
    } else {  // zN or nZ
