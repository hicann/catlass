...
#ifndef ACT_GEMV_DEVICE_GEMV_UNIVERSAL_ADAPTER_HPP
#define ACT_GEMV_DEVICE_GEMV_UNIVERSAL_ADAPTER_HPP

#include "act/act.hpp"
#include "act/status.hpp"
#include "act/gemv/device/kernel_adapter.hpp"


namespace Act::Gemv::Device {
...
public:
    ...
    {
        if (fftsAddr == 0) {
            Act::KernelAdapter<GemvKernel><<<blockDim, nullptr, stream>>>(params_);
        }
        else {
            Act::KernelAdapter<GemvKernel><<<blockDim, nullptr, stream>>>(params_, fftsAddr);
        }
        return Status::kSuccess;
    }
