...
namespace acot::gemv::block
{
    ...
    public:
        ...
        using L1AlignHelper = gemv::helper::L1AlignHelper<ElementA, LayoutA>;
        ...
        {
            auto layoutxInL1 = LayoutxInL1::template MakeLayout<Elementx>(16, L1TileShape::N);             // 16, N 这里这个16就是向上对齐，后续需要封装
            auto layoutAInL1 = LayoutAInL1::template MakeLayout<ElementA>(L1TileShape::M, L1TileShape::N); // M,N 行优先，列优先也是这个
            auto layoutInL0C = LayoutyInL0::MakeLayoutInL0C(MatrixCoord(16, actualShape.m()));             //  可能有问题(已修改)

            uint32_t mRound = RoundUp<L1AlignHelper::M_ALIGNED>(actualShape.m()); // m方向要和16或者32对齐，判断条件具体和数据类型有关
            ...
            for (uint32_t nLoopIdx = 0; nLoopIdx < nTileCount; nLoopIdx++)
            {
                ...
                // auto layoutTileA = layoutA.GetTileLayout(MakeCoord(nActual, actualShape.m()));
                copyGmToL1B(l1BTensor, gmTileA, layoutAInL1, layoutTileA);

                AscendC::PipeBarrier<PIPE_ALL>();

                // 需要对齐，因为在列优先中，矩阵A的列方向默认对齐16，向量x的列方向默认对齐32B/sizeof。因此不同数据之间会存在对不齐的情况
                uint32_t nRound = RoundUp<L1AlignHelper::N_ALIGNED>(nActual); // 这里需要重点检查
