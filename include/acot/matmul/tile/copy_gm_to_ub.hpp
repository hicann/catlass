...
#ifndef ACOT_MATMUL_TILE_COPY_GM_TO_UB_HPP
#define ACOT_MATMUL_TILE_COPY_GM_TO_UB_HPP
...
struct TileCopyV2<arch::AtlasA2, Tensor<AscendC::GlobalTensor<ElementSrc>, LayoutSrc_, AscendC::TPosition::GM>,
    ...
    void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        AscendC::DataCopyExtParams dataCopyParams(
            get<0>(srcTensor.shape()),
            get<1>(srcTensor.shape()) * sizeof(ElementSrc),
            (get<0>(srcTensor.stride()) - get<1>(srcTensor.shape())) * sizeof(ElementSrc),
            (get<0>(dstTensor.stride()) - get<1>(dstTensor.shape())) / ELE_NUM_PER_BLK,
            0
        );
...
#endif // ACOT_MATMUL_TILE_COPY_GM_TO_UB_HPP
