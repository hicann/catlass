...
private:
    ...
    AscendC::LocalTensor<ElementC> ubCTensor[DOUBLESTAGES];
    AscendC::LocalTensor<ElementX> ubXTensor[DOUBLESTAGES];
    AscendC::LocalTensor<ElementD> ubDTensor[DOUBLESTAGES];
    ...
    void Resource(){
        ...
        ubCTensor[1] = ubTensor[ubByteStart].template ReinterpretCast<ElementC>(); 
        ubByteStart += ubCSize;
        ubCTensor[2] = ubTensor[ubByteStart].template ReinterpretCast<ElementC>(); 
        ubByteStart += ubCSize;
        ubCTensor[3] = ubTensor[ubByteStart].template ReinterpretCast<ElementC>(); 
        ubByteStart += ubCSize;
        ubXTensor[0] = ubTensor[ubByteStart].template ReinterpretCast<ElementX>();
        ubByteStart += ubXSize;
        ubXTensor[1] = ubTensor[ubByteStart].template ReinterpretCast<ElementX>();
        ubByteStart += ubXSize;
        ubXTensor[2] = ubTensor[ubByteStart].template ReinterpretCast<ElementX>();
        ubByteStart += ubXSize;
        ubXTensor[3] = ubTensor[ubByteStart].template ReinterpretCast<ElementX>();
        ubByteStart += ubXSize;
        ubDTensor[0] = ubTensor[ubByteStart].template ReinterpretCast<ElementD>();
        ubByteStart += ubDSize;
        ubDTensor[1] = ubTensor[ubByteStart].template ReinterpretCast<ElementD>();
        ubByteStart += ubDSize;
        ubDTensor[2] = ubTensor[ubByteStart].template ReinterpretCast<ElementD>();
        ubByteStart += ubDSize;
        ubDTensor[3] = ubTensor[ubByteStart].template ReinterpretCast<ElementD>();
        ubByteStart += ubDSize;
    }
    ...
    void EpilogueRowMajor(
        ...
        uint32_t offsetIndex = aivIndex ? aivNum : 0;
        // 对N方向进行切割
        ...
        for(uint32_t NIdx = 0; NIdx < NLoops; NIdx++){
            ...
            auto layoutXInUb = params.layoutX.GetTileLayout(MakeCoord(MUbActual, NUbActual));
            auto layoutTileX = params.layoutX.GetTileLayout(MakeCoord(MUbActual, NUbActual));
            copyGmToUbX(
                ubXTensor[offsetIndex + NIdx % STAGES],
                xGm[aivIndex * maxMPerBlock * params.layoutX.stride(0) + NIdx * maxNPerBlock],
                layoutXInUb, layoutTileX
            );
            ...
            auto layoutCInUb = layoutC.GetTileLayout(MakeCoord(MUbActual, NUbActual));
            copyGmToUbC(
                ubCTensor[offsetIndex + NIdx % STAGES],
                cGm[offsetC + aivIndex * maxMPerBlock * layoutC.stride(0) + NIdx * maxNPerBlock],
                layoutCInUb, layoutTileC
            );
            ...
            tileElemWiseEpilogueAdd(
                ubDTensor[offsetIndex + NIdx % STAGES],
                ubXTensor[offsetIndex + NIdx % STAGES],
                ubCTensor[offsetIndex + NIdx % STAGES]
            );
                // AscendC::PipeBarrier<PIPE_ALL>();
    ...
    void EpilogueColumnMajor(
        ...
        for(uint32_t MIdx = 0; MIdx < MLoops; MIdx++){
            ...
            auto layoutXInUb = params.layoutX.GetTileLayout(MakeCoord(NUbActual, MUbActual));
            auto layoutTileX = params.layoutX.GetTileLayout(MakeCoord(NUbActual, MUbActual));
            copyGmToUbX(
                ubXTensor[offsetIndex + MIdx % STAGES],
                xGm[aivIndex * maxNPerBlock * params.layoutX.stride(1) + MIdx * maxMPerBlock],
                layoutXInUb, layoutTileX
            );
            ...
            auto layoutTileC = layoutC.GetTileLayout(MakeCoord(NUbActual, MUbActual));
            auto layoutCInUb = layoutC.GetTileLayout(MakeCoord(NUbActual, MUbActual));
            copyGmToUbC(
                ubCTensor[offsetIndex + MIdx % STAGES],
                cGm[offsetC + aivIndex * maxNPerBlock * layoutC.stride(1) + MIdx * maxMPerBlock],
                layoutCInUb, layoutTileC
            );
            ...
            tileElemWiseEpilogueAdd(
                ubDTensor[offsetIndex + MIdx % STAGES],
                ubXTensor[offsetIndex + MIdx % STAGES],
                ubCTensor[offsetIndex + MIdx % STAGES]
            );
             // AscendC::PipeBarrier<PIPE_ALL>();
    ...
    void EpilogueGemm(
        ...
        if constexpr (RowOrColumn){ // 开启双缓冲
            EpilogueRowMajor(offsetC, offsetD, actualShape, layoutC);
        }else{ // 对着N方向进行切分处理
            EpilogueColumnMajor(offsetC, offsetD, actualShape, layoutC);
        }
    }
};
}
