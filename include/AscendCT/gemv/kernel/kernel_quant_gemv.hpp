...
#ifndef ASCENDCT_GEMV_KERNEL_QUANT_GEMV_HPP
#define ASCENDCT_GEMV_KERNEL_QUANT_GEMV_HPP
...
public:
    ...
    struct Arguments {
        ...
        ElementPerTokenScale PerTokenScale;
        GM_ADDR ptrBias;
    ...
    void operator()<AscendC::AIV>(Params const& params) 
    {
        ...
        TensorCoord blockShape{L1TileShape::M};

        for (uint32_t loopIdx = coreIdx; loopIdx < coreLoops; loopIdx += coreNum) {
            // Compute block location
            TensorCoord blockCoord{loopIdx};
            uint32_t MGmActual = (loopIdx == coreLoops) ? M - loopIdx * maxMPerBlock : maxMPerBlock;

            TensorCoord actualBlockShape{MGmActual};

            // Get the offset
            TensorCoord blockOffset = blockCoord * blockShape;
...
#endif // ASCENDCT_GEMV_KERNEL_QUANT_GEMV_HPP