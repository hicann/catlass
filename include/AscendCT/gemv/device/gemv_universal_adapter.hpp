...
#ifndef ASCENDCT_GEMV_DEVICE_GEMV_UNIVERSAL_ADAPTER_HPP
#define ASCENDCT_GEMV_DEVICE_GEMV_UNIVERSAL_ADAPTER_HPP

#include "AscendCT/AscendCT.hpp"
#include "AscendCT/status.hpp"
#include "AscendCT/gemv/device/kernel_adapter.hpp"


namespace AscendCT::gemv::device {

template <class GemvKernel>
class GemvUniversalAdapter {
public:
...
public:
    ...
    {
        if (fftsAddr == 0) {
            AscendCT::KernelAdapter<GemvKernel><<<blockDim, nullptr, stream>>>(params_);
        }
        else {
            AscendCT::KernelAdapter<GemvKernel><<<blockDim, nullptr, stream>>>(params_, fftsAddr);
        }
        return Status::kSuccess;
    }
