...
template<class GemvKernel, CatlassKernelType Type = CatlassKernelType::KERNEL_TYPE_AIV_ONLY>
class DeviceGemv {
public:
...
public:
    ...
    inline Status Run(aclrtStream stream, uint32_t blockDim, uint64_t fftsAddr)
    {
        ...
        if constexpr (Type == CatlassKernelType::KERNEL_TYPE_AIV_ONLY ||
                      Type == CatlassKernelType::KERNEL_TYPE_AIC_ONLY) {
            Catlass::KernelAdapter<GemvKernel, Type>::RunKernel<<<blockDim, nullptr, stream>>>(params_, ptrDump);
        } else {
            Catlass::KernelAdapter<GemvKernel, Type>::RunKernel<<<blockDim, nullptr, stream>>>(
        ...
        if constexpr (Type == CatlassKernelType::KERNEL_TYPE_AIV_ONLY ||
                      Type == CatlassKernelType::KERNEL_TYPE_AIC_ONLY) {
            Catlass::KernelAdapter<GemvKernel, Type>::RunKernel<<<blockDim, nullptr, stream>>>(params_);
        } else {
            Catlass::KernelAdapter<GemvKernel, Type>::RunKernel<<<blockDim, nullptr, stream>>>(params_, fftsAddr);
        }
#endif
        return Status::kSuccess;
    }
