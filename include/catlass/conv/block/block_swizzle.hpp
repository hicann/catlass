...
struct Conv3dIdentityBlockSwizzle {
    ...
    CATLASS_DEVICE
    Conv3dIdentityBlockSwizzle(Conv3d6HdCoord const &outShape_, Conv3d6HdCoord const &coreTileShape_)
        : outShape(outShape_), coreTileShape(coreTileShape_) 
    {
        loops = Conv3d6HdCoord{CeilDiv(outShape.n(), coreTileShape.n()), CeilDiv(outShape.d(), coreTileShape.d()), CeilDiv(outShape.c1(), coreTileShape.c1()), CeilDiv(outShape.hw(), coreTileShape.hw())};
    ...
    Conv3d6HdCoord GetBlockCoord(uint32_t taskIdx)
    {
        ...
        if constexpr (SwizzleDirection == 0) {
            ...
            uint32_t doIdx = nIdx / loops[1] % loops[2];
            uint32_t c1Idx = nIdx % loops[1];
    ...
    Conv3d6HdCoord GetActualBlockShape(Conv3d6HdCoord blockCoord, Conv3d6HdCoord dimStartIdx)
    {
        ...
        uint32_t doActual = (blockCoord.d() == loops.d() - 1) ?
            (outShape[2] - dimStartIdx.d()) : coreTileShape.d();
        
        uint32_t c1Actual = (blockCoord.c1() == loops.c1() - 1) ?
            (outShape[1] - dimStartIdx.c1()) : coreTileShape.c1();
        
        uint32_t hwActual = (blockCoord.hw() == loops.hw() - 1) ?
            (outShape[3] * outShape[4] - dimStartIdx.hw()) : coreTileShape.hw();
