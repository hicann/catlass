...
public:
    ...
    CATLASS_DEVICE
    BlockConv(Arch::Resource<ArchTag> &resource, Catlass::Conv3dParams &conv3dParams_, uint32_t l1BufAddrStart = 0)
    : conv3dParams(conv3dParams_)
    {
        copyL1ToL0A = CopyL1ToL0A::MakeCopyL1ToL0A(conv3dParams.sW(), conv3dParams.sH(), conv3dParams.kw(), conv3dParams.kh(), conv3dParams.dilatedKernelW(), conv3dParams.dilatedKernelH());
        uint64_t bl1Spacesize = FilterL1TileShape::Kd * FilterL1TileShape::Ci1 * conv3dParams.kh() * conv3dParams.kw() * FilterL1TileShape::nBL1 * BLOCK_L0_N * C0_SIZE;
        uint64_t hoAL1Max = ((FmapL1TileShape::mAL1 < CoreTileShape::singleCoreHoWo ? FmapL1TileShape::mAL1 : CoreTileShape::singleCoreHoWo) / conv3dParams.wo()) + 2;
        ...
        uint64_t al1Spacesize = FilterL1TileShape::Kd * FilterL1TileShape::Ci1 * hiAL1Max * conv3dParams.wi() * C0_SIZE;
        ...
        Conv3d6HdCoord const &actualBlockShape, Conv3d6HdCoord const &actualIdxStartFmap)
    {
        ...
        //////Cout方向的循环参数
        iterParams.maxNBL1Iter = CeilDiv(actualBlockShape.c1(), FilterL1TileShape::nBL1) - 1;
        iterParams.nBL1Tail = actualBlockShape.c1() % FilterL1TileShape::nBL1;
        ...
        iterParams.l12l0LoopN = CeilDiv(FilterL1TileShape::nBL1, L0TileShape::nL0);
        ...
        for (uint32_t batchIter = 0; batchIter < actualBlockShape.n(); ++batchIter) {
            ...
            while (true) {
                ...
                //////TODO同步
                auto layoutTileBias = layout::VectorLayout(actualBlockShape.n());
                auto layoutBiasInL1 = layout::VectorLayout(n);
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(L1A_STAGES + L1B_STAGES);
                ...
                while (iterParams.kIter < iterParams.ddr2l0LoopK) {
                    if (iterParams.loadAL1Flag || (!iterParams.kAL1fullload && iterParams.kIter % iterParams.multiKAL1 == 0)) {
                        ...
                        LoadAL1Process(gmBatchFmap, iterParams.kIter / iterParams.multiKAL1);
                    ...
                    if (iterParams.loadBL1Flag || (!iterParams.kBL1fullload && iterParams.kIter % iterParams.multiKBL1 == 0)) {
                        ...
                        LoadBL1Process(filterGm, iterParams.kIter / iterParams.multiKBL1);
                ...
                LayoutFmap layoutOutGm = layoutOut.GetTileLayout(MakeCoord((uint32_t)1, conv3dParams.cout(), conv3dParams.dout(), conv3dParams.ho(), conv3dParams.wo()));
                uint32_t cout1L1Idx = FilterL1TileShape::nBL1 * iterParams.nBL1Iter + L0TileShape::nL0 * iterParams.nBL0Iter;
...
protected:
    ...
    CATLASS_DEVICE
    void LoadAL1Process(AscendC::GlobalTensor<ElementFmap> const &fmapGm, uint32_t kAL1Iter)
    {
        ...
        uint32_t hiLoadL1 = ((hoEndIdx - hoStartIdx) - 1) * conv3dParams.sH() + conv3dParams.dilatedKernelH();
        ...
        uint32_t cin1LoadL1 = kAL1Tmp / conv3dParams.kh() / conv3dParams.kw() / conv3dParams.cin0();
        ...
        if (!iterParams.aL1IsFullPad) {
            ...
            auto layoutTileFmap = layoutFmap.GetTileLayout(MakeCoord((uint32_t)1, conv3dParams.cin(), conv3dParams.dD(), conv3dParams.hi(), conv3dParams.wi()));
            auto gmTileFmap = fmapGm[layoutTileFmap.GetOffset(gmTileFmapOffset)];
            layoutFmapInL1 = LayoutFmapInL1::template MakeLayout<ElementFmap>(1, cin1LoadL1 * conv3dParams.cin0(), 1, hiLoadL1, conv3dParams.wi());
            copyGmToL1A(l1ATensorList[l1ListId], gmTileFmap, layoutFmapInL1, layoutTileFmap);
            if (set2dFlagDTail) {
                // 后di pad
                AscendC::InitConstValueParams<ElementFmap> initConstValueParams;
    ...
    void LoadBL1Process(AscendC::GlobalTensor<ElementFilter> const &filterGm, uint32_t kBL1Iter)
    {
        ...
        uint32_t currentNBL1 = ((iterParams.nBL1Iter != iterParams.maxNBL1Iter) || (FilterL1TileShape::nBL1 >= conv3dParams.cout())) ? FilterL1TileShape::nBL1 : iterParams.nBL1TailAlign;
        ...
        Conv3dFracZ3dCoord gmTileFilterOffset{iterParams.kBL1Iter * FilterL1TileShape::Kd * FilterL1TileShape::Ci1 * conv3dParams.khkwcin0(), iterParams.nBL1Iter * FilterL1TileShape::nBL1};   //TODO
        auto gmTileFiler = filterGm[layoutFilter.GetOffset(gmTileFilterOffset)];
