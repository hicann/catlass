...
#ifndef CATLASS_GEMM_TILE_PROLOGUE_HPP
#define CATLASS_GEMM_TILE_PROLOGUE_HPP
...
struct TileCastInt4ToInt8 {
    ...
    using ElementIn = typename SrcType::Element;
    using ElementOut = typename DstType::Element;
    using LayoutIn = typename SrcType::Layout;
    using LayoutOut = typename DstType::Layout;
    ...
    /// Construct
    CATLASS_DEVICE
    TileCastInt4ToInt8(Arch::Resource<ArchTag> &resource, uint32_t ubBufAddrStart = 0) {
    ...
    void operator()(AscendC::GlobalTensor<ElementOut> const &gmDst,
        AscendC::GlobalTensor<ElementIn> const &gmSrc, LayoutOut const &layoutDst, LayoutIn const &layoutSrc)
        {
            ...
            if constexpr (std::is_same_v<LayoutIn, layout::ColumnMajor>) {
...
#endif // CATLASS_GEMM_TILE_PROLOGUE_HPP
