...
struct BlockMmad<
    MmadAtlasA2XFAIPV<PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_>, L1TileShape_, L0TileShape_, AType_, BType_, CType_,
...
public:
    ...
    __aicore__ inline BlockMmad(
        Arch::Resource<ArchTag>& resource, uint32_t nDyn, uint32_t kDyn, uint32_t l1BufAddrStart = 0)
    {
        // Allocate L1 memory space
        l1BTensor = resource.l1Buf.template GetBufferByByte<ElementB>(
            l1BufAddrStart + L1TileShape::M * kDyn * sizeof(ElementA) * 2);
        for (uint32_t i = 0; i < STAGES; i++) {
            l1ATensor[i] = resource.l1Buf.template GetBufferByByte<ElementA>(
                l1BufAddrStart + L1TileShape::M * kDyn * sizeof(ElementA) * i);
    ...
    __aicore__ inline void getKVOffset(
        AscendC::GlobalTensor<int32_t>& gBlockTable, uint32_t& kOffset, uint32_t& nowNIdx, uint32_t& strideKV,
        uint32_t& blockSize)
    {
    ...
    __aicore__ inline void operator()(
        ...
        AscendC::GlobalTensor<int32_t> gBlockTable, LayoutA layoutA, LayoutB layoutB, LayoutC layoutC,
            ...
            for (uint32_t mL1Idx = 0; mL1Idx < mL1Loop; mL1Idx++) {
                ...
                for (uint32_t kL1Idx = 0; kL1Idx < kL1Loop; kL1Idx++) {
                    ...
                    for (uint32_t kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                        ...
                        MatrixCoord l1BTileCoord{
                            kL1Idx * l1KDynamic + kL0Idx * L0TileShape::K, L0TileShape::N * nL1Idx};
                        auto l1BTile = l1BTensor[layoutBInL1.GetOffset(l1BTileCoord)];
                        ...
                        tileMmad(
                            l0CTensor[l0CPingPongFlag], l0ATensor[l0ABPingPongFlag], l0BTensor[l0ABPingPongFlag],
