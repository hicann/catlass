...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FIXPIPE_QUANT_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FIXPIPE_QUANT_HPP
...
namespace Catlass::Gemm::Block
{
    ...
    public:
        ...
        using ElementQ = typename QType_::Element;
        using LayoutQ = typename QType_::Layout;
        ...
        using CopyL0CToGm = Gemm::Tile::CopyL0CToGm<ArchTag, ElementAccumulator, CType_, Tile::ScaleGranularity::PER_GROUP>;
        ...
        static constexpr uint32_t L1A_SIZE = static_cast<uint32_t>(L1TileShape::M * L1TileShape::K / 2);
        static constexpr uint32_t L1B_SIZE = static_cast<uint32_t>(L1TileShape::N * L1TileShape::K / 2);
        ...
        // Check L0TileShape
        static constexpr uint32_t L0A_TILE_SIZE = L0TileShape::M * L1TileShape::K / 2;
        static constexpr uint32_t L0B_TILE_SIZE = L0TileShape::N * L1TileShape::K / 2;
        ...
        /// Construct
        CATLASS_DEVICE
        BlockGmmQuant(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
        {
            ...
            // quantTensor = resource.c1Buf.template GetBufferByByte<ElementQ>(0);
            quantTensor = resource.l1Buf.template GetBufferByByte<ElementQ>(l1AOffset + L1A_SIZE * 2);
        ...
        CATLASS_DEVICE
        void CopyDeqTensorToL1(const AscendC::LocalTensor<uint64_t> &dst, const AscendC::GlobalTensor<uint64_t> &src,
                               int32_t calNSize)
        {
            ...
            constexpr int FLOAT_FACTOR = 2;
            constexpr int ONE_BLK_SIZE = 32;
            constexpr int BLOCK_CUBE = 16;
            uint16_t deqDataSize = CeilDiv(calNSize * sizeof(uint64_t), DEQ_SIZE) * DEQ_SIZE;
            // GM -> L1
            if (calNSize % BLOCK_CUBE)
            {
                // nd2nz pad to 32Bytes align
                uint16_t dValue = calNSize * FLOAT_FACTOR;
                AscendC::Nd2NzParams intriParams{1, 1, dValue, 0, dValue, 1, 1, 0};
                AscendC::GlobalTensor<uint32_t> srcTmp;
            ...
            {
                AscendC::DataCopyParams intriParams{1, static_cast<uint16_t>(deqDataSize / ONE_BLK_SIZE), 0, 0};
        ...
        void operator()(
            ...
            AscendC::GlobalTensor<ElementQ> const &gmQ, LayoutB const &layoutQ,
                ...
                for (int mPartIdx = 0; mPartIdx < mPartLoop; mPartIdx++)
                {
                    ...
                    for (int kPartIdx = 0; kPartIdx < kPartLoop; kPartIdx++)
                    {
                        ...
                        for (int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++)
                        {
                            ...
                            tileMmad(l0CTile, l0ATile, l0BTile, mPartActual, nPartActual, kPartActual, true, unitFlag);
                            AscendC::PipeBarrier<PIPE_ALL>();
                            MatrixCoord gmQOffset{kLoopIdx * (L1TileShape::K / L0TileShape::K) + kPartIdx, nPartIdx * L0TileShape::N};
                            CopyDeqTensorToL1(quantTensor, gmQ[layoutQ.GetOffset(gmQOffset)], nPartActual);
                            AscendC::PipeBarrier<PIPE_ALL>();

                            MatrixCoord gmCOffset{mPartIdx * L0TileShape::M, nPartIdx * L0TileShape::N};
                            auto gmCAddr = layoutC.GetOffset(gmCOffset);
                            LayoutC layoutBlock = layoutC.GetTileLayout(MakeCoord(mPartActual, nPartActual));
                            auto layoutInL0CTemp = LayoutCInL0::MakeLayoutInL0C(MakeCoord(mPartActual, nPartActual));
                            // If the current tile is the first tile on the k axis, the accumulator needs to be reset to 0
                            bool initC = ((kLoopIdx == 0) && (kPartIdx == 0));
                            if (initC)
                            {
                                copyL0CToGm(gmC[gmCAddr], l0CTile, quantTensor, layoutBlock, layoutInL0CTemp, unitFlag);
                            }
                            else
                            {
    ...
    protected:
        ...
        AscendC::LocalTensor<ElementQ> quantTensor;
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FIXPIPE_QUANT_HPP
