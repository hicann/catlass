...
struct BlockMmad<
    MmadAtlasA2XFAIQK<PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_>, L1TileShape_, L0TileShape_, AType_, BType_, CType_,
...
public:
    ...
    __aicore__ inline void getBlockShape(
        GemmCoord& actualShape, uint32_t nL1Idx, uint32_t nL1Loop, uint32_t stackSeqTile)
    {
    ...
    __aicore__ inline void getKVOffset(
        AscendC::GlobalTensor<int32_t>& gBlockTable, uint32_t& kOffset, uint32_t nowNIdx, uint32_t nL1Idx,
    ...
    __aicore__ inline void operator()(
        ...
        GemmCoord actualOriShape, uint32_t nIdx, uint32_t nLoop, uint32_t blockSize, uint32_t strideKV)
    {
        ...
        for (uint32_t nL1Idx = 0; nL1Idx < nL1Loop; ++nL1Idx) {
            ...
            for (uint32_t mL0Idx = 0; mL0Idx < mL0Loop; mL0Idx++) {
                ...
                for (uint32_t kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                    ...
                    tileMmad(
                        l0CTensor[l0CPingPongFlag], l0ATensor[l0ABPingPongFlag], l0BTensor[l0ABPingPongFlag], mL0Align,
