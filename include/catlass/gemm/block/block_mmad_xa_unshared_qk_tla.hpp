...
struct BlockMmadTla<
    MmadXAUnsharedQK<Arch::Ascend950>, L1TileShape_, L0TileShape_, ElementA_, ElementB_, ElementC_, ElementBias_,
    TileCopy_, TileMmad_> {
public:
    ...
    static_assert(
        L1_TILE_M == L0_TILE_M && L1_TILE_K == L0_TILE_K, "L1 and L0 tile shapes must match on the M and K axes");
    static_assert(
...
public:
    ...
    CATLASS_DEVICE void operator()(
        TensorA& tensorA, TensorB& tensorB, TensorC& tensorC, Shape& actualShape, uint64_t QK_UB_RELEASE_FLAG,
        uint32_t& taskIdL0A, uint32_t& taskIdL0B, uint32_t& taskIdL0C)
    {
        ...
        auto tensorAL1Tile = GetTile(tensorAInL1, tla::MakeCoord(0, 0), tla::MakeShape(qActual, dimActual));
        ...
        auto tensorInL0C = tla::MakeTensor(l0CTensorList_[taskIdL0C], layoutInL0C, Arch::PositionL0C{});
        ...
        for (uint32_t nIdx = 0; nIdx < nLoops; ++nIdx) {
            ...
            auto tensorBTileInL1 = GetTile(tensorBInL1, tla::MakeCoord(0, nOffset), tla::MakeShape(dimActual, tileN));
            ...
            auto tensorTileInL0C = GetTile(tensorInL0C, tla::MakeCoord(0, nOffset), tla::MakeShape(qActual, tileN));
