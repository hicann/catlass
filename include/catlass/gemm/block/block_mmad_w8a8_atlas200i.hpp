...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_W8A8_ATLAS200I_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_W8A8_ATLAS200I_HPP
...
template <
    ...
    class TileCopy_,
    class TileMmad_>
struct BlockMmad<MmadAtlas200IW8a8, L1TileShape_, L0TileShape_, AType_, BType_, CType_, BiasType_, TileCopy_, TileMmad_> {
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlas200IW8a8;
    ...
    BlockMmad(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {
        ...
        uint32_t L1ScaleOffset = L1A_SIZE * STAGES + L1B_SIZE * STAGES;
        uint32_t L1BiasOffset = L1ScaleOffset + L1TileShape::N * sizeof(uint64_t);
        ...
        EVENT_ID_SCALE = EVENT_ID_BIAS + 1;

        L1ScaleTensor = resource.l1Buf.template GetBufferByByte<ElementScale>(L1ScaleOffset);
        // L1ScaleTensor = resource.l1Buf.template GetBufferByByte<ElementScale>(0);
        FbScaleTensor = resource.fpBuf.template GetBufferByByte<uint64_t>(0);

        l0BiasTensor = resource.btBuf.template GetBufferByByte<ElementBias>(0);
        l1BiasTensor = resource.l1Buf.template GetBufferByByte<ElementBias>(L1BiasOffset);

        l0CTensor = resource.l0CBuf.template GetBufferByByte<ElementAccumulator>(0);
        AscendC::SetFlag<AscendC::HardEvent::FIX_M>(EVENT_ID0);

        AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(EVENT_ID_SCALE);
    ...
    ~BlockMmad()
    {
        ...
        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(EVENT_ID_SCALE);
    ...
    )
    {
        ...
        layout::VectorLayout layoutScaleInFpBuf{nRound}; // layoutScale::template MakeLayout<ElementScale>(1, nRound);

        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(EVENT_ID_SCALE);
        copyGmToL1Scale(L1ScaleTensor, gmScale, layoutScaleInFpBuf, layoutScaleInFpBuf);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_FIX>(EVENT_ID_SCALE);
        ...
        uint32_t lastTileIdx = kTileCount - 1;
        uint32_t kActual = (firstTileIdx < kTileCount - 1) ? L1TileShape::K
                                                           : (actualShape.k() - firstTileIdx * L1TileShape::K);
        uint32_t firstTileIdxNext = 0;
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++) {
            ...
            if (kLoopIdx == firstTileIdx && isFirstBlock) {
                MatrixCoord gmTileAOffset{0, kLoopIdx * L1TileShape::K};
                MatrixCoord gmTileBOffset{kLoopIdx * L1TileShape::K, 0};
            ...
            if (kLoopIdx == lastTileIdx && hasNextBlock) {
                // Get L1 tensor for next stage
                auto l1ATensor = l1ATensorList[l1ListIdNext];
                auto l1BTensor = l1BTensorList[l1ListIdNext];
                // Get GM tensor for next stage
        ...
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_FIX>(EVENT_ID_SCALE);
        copyL1ToFP(FbScaleTensor, L1ScaleTensor, layoutScaleInFpBuf, layoutScaleInFpBuf);
        AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(EVENT_ID_SCALE);

        AscendC::SetFlag<AscendC::HardEvent::M_FIX>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(EVENT_ID0);
        copyL0CToGm(gmBlockC, l0CTensor, FbScaleTensor, layoutBlock, layoutInL0C);
        AscendC::SetFlag<AscendC::HardEvent::FIX_M>(EVENT_ID0);
    }
...
protected:
    ...
    AscendC::LocalTensor<uint64_t> FbScaleTensor;
