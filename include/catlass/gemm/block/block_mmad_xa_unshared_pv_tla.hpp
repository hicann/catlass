...
struct BlockMmadTla<
    MmadXAUnsharedPV<Arch::Ascend950>, L1TileShape_, L0TileShape_, ElementA_, ElementB_, ElementC_, ElementBias_,
    TileCopy_, TileMmad_> {
public:
    ...
    static_assert(
        L1_TILE_M == L0_TILE_M && L1_TILE_N == L0_TILE_N, "L1 and L0 tile shapes must match on the M and N axes");
    static_assert(
...
public:
    ...
    CATLASS_DEVICE void operator()(
        TensorA& tensorA, TensorB& tensorB, TensorC& tensorC, Shape& actualShape, uint32_t& taskIdL0A,
        uint32_t& taskIdL0B, uint32_t& taskIdL0C)
    {
        ...
        for (uint32_t kIdx = 0; kIdx < kLoops; ++kIdx) {
            ...
            auto tensorATileInL1 = GetTile(tensorA, tla::MakeCoord(0, kOffset), tla::MakeShape(mActual, tileK));
            ...
            auto tensorBTileInL1 = GetTile(tensorBInL1, tla::MakeCoord(kOffset, 0), tla::MakeShape(tileK, nActual));
