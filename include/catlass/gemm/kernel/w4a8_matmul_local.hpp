...
public:
    ...
    using LayoutBIn = typename BlockMmad::LayoutSrcB;
    ...
    struct Params {
        ...
        Params(
            ...
            GM_ADDR ptrB_, LayoutB const &layoutB_, LayoutBIn const &layoutBIn_,
            GM_ADDR ptrC_, LayoutC const &layoutC_,
            GM_ADDR ptrWorkspace_,
            uint64_t deqScalar_
        ): problemShape(problemShape_), ptrA(ptrA_), layoutA(layoutA_), ptrB(ptrB_), layoutB(layoutB_), layoutBIn(layoutBIn_),
    ...
    static Params ToUnderlyingArguments(const Arguments &args, uint8_t *workspace)
    {
        ...
        LayoutA layoutA{m, k};
        LayoutB layoutB{k, n};
        LayoutBIn layoutBIn{k, n};
        LayoutC layoutC{m, n};
        Params params{problemShape, args.deviceA, layoutA, args.deviceB, layoutB, layoutBIn, args.deviceC, layoutC, workspace, args.scalar};
        return params;
    }
    ...
    void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        for (uint32_t loopIdx = AscendC::GetBlockIdx() / 2; loopIdx < coreLoops; loopIdx += AscendC::GetBlockNum()) {
            ...
            blockMmad.Prologue(
                gmB[gmOffsetB], params.layoutBIn,
                gmWB[gmOffsetBWksp],
                actualBlockShape);
        }
    }
