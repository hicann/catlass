...
class BlockEpilogue<EpilogueAtlasA2FAUnsharedSoftmax, OutputType_, InputType_, MaskType_> {
public:
    // Type aliases
    ...
    CATLASS_DEVICE
    BlockEpilogue(
        Arch::Resource<ArchTag>& resource, float tor_, uint32_t unsharedKvSeqLen, uint32_t maxDecodeStep,
        ...
        uint32_t kSeqTile, uint32_t kSeqTileRound)
    {
        if (kSeqTile <= FLOAT_VECTOR_SIZE) {
            ...
            for (uint32_t rowsum_idx = 1; rowsum_idx < repeatTimes; ++rowsum_idx) {
                AscendC::Add(
                    src, src, src[rowsum_idx * FLOAT_VECTOR_SIZE], FLOAT_VECTOR_SIZE, curRowNum,
            ...
            if (tailLen > 0) {
                AscendC::Add(
                    src, src, src[kSeqTile / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], tailLen, curRowNum,
                    AscendC::BinaryRepeatParams(
                        1, 1, 1, kSeqTileRound / FLOAT_BLOCK_SIZE, kSeqTileRound / FLOAT_BLOCK_SIZE,
                        kSeqTileRound / FLOAT_BLOCK_SIZE));
            }
        ...
        uint32_t curRowNum, uint32_t subMRound, uint32_t kSeqTile, uint32_t kSeqTileRound)
    {
        ...
        AscendC::Brcb(tempMaxTensor, MaxTensor, brcbRepeatTimes, AscendC::BrcbRepeatParams(1, 8));
        AscendC::PipeBarrier<PIPE_V>();
        for (uint32_t subIdx = 0; subIdx < kSeqTile / FLOAT_VECTOR_SIZE; ++subIdx) {
            AscendC::Sub(
                dst[subIdx * FLOAT_VECTOR_SIZE], src[subIdx * FLOAT_VECTOR_SIZE], tempMaxTensor, FLOAT_VECTOR_SIZE,
                curRowNum,
                AscendC::BinaryRepeatParams(
        ...
        if (kSeqTile % FLOAT_VECTOR_SIZE > 0) {
            AscendC::Sub(
                ...
                src[kSeqTile / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], tempMaxTensor, kSeqTile % FLOAT_VECTOR_SIZE,
                curRowNum,
                AscendC::BinaryRepeatParams(
                    1, 1, 0, kSeqTileRound / FLOAT_BLOCK_SIZE, kSeqTileRound / FLOAT_BLOCK_SIZE, 1));
        }
        ...
        const AscendC::LocalTensor<float>& tempTensor, uint32_t curRowNum, uint32_t kSeqTile, uint32_t kSeqTileRound)
    {
        if (kSeqTile <= FLOAT_VECTOR_SIZE) {
            AscendC::WholeReduceMax(
                dst, src, kSeqTile, curRowNum, 1, 1, kSeqTileRound / FLOAT_BLOCK_SIZE,
                AscendC::ReduceOrder::ORDER_ONLY_VALUE);
        } else {
            AscendC::DataCopy(
                tempTensor, src,
                AscendC::DataCopyParams(
                    curRowNum, FLOAT_VECTOR_SIZE / FLOAT_BLOCK_SIZE,
            ...
            for (uint32_t rowmaxIdx = 1; rowmaxIdx < repeatTimes; ++rowmaxIdx) {
                AscendC::Max(
                    tempTensor, tempTensor, src[rowmaxIdx * FLOAT_VECTOR_SIZE], FLOAT_VECTOR_SIZE, curRowNum,
                    AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, kSeqTileRound / FLOAT_BLOCK_SIZE));
                AscendC::PipeBarrier<PIPE_V>();
            }
            if (tailLen > 0) {
                AscendC::Max(
                    tempTensor, tempTensor, src[kSeqTile / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], tailLen, curRowNum,
                    AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, kSeqTileRound / FLOAT_BLOCK_SIZE));
            }
            AscendC::PipeBarrier<PIPE_V>();
            uint32_t mask = repeatTimes > 0 ? FLOAT_VECTOR_SIZE : tailLen;
            AscendC::WholeReduceMax(dst, tempTensor, mask, curRowNum, 1, 1, 8, AscendC::ReduceOrder::ORDER_ONLY_VALUE);
        }
        AscendC::PipeBarrier<PIPE_V>();
    }
        ...
        const LayoutOutput& layoutOutput, const LayoutInput& layoutInput, uint32_t curHeadNum, uint32_t headOffset)
    {
        ...
        AscendC::DataCopy(
            lsUbTensor, gInput, AscendC::DataCopyParams(1, curRowNum * kSeqTileRound / FLOAT_BLOCK_SIZE, 0, 0));
        ...
        TensorSubValueRepeatM(
            lsUbTensor, lsUbTensor, lmUbTensor, tvUbTensor, curRowNum, subMRound, kSeqTile, kSeqTileRound);
