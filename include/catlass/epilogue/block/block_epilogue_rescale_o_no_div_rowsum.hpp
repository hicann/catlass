...
public:
    ...
    CATLASS_DEVICE
    void CopyFloatOToGm(
        AscendC::GlobalTensor<float> gSharedOut, uint32_t curRowNum, uint32_t qSBlockSize, uint32_t embed,
        uint32_t embedRound, uint32_t qNThisSubBlock, uint32_t oHiddenSize)
    {
        if (qNThisSubBlock == 0) {
            AscendC::DataCopyPad(
                gSharedOut, goUbTensor32,
                AscendC::DataCopyExtParams(curRowNum, embed * 4, 0, (oHiddenSize - embed) * 4, 0));
        } else {
            for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                AscendC::DataCopyPad(
                    gSharedOut[qNIdx * embed], goUbTensor32[qNIdx * embedRound * qSBlockSize],
    ...
    void SubCoreCompute(
        ...
        AscendC::GlobalTensor<float> gSharedOut, const LayoutOutput& layoutOutput, const LayoutInput& layoutInput,
        uint32_t qNThisSubBlock, uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
    ...
    void operator()(
        AscendC::GlobalTensor<ElementOutput> gOutput, AscendC::GlobalTensor<ElementInput> gInput,
        AscendC::GlobalTensor<float> gSharedOut, const LayoutOutput& layoutOutput, const LayoutInput& layoutInput,
        GemmCoord actualBlockShape, uint32_t qSBlockSize, uint32_t qNBlockSize, uint32_t isFirstStackTile,
        uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
        ...
        if (inRowActualThisSubBlock > 0) {
            ...
            SubCoreCompute(
                gOutputThisSubBlock, gInputThisSubBlock, gSharedOutThisSubBlock, layoutOutputThisSubBlock,
                layoutInputThisSubBlock, qNThisSubBlock, isFirstStackTile, isLastStackTile, curStackTileMod);
        }
    }
