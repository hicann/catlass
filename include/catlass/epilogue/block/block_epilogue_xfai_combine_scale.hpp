...
template <class OutputType_, class LseType_>
class CombineScale {
public:
    ...
    __aicore__ inline CombineScale(Arch::Resource<ArchTag>& resource)
    {
        ...
        constexpr uint32_t GO16_UB_OFFSET =
            8 * STAGE2_UB_UINT8_BLOCK_SIZE + 2 * UB_UINT8_LINE_SIZE;               // (q * h)_algin * v
    ...
    __aicore__ inline void operator()(
        uint32_t qHeads, uint32_t kvSplitCoreNum, uint32_t headSizeV, __gm__ SplitKvExtraInfo* extraInfo,
        ...
        bool inputLayoutTND = true)
    {
        ...
        for (uint32_t process = subBlockID; process < kvSplitCoreNum * 2; process += subBlockNum) {
            ...
            uint32_t addrLOffset = vectorsubBlockID == 0 ? extraInfo->splitInfo[process / 2].lseTaskOffset :
                                                           extraInfo->splitInfo[process / 2].lseTaskOffset + sum_former;
            uint32_t addrOOffset = vectorsubBlockID == 0 ?
                                       extraInfo->splitInfo[process / 2].oTaskOffset :
                                       extraInfo->splitInfo[process / 2].oTaskOffset + sum_former * headSizeV;

            uint32_t prevQSeqlenSum = 0;
            if (inputLayoutTND) {
                prevQSeqlenSum = (batchIdx == 0) ? 0 : static_cast<uint32_t>(gActualQseqlen.GetValue(batchIdx - 1));
            }
            ...
            // // Preload O
            AscendC::DataCopyPad(
                loFloatUbTensor[pingpongFlag * BASIC_BLOCK], oCoreTmpGmTensor[addrOOffset],
            ...
            AscendC::ReduceMax<float, AscendC::Pattern::Reduce::RA, false>(
                lmUbTensor, llUbTensor, tempReduceMax, reduceMaxShape, true);
            ...
            AscendC::BroadCast<float, 2, 0>(
                broadCastTensor, lmUbTensor, dstShapeBroadcast, srcShapeBroadcast, tempReduceSum);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            AscendC::ReduceSum<float, AscendC::Pattern::Reduce::RA, false>(
                rsUbTensor, tlUbTensor, tempReduceSum, reduceSumShape, true);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            // Broadcast scale
            AscendC::BroadCast<float, 2, 0>(
                broadCastScaleTensor, tsUbTensor, dstShapeBroadcast, srcShapeBroadcast, tempReduceSum);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            for (uint32_t nIdx = 0; nIdx < splitNum; nIdx++) {
                if (nIdx != 0) {
                    ...
                    AscendC::DataCopyPad(
                        loFloatUbTensor[pingpongFlag * BASIC_BLOCK],
                        oCoreTmpGmTensor[addrOOffset + nIdx * oCount_vector],
                        AscendC::DataCopyExtParams(1, oCount * sizeof(float), 0, 0, 0),
                        AscendC::DataCopyPadExtParams<float>(true, 0, 0, 0));
                    AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID1 + pingpongFlag);
                }
                ...
                AscendC::BroadCast<float, 2, 1>(
                    broadCastOTensor[pingpongFlag * BASIC_BLOCK], glUbTensor[nIdx * lseBlockAlign], dstShapeO,
                    srcShapeO, tempReduceSum);
                AscendC::PipeBarrier<PIPE_V>();

                AscendC::Mul(
                    toUbTensor[pingpongFlag * BASIC_BLOCK], loFloatUbTensor[pingpongFlag * BASIC_BLOCK],
                    broadCastOTensor[pingpongFlag * BASIC_BLOCK], oCount); // toUbTensor和broadCastOTensor共用一块空间
                AscendC::PipeBarrier<PIPE_V>();

                if (nIdx == 0) {
                    AscendC::Adds(goUbTensor, toUbTensor[pingpongFlag * BASIC_BLOCK], 0.0f, oCount);
                    AscendC::PipeBarrier<PIPE_V>();
                } else {
            ...
            if (q_len == 1) {
                AscendC::DataCopyPad(
                    oGmTensor[gmOScalar], go16UbTensor,
                    AscendC::DataCopyExtParams(1, oCount * sizeof(ElementOutput), 0, 0, 0));
            } else {
                uint32_t q_half = q_len / 2;
                if (vectorsubBlockID == 0) {
                    AscendC::DataCopyPad(
                        ...
                        AscendC::DataCopyExtParams(
                            q_half, (headEndIndx - headStartIndx) * headSizeV * sizeof(ElementOutput), 0,
                            (qHeads - (headEndIndx - headStartIndx)) * headSizeV * sizeof(ElementOutput), 0));
                } else {
                    AscendC::DataCopyPad(
                        oGmTensor[gmOScalar], go16UbTensor,
                        AscendC::DataCopyExtParams(
                            q_len - q_half, (headEndIndx - headStartIndx) * headSizeV * sizeof(ElementOutput), 0,
                            (qHeads - (headEndIndx - headStartIndx)) * headSizeV * sizeof(ElementOutput), 0));
                }
            }
        }
