...
public:
    ...
    __aicore__ inline void CopyOToGm(
        AscendC::GlobalTensor<ElementOutput> gOutput, uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum,
        uint32_t integralHeadNum, uint32_t qSThisSubBlock, uint32_t embed, uint32_t oHiddenSize)
    {
        uint32_t innerOGmOffset = 0;
        uint32_t innerGOUbOffset = 0;
        if (proTokenNum != 0U) {
            AscendC::DataCopyPad(
                gOutput[innerOGmOffset + proTokenIdx * oHiddenSize], goUbTensor16[innerGOUbOffset],
        ...
        for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
            AscendC::DataCopyPad(
                gOutput[innerOGmOffset], goUbTensor16[innerGOUbOffset],
                AscendC::DataCopyExtParams(
    ...
    __aicore__ inline void CopyOToGmFp32(
        AscendC::GlobalTensor<float> gOutput, uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum,
        uint32_t integralHeadNum, uint32_t qSThisSubBlock, uint32_t embed, uint32_t oHiddenSize,
        uint32_t oHiddenSize_gmlo)
    {
        uint32_t innerOGmOffset = 0;
        uint32_t innerGOUbOffset = 0;
        ...
        if (proTokenNum != 0U) {
            AscendC::DataCopyPad(
                gOutput[innerOGmOffset + proTokenIdx * oHiddenSize], goUbTensor32[innerGOUbOffset],
                AscendC::DataCopyExtParams(
                    proTokenNum,
        ...
        for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
            AscendC::DataCopyPad(
                gOutput[innerOGmOffset], goUbTensor32[innerGOUbOffset],
                AscendC::DataCopyExtParams(
    ...
    __aicore__ inline void SubCoreCompute(
        ...
        AscendC::GlobalTensor<ElementLse> gCombineLse, AscendC::GlobalTensor<ElementLse> gCombineo,
        ...
        uint32_t epiTokenNum, uint32_t integralHeadNum, bool isSplitkv, const LayoutOutput& layoutgmLse,
        const LayoutInput& layoutgmLo)
    {
        ...
        if (!isFirstStackTile) {
            ...
            if (needRowLoop) {
                AscendC::DataCopy(
                    goUbTensor32, gUpdate, AscendC::DataCopyParams(1, curRowNum * embedRound / FLOAT_BLOCK_SIZE, 0, 0));
        ...
        if (isLastStackTile) {
            ...
            AscendC::Brcb(
                tvUbTensor.ReinterpretCast<uint32_t>(), glUbTensor.ReinterpretCast<uint32_t>()[rowOffsetLoop],
                curRowNumRound / FLOAT_BLOCK_SIZE, AscendC::BrcbRepeatParams(1, 8));
            AscendC::PipeBarrier<PIPE_V>();
            // *** go = go / gl_block
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
            ...
            if (isSplitkv) {
                CopyOToGmFp32(
                    ...
                    proTokenIdx, proTokenNum, epiTokenNum, integralHeadNum, qSThisSubBlock, embed, oHiddenSize,
                    oHiddenSize_gmlo);
            } else {
                CopyOToGm(
                    gOutput, proTokenIdx, proTokenNum, epiTokenNum, integralHeadNum, qSThisSubBlock, embed,
                    oHiddenSize);
            }
            ...
            if constexpr (LSE_MODE_ == LseMode::LSE_OUT) {
                if (isLastRowLoop) {
                    ...
                    AscendC::Ln<float, false>(
                        lse32_ubuf_tensor, glUbTensor, (uint64_t)0, CeilDiv(totalRowNum, FLOAT_VECTOR_SIZE),
                        AscendC::UnaryRepeatParams(1, 1, 8, 8));

                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::Add<float, false>(
                        lse32_ubuf_tensor, lse32_ubuf_tensor, gmUbTensor, (uint64_t)0,
                        CeilDiv(totalRowNum, FLOAT_VECTOR_SIZE), AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, 8));
                    AscendC::PipeBarrier<PIPE_V>();

                    // *** lse_block = expand_to_block(lse), 存放于 tv
                    AscendC::Brcb(
                        tvUbTensor.ReinterpretCast<uint32_t>(), lse32_ubuf_tensor.ReinterpretCast<uint32_t>(),
                    ...
                    if (qNThisSubBlock == 0U) {
                        ...
                        for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                            if (isSplitkv) {
                                AscendC::DataCopyPad(
                                    gCombineLse[qNIdx], tvUbTensor[qNIdx * qSBlockSize * FLOAT_BLOCK_SIZE],
                                    AscendC::DataCopyExtParams(
                                        qSBlockSize, sizeof(float), 0, (qHeads_gmlse - 1) * sizeof(float), 0));
                            }
                            AscendC::DataCopyPad(
                                gLse[qNIdx], tvUbTensor[qNIdx * qSBlockSize * FLOAT_BLOCK_SIZE],
                                AscendC::DataCopyExtParams(
    ...
    __aicore__ inline void operator()(
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod, bool isSplitkv,
        const LayoutLse& layoutgmLse, const LayoutInput& layoutgmLo)
    {
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                SubCoreCompute(
                    gOutputCurLoop, gInputCurLoop, gUpdateCurLoop, gLseThisSubBlock, gCombineLseSubBlock,
                    gCombineoSubBlock, layoutOutputCurLoop, layoutInputCurLoop, layoutUpdateCurLoop,
                    layoutOutLseThisSubBlock, qNThisSubBlock, qSThisSubBlock, inRowActualThisSubBlock, isFirstStackTile,
                    isLastStackTile, curStackTileMod, needRowLoop, (rowLoopIdx == rowLoop - 1U), rowOffsetLoop,
                    proTokenIdx, proTokenNum, epiTokenNum, integralHeadNum, isSplitkv, layoutgmLse, layoutgmLo);
            }
        }
    }
