...
public:
    ...
    void SubCoreCompute(
        ...
        uint32_t needRowLoop, uint32_t rowOffsetLoop, uint32_t qSThisSubBlock,
        uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum, uint32_t integralHeadNum)
    {
        ...
        uint32_t dmUbOffsetCurStackTile =
            curStackTileMod * MAX_ROW_NUM_SUB_CORE + rowOffsetLoop;
        ...
        if (isLastStackTile) {
            ...
            for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
                ...
                innerOGmOffset += embed;
                inner_go_ubuf_offset += qSThisSubBlock * embed;
            }
            if (epiTokenNum != 0) {
                AscendC::DataCopyPad(gOutput[innerOGmOffset],
                    goUbTensor16[inner_go_ubuf_offset],
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
        ...
        if (inRowActualThisSubBlock > 0) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                uint32_t rowActualCurLoop = (rowLoopIdx == (rowLoop - 1)) ?
                    inRowActualThisSubBlock - rowLoopIdx * rowTile : rowTile;
                ...
                SubCoreCompute(
                    ...
                    needRowLoop, rowOffsetLoop, qSThisSubBlock,
