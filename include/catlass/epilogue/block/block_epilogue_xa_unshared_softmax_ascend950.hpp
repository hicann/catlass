...
class BlockEpilogue<EpilogueAscend950XAUnsharedSoftmax, L1TileShape_, PType_, SType_> {
public:
    ...
    CATLASS_DEVICE void operator()(
        TensorDst& pOutL1, TensorSrc& qkRes, AscendC::GlobalTensor<ElementS>& unSharedMaxGm,
        AscendC::GlobalTensor<ElementS>& unSharedSumGm, uint16_t SYNC_QK_READY_FLAG, uint16_t SYNC_SOFTMAX_READY_FLAG,
        uint16_t QK_UB_RELEASE_FLAG, int8_t taskIdMod2, int8_t taskIdMod3)
    {
        ...
        if (n == 128) {
            ComputeMaskandScale<ElementS, S2_BASE_SIZE, NRangeIndex::N128>(
                inputAddr, maskUbUnRoll1, maskUbUnRoll2, maxUbAddr, static_cast<uint16_t>(m), tailN, scaleValue);
        } else if (n <= 64) {
...
private:
        ...
        __ubuf__ ElementS* maxUb, uint16_t m, uint32_t tailN, ElementS scale)
    {
        ...
        for (uint16_t i = 0; i < m; ++i) {
            if constexpr (NRange == NRangeIndex::N0_64) {
                ...
                Muls(srcVreg1, srcVreg1, scale, NRange == NRangeIndex::N128 ? pregFull : pregTailN);
                LoadAlign<uint32_t, PostLiteral::POST_MODE_UPDATE, MaskDist::DIST_DS>(
                    pregCompare0, (__ubuf__ uint32_t*&)maskUbUnRoll1, S2BaseSize);
                LoadAlign<uint32_t, PostLiteral::POST_MODE_UPDATE, MaskDist::DIST_DS>(
    ...
    CATLASS_DEVICE void CopyOutMaxAndSum(
        AscendC::GlobalTensor<ElementS> maxGm, AscendC::GlobalTensor<ElementS> sumGm, uint16_t tailM)
    {
        AscendC::DataCopyExtParams copyOutParams(1, static_cast<uint32_t>(tailM) * sizeof(ElementS), 0, 0, 0);
