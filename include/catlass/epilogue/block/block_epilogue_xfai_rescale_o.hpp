...
public:
    ...
    __aicore__ inline void SubCoreCompute(
        ...
        const LayoutLse& layoutLse, uint32_t qNThisSubBlock, uint32_t isFirstStackTile, uint32_t isLastStackTile,
        uint32_t curStackTileMod, uint32_t needRowLoop, uint32_t isLastRowLoop, uint32_t rowOffsetLoop,
        uint32_t qSThisSubBlock, uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum,
            ...
            if constexpr (LSE_MODE_ == LseMode::LSE_OUT) {
                if (isLastRowLoop) {
                    ...
                    AscendC::Ln<float, false>(
                        lse32_ubuf_tensor, glUbTensor, (uint64_t)0, 1, AscendC::UnaryRepeatParams(1, 1, 8, 8));

                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::Add<float, false>(
                        lse32_ubuf_tensor, lse32_ubuf_tensor, gmUbTensor, (uint64_t)0, 1,
                        AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, 8));
                    AscendC::PipeBarrier<PIPE_V>();
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
        ...
        uint32_t outLseRowOffsetThisSubBlock = (qNBlockSize == 1) ? inRowOffsetThisSubBlock : 0;
        uint32_t outLseColOffsetThisSubBlock = (qNBlockSize == 1) ? 0 : subBlockIdx * qNSplitSubBlock;
        ...
        if (inRowActualThisSubBlock > 0) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                SubCoreCompute(
                    gOutputCurLoop, gInputCurLoop, gUpdateCurLoop, gLseThisSubBlock, layoutOutputCurLoop,
                    layoutInputCurLoop, layoutUpdateCurLoop, layoutOutLseThisSubBlock, qNThisSubBlock, isFirstStackTile,
                    isLastStackTile, curStackTileMod, needRowLoop, (rowLoopIdx == rowLoop - 1), rowOffsetLoop,
                    qSThisSubBlock, proTokenIdx, proTokenNum, epiTokenNum, integralHeadNum);
            }
        }
    }
