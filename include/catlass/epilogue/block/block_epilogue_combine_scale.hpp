...
public:
    ...
    BlockEpilogue(Arch::Resource<ArchTag>& resource)
    {
        ...
        */
        ...
        unsharedGlUbTensor = resource.ubBuf.template GetBufferByByte<float>(
            ATTEN_OUT_OFFSET + 2 * SUM_MAX_UB_PINGPONG_OFFSET); // 132k - 136k
        sharedGmUbTensor = resource.ubBuf.template GetBufferByByte<float>(
            ATTEN_OUT_OFFSET + 4 * SUM_MAX_UB_PINGPONG_OFFSET); // 136k - 140k
        unsharedGmUbTensor = resource.ubBuf.template GetBufferByByte<float>(
            ATTEN_OUT_OFFSET + 6 * SUM_MAX_UB_PINGPONG_OFFSET); // 140k - 144k
        realGmUbTensor = resource.ubBuf.template GetBufferByByte<float>(
            ATTEN_OUT_OFFSET + 8 * SUM_MAX_UB_PINGPONG_OFFSET); // 144k - 148k
        realGlUbTensor = resource.ubBuf.template GetBufferByByte<float>(
            ATTEN_OUT_OFFSET + 10 * SUM_MAX_UB_PINGPONG_OFFSET); // 148k - 152k
        outUbTensor = resource.ubBuf.template GetBufferByByte<ElementOutput>(
            ATTEN_OUT_OFFSET + 12 * SUM_MAX_UB_PINGPONG_OFFSET); // 152k - 184k
    }
    ...
    CATLASS_DEVICE
    void operator()(
        AscendC::GlobalTensor<float> gSharedGm, AscendC::GlobalTensor<float> gUnsharedGm,
        AscendC::GlobalTensor<float> gSharedGl, AscendC::GlobalTensor<float> gUnsharedGl,
        AscendC::GlobalTensor<float> gSharedOut, AscendC::GlobalTensor<float> gUnsharedOut,
        AscendC::GlobalTensor<ElementOutput> gFinalOutput, const MatrixCoord& actualBlockShape)
    {
        // 伪代码实现：
        // 1. 计算分块参数
        ...
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum + preLoad; rowLoopIdx++) {
            if (rowLoopIdx < rowLoopNum) {
                ...
                AscendC::DataCopy(
                    sharedGmUbTensor[pingPongOffset], gSharedGm[sumMaxOffsetIoShared],
                    rowNumCurLoop * SOFTMAX_BROAD_SIZE);
                // Copy GL
                AscendC::DataCopy(
                    sharedGlUbTensor[pingPongOffset], gSharedGl[sumMaxOffsetIoShared],
                    rowNumCurLoop * SOFTMAX_BROAD_SIZE);

                // Copy Shared and Unshared
                // [rowNum, column]
                if (rowNumCurLoop % FLOAT_BLOCK_SIZE == 0) {
                    AscendC::DataCopy(unsharedGmUbTensor[pingPongOffset], gUnsharedGm[sumMaxOffsetIoGm], rowNumCurLoop);
                    AscendC::DataCopy(unsharedGlUbTensor[pingPongOffset], gUnsharedGl[sumMaxOffsetIoGm], rowNumCurLoop);
                } else {
                    AscendC::DataCopyExtParams copyInParams{
                        1, static_cast<uint32_t>(rowNumCurLoop * sizeof(float)), 0, 0, 0};
                    AscendC::DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
                    AscendC::DataCopyPad(
                        unsharedGmUbTensor[pingPongOffset], gUnsharedGm[sumMaxOffsetIoGm], copyInParams, padParams);
                    AscendC::DataCopyPad(
                        unsharedGlUbTensor[pingPongOffset], gUnsharedGl[sumMaxOffsetIoGm], copyInParams, padParams);
                }
                AscendC::DataCopy(
                    sharedOutUbTensor[attnPingPongOffset], gSharedOut[attnOffsetIoGm], rowNumCurLoop * columnNum);
                AscendC::DataCopy(
                    unsharedOutUbTensor[attnPingPongOffset], gUnsharedOut[attnOffsetIoGm], rowNumCurLoop * columnNum);
            }
            ...
            if (rowLoopIdx >= preLoad) {
                ...
                SubCoreCompute(
                    sharedGmUbTensor[pingPongOffset], unsharedGmUbTensor[pingPongOffset],
                    realGmUbTensor[pingPongOffset], sharedGlUbTensor[pingPongOffset],
                    unsharedGlUbTensor[pingPongOffset], realGlUbTensor[pingPongOffset],
                    sharedOutUbTensor[attnPingPongOffset], unsharedOutUbTensor[attnPingPongOffset], pingpongFlag,
                    rowNumCurLoop, rowNumCurLoopRound, columnNum, columnNumRound);
                ...
                if constexpr (std::is_same_v<ElementOutput, bfloat16_t>) {
                    AscendC::Cast<ElementOutput, float, false>(
                        ...
                        sharedOutUbTensor[attnPingPongOffset], AscendC::RoundMode::CAST_RINT, (uint64_t)0,
                    ...
                    AscendC::Cast<ElementOutput, float, false>(
                        ...
                        sharedOutUbTensor[attnPingPongOffset], AscendC::RoundMode::CAST_NONE, (uint64_t)0,
                        (rowNumCurLoop * columnNumRound + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                        AscendC::UnaryRepeatParams(1, 1, 4, 8));
                }
                ...
                if constexpr (std::is_same_v<ElementOutput, bfloat16_t>) {
                    AscendC::DataCopy(
                        gFinalOutput[attnOffsetIoGm],
                        outUbTensor.template ReinterpretCast<bfloat16_t>()[attnPingPongOffset],
                        rowNumCurLoop * columnNum);
                } else {
                    AscendC::GlobalTensor<ElementOutput> gFinal = gFinalOutput[attnOffsetIoGm];
                    AscendC::DataCopy(
                        gFinal, outUbTensor.template ReinterpretCast<half>()[attnPingPongOffset],
                        rowNumCurLoop * columnNum);
                }
...
private:
    ...
    CATLASS_DEVICE
    void SubCoreCompute(
        AscendC::LocalTensor<float> sharedGmUbLoopTensor, AscendC::LocalTensor<float> unsharedGmUbLoopTensor,
        AscendC::LocalTensor<float> realGmUbLoopTensor, AscendC::LocalTensor<float> sharedGlUbLoopTensor,
        AscendC::LocalTensor<float> unsharedGlUbLoopTensor, AscendC::LocalTensor<float> realGlUbLoopTensor,
        AscendC::LocalTensor<float> sharedOutUbLoopTensor, AscendC::LocalTensor<float> unsharedOutUbLoopTensor,
        uint32_t pingpongFlag, uint32_t rowNumCurLoop, uint32_t rowNumCurLoopRound, uint32_t columnNum,
        uint32_t columnNumRound)
    {
        ...
        AscendC::BlockReduceMax<float, false>(
            sharedGmUbLoopTensor, sharedGmUbLoopTensor, rowNumCurLoop, uint64_t(0), 1, 1, 8);
        AscendC::PipeBarrier<PIPE_V>();

        AscendC::BlockReduceMax<float, false>(
            sharedGlUbLoopTensor, sharedGlUbLoopTensor, rowNumCurLoop, uint64_t(0), 1, 1, 8);
        AscendC::PipeBarrier<PIPE_V>();

        AscendC::Max<float>(realGmUbLoopTensor, sharedGmUbLoopTensor, unsharedGmUbLoopTensor, rowNumCurLoop);
        AscendC::PipeBarrier<PIPE_V>();

        // 2. update_shared_expgm = exp(shared_gm - gm)
        //    update_unshared_expgm = exp(unshared_gm - gm)
        AscendC::Sub<float>(sharedGmUbLoopTensor, sharedGmUbLoopTensor, realGmUbLoopTensor, rowNumCurLoop);
        AscendC::Sub<float>(unsharedGmUbLoopTensor, unsharedGmUbLoopTensor, realGmUbLoopTensor, rowNumCurLoop);
        AscendC::PipeBarrier<PIPE_V>();

        AscendC::Exp<float>(sharedGmUbLoopTensor, sharedGmUbLoopTensor, rowNumCurLoop);
        AscendC::Exp<float>(unsharedGmUbLoopTensor, unsharedGmUbLoopTensor, rowNumCurLoop);
        AscendC::PipeBarrier<PIPE_V>();
        ...
        AscendC::Mul<float>(sharedGlUbLoopTensor, sharedGlUbLoopTensor, sharedGmUbLoopTensor, rowNumCurLoop);

        // 计算 unshared_gl * update_unshared_expgm 并累加到 gl
        AscendC::Mul<float>(unsharedGlUbLoopTensor, unsharedGlUbLoopTensor, unsharedGmUbLoopTensor, rowNumCurLoop);
        AscendC::PipeBarrier<PIPE_V>();

        AscendC::Add<float>(realGlUbLoopTensor, sharedGlUbLoopTensor, unsharedGlUbLoopTensor, rowNumCurLoop);
        AscendC::PipeBarrier<PIPE_V>();

        // BroadCast update_shared_expgm and update_unshared_expgm
        AscendC::Brcb(
            sharedGmUbLoopTensor, sharedGmUbLoopTensor, rowNumCurLoopRound / FLOAT_BLOCK_SIZE,
            AscendC::BrcbRepeatParams(1, 8));
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Brcb(
            unsharedGmUbLoopTensor, unsharedGmUbLoopTensor, rowNumCurLoopRound / FLOAT_BLOCK_SIZE,
            AscendC::BrcbRepeatParams(1, 8));
        AscendC::PipeBarrier<PIPE_V>();
        ...
        for (uint32_t mulIdx = 0; mulIdx < columnNum / FLOAT_VECTOR_SIZE; ++mulIdx) {
            AscendC::Mul<float, false>(
                sharedOutUbLoopTensor[mulIdx * FLOAT_VECTOR_SIZE], sharedOutUbLoopTensor[mulIdx * FLOAT_VECTOR_SIZE],
                sharedGmUbLoopTensor, AscendC::MASK_PLACEHOLDER, rowNumCurLoop,
        ...
        if (columnNum % FLOAT_VECTOR_SIZE > 0) {
            ...
            AscendC::Mul<float, false>(
                ...
                sharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], sharedGmUbLoopTensor,
        ...
        for (uint32_t mulIdx = 0; mulIdx < columnNum / FLOAT_VECTOR_SIZE; ++mulIdx) {
            AscendC::Mul<float, false>(
                ...
                unsharedOutUbLoopTensor[mulIdx * FLOAT_VECTOR_SIZE], unsharedGmUbLoopTensor, (uint64_t)0, rowNumCurLoop,
                AscendC::BinaryRepeatParams(
                    1, 1, 0, columnNumRound / FLOAT_BLOCK_SIZE, columnNumRound / FLOAT_BLOCK_SIZE, 1));
        }
        AscendC::PipeBarrier<PIPE_V>();
        if (columnNum % FLOAT_VECTOR_SIZE > 0) {
            ...
            AscendC::Mul<float, false>(
                ...
                unsharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], unsharedGmUbLoopTensor,
        ...
        for (uint32_t addIdx = 0; addIdx < columnNum / FLOAT_VECTOR_SIZE; ++addIdx) {
            AscendC::Add<float, false>(
                sharedOutUbLoopTensor[addIdx * FLOAT_VECTOR_SIZE], sharedOutUbLoopTensor[addIdx * FLOAT_VECTOR_SIZE],
                unsharedOutUbLoopTensor[addIdx * FLOAT_VECTOR_SIZE], (uint64_t)0, rowNumCurLoop,
                AscendC::BinaryRepeatParams(
                    1, 1, 1, columnNumRound / FLOAT_BLOCK_SIZE, columnNumRound / FLOAT_BLOCK_SIZE,
        ...
        if (columnNum % FLOAT_VECTOR_SIZE > 0) {
            ...
            AscendC::Add<float, false>(
                sharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE],
                sharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE],
                unsharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], (uint64_t)0, rowNumCurLoop,
                AscendC::BinaryRepeatParams(
                    1, 1, 1, columnNumRound / FLOAT_BLOCK_SIZE, columnNumRound / FLOAT_BLOCK_SIZE,
                    columnNumRound / FLOAT_BLOCK_SIZE));
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
        ...
        // 除以 gl
        AscendC::Brcb(
            realGlUbLoopTensor, realGlUbLoopTensor, rowNumCurLoopRound / FLOAT_BLOCK_SIZE,
            AscendC::BrcbRepeatParams(1, 8));
        AscendC::PipeBarrier<PIPE_V>();
        for (uint32_t divIdx = 0; divIdx < columnNum / FLOAT_VECTOR_SIZE; ++divIdx) {
            AscendC::Div<float, false>(
                sharedOutUbLoopTensor[divIdx * FLOAT_VECTOR_SIZE], sharedOutUbLoopTensor[divIdx * FLOAT_VECTOR_SIZE],
                realGlUbLoopTensor, (uint64_t)0, rowNumCurLoop,
                AscendC::BinaryRepeatParams(
                    1, 1, 0, columnNumRound / FLOAT_BLOCK_SIZE, columnNumRound / FLOAT_BLOCK_SIZE, 1));
        }
        AscendC::PipeBarrier<PIPE_V>();
        if (columnNum % FLOAT_VECTOR_SIZE > 0) {
            ...
            AscendC::Div<float, false>(
                sharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE],
                sharedOutUbLoopTensor[columnNum / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], realGlUbLoopTensor,
                (uint64_t)0, rowNumCurLoop,
                AscendC::BinaryRepeatParams(
                    1, 1, 0, columnNumRound / FLOAT_BLOCK_SIZE, columnNumRound / FLOAT_BLOCK_SIZE, 1));
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
