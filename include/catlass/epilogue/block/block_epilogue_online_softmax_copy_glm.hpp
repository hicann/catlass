...
public:
    ...
    void CopyMaskGmToUb(
        ...
        uint32_t qSBlockSize, uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t integralHeadNum,
    ...
    CATLASS_DEVICE void SubCoreCompute(
        AscendC::GlobalTensor<ElementOutput> gOutput, AscendC::GlobalTensor<ElementUpdate> gSharedMax,
        AscendC::GlobalTensor<ElementUpdate> gSharedSum, const LayoutOutput& layoutOutput, uint32_t rowOffset,
        uint32_t isFirstStackTile, uint32_t columnNumRound, uint32_t pingpongFlag, uint32_t curStackTileMod,
        uint32_t isLastStackTile, uint32_t headNum)
    {
        ...
        if (isLastStackTile) {
            ...
            AscendC::LocalTensor<float> sharedGmUbTensor = maskUbTensor.template ReinterpretCast<
                float>()[pingpongFlag * ROW_SUM_PINGPONG_OFFSET + rowOffset * FLOAT_BLOCK_SIZE];
            AscendC::Brcb(
                sharedGmUbTensor, gmUbTensor[rowOffset], rowNumCurLoopRound / FLOAT_BLOCK_SIZE,
                AscendC::BrcbRepeatParams(1, 8));
            // AscendC::PipeBarrier<PIPE_V>();
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(pingpongFlag);
            AscendC::DataCopy(gSharedMax, sharedGmUbTensor, AscendC::DataCopyParams(rowNumCurLoop, 1, 0, headNum - 1));
        }
        ...
        if (isLastStackTile) {
            // AscendC::LocalTensor<float> copyTempTensor = maskUbTensor32.template
            // ReinterpretCast<float>()[pingpongFlag * 128];
            AscendC::SetFlag<AscendC::HardEvent::MTE3_V>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(pingpongFlag);
            AscendC::LocalTensor<float> sharedGlUbTensor = maskUbTensor.template ReinterpretCast<
                float>()[pingpongFlag * ROW_SUM_PINGPONG_OFFSET + 2048 + rowOffset * FLOAT_BLOCK_SIZE];
            AscendC::Brcb(
                sharedGlUbTensor, glUbTensor[rowOffset], rowNumCurLoopRound / FLOAT_BLOCK_SIZE,
                AscendC::BrcbRepeatParams(1, 8));
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(pingpongFlag);
            AscendC::DataCopy(gSharedSum, sharedGlUbTensor, AscendC::DataCopyParams(rowNumCurLoop, 1, 0, headNum - 1));
        }
    }
    ...
    void operator()(
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t qSBlockSize, uint32_t qNBlockSize,
        uint32_t curStackTileMod, uint32_t headNum)
    {
        ...
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum + preLoad; rowLoopIdx++) {
            ...
            if (rowLoopIdx >= preLoad) {
                ...
                SubCoreCompute<MaskCategory::NO_MASK>(
                    gOutputCurLoop, gSharedMaxCurLoop, gSharedSumCurLoop, layoutOutputCurLoop, rowOffsetCurLoop,
                    isFirstStackTile, columnNumRound, pingpongFlag, curStackTileMod, isLastStackTile, headNum);
            }
        }
    }
...
template <class OutputType_, class InputType_, class MaskType_>
class BlockEpilogue<EpilogueAscend950OnlineSoftmaxCopySumMax, OutputType_, InputType_, MaskType_>
