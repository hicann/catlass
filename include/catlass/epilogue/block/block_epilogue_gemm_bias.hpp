...
#ifndef CATLASS_EPILOGUE_BLOCK_EPILOGUE_GEMM_BIAS_HPP
#define CATLASS_EPILOGUE_BLOCK_EPILOGUE_GEMM_BIAS_HPP
...
public:
    // Type aliases
    using DispatchPolicy = EpilogueAtlasA2VectorBias;
    ...
    static_assert(std::is_same_v<LayoutC, layout::RowMajor> && std::is_same_v<LayoutBias, layout::VectorLayout> &&
    ...
    struct Params {
        ...
        CATLASS_DEVICE
        Params(GM_ADDR ptrBias_, LayoutBias const &layoutBias_, GM_ADDR ptrD_, LayoutD const &layoutD_)
            : ptrBias(ptrBias_), layoutBias(layoutBias_), ptrD(ptrD_), layoutD(layoutD_) {}
    };
    ...
    )
    {
        ...
        // aicore上根据aiv数量row方向切分为subblock
        ...
        // layout包含实际subblock的row，col信息，但stride为layoutC的stride
        ...
        auto layoutBiasInUb = LayoutBias::template MakeLayoutInUb<ElementBias>(biasSubblockShape);
        ...
        copyGmToubBias(ubBias, gmSubblockBias, layoutBiasInUb, layoutSubblockBias);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);

        // Perform epilogue calculation
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(EVENT_ID0);
        tileEpilogue(ubD, ubC, ubBias);
        AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(EVENT_ID0);
