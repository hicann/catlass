...
void Run(Options const &options)
{
    ...
    for (uint32_t i = 0; i < groupCnt; ++i)
    {
        ...
        layoutBList[i] = LayoutB::template MakeLayout<int4b_t>(options.kList[i], options.nList[i]);
    ...
    // 左右矩阵全随机
    golden::FillRandomData<int4_t>(hostA, -8, 7);
    golden::FillRandomData<int4_t>(hostB, -8, 7);
    golden::FillRandomData<bfloat16>(hostQ, 1.0, 1.0);
    kernelQ = convertQuantTensor(hostQ);

    std::vector<float> hostGolden(allMNCnt);

    golden::ComputeGroupMatmulPerGroupQuant(groupCnt, problemShapeList, hostA, layoutAList, hostB, layoutBList, hostQ, layoutQList, hostGolden, layoutCList);
    ...
    using AType = Gemm::GemmType<int4b_t, LayoutA>;
    using BType = Gemm::GemmType<int4b_t, LayoutB>;
    using CType = Gemm::GemmType<OutCKernelType, LayoutC>;
    using QType = Gemm::GemmType<uint64_t, LayoutQ>;

    using BlockMmad = Gemm::Block::BlockGmmQuant<DispatchPolicy, L1TileShape, L0TileShape, AType, BType, CType, QType>;
    ...
    // kernel level
    using MatmulKernel = Gemm::Kernel::MatmulA4W4<BlockMmad, BlockEpilogue, BlockScheduler>;
    ...
    std::vector<uint64_t> errorIndices = golden::CompareData(hostC, hostGolden, options.kList[0]);
