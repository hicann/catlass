...
public:
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(FAIKernelParams const &params)
    {
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            ...
            LayoutK layoutKTemp = LayoutK::template MakeLayout<ElementK>(strideKV, blockStackNum * pagedBlockSize);
            LayoutV layoutVTemp = LayoutV::template MakeLayout<ElementV>(blockStackNum * pagedBlockSize, strideKV);
