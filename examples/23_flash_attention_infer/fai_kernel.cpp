...
public:
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            ...
            uint32_t maskSIdx = noMaskTailS != 0 ? kvSLoopNumNoMask - 1 : kvSLoopNumNoMask;
            ...
            for (uint32_t kvSIdx = maskSIdx; kvSIdx < kvSLoopNumTotal + preKVNum; kvSIdx += blockStackNum) {
                if (kvSIdx < kvSLoopNumTotal) {
                    stackSeqTile = qSBlockSize;
                    uint32_t SWorkSpacePingPongFlag = stackSeqCount % (preLaunch + 1);
                    uint64_t gmSOffset = coreIdx * WORKSPACE_BLOCK_SIZE_DB * 2 + SWorkSpacePingPongFlag * WORKSPACE_BLOCK_SIZE_DB;
                    GemmCoord actualBlockShapeQK{rowNum, stackSeqTile, embed};
                    if constexpr (PAGED_CACHE_FLAG) {
                        blockMmadQK(gQ[gmQOffset], gK[gmKOffset], gS[gmSOffset],
                            gBlockTable[blockBOffset], layoutQTemp, layoutKTemp, actualBlockShapeQK,
                ...
                if (kvSIdx == maskSIdx) {
                    ...
                    stackSeqTile = stackSeqTile != 0 ? stackSeqTile : pagedBlockSize * blockStackNum;
                    nowkvSIdx = kvSLoopNumNoMask - noMaskEndStackNum;
                }
                uint32_t PVWorkSpacePingPongFlag = (stackSeqCount - preLaunch) % (preLaunch + 1);
                uint64_t gmPOffset = coreIdx * WORKSPACE_BLOCK_SIZE_DB * (preLaunch + 1) + PVWorkSpacePingPongFlag * WORKSPACE_BLOCK_SIZE_DB;
                uint64_t gmOTmpOffset = coreIdx * WORKSPACE_BLOCK_SIZE_DB * (preLaunch + 1) + PVWorkSpacePingPongFlag * WORKSPACE_BLOCK_SIZE_DB;
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            ...
            for (uint32_t kvSIdx = maskedStartIdx;
                ...
                if (kvSIdx == maskedStartIdx) {
                    stackSeqTile = noMaskKvS % (pagedBlockSize * blockStackNum);
                    stackSeqTile = stackSeqTile != 0 ? stackSeqTile : pagedBlockSize * blockStackNum;
                    delayedKvSIdx = kvSLoopNumNoMask - noMaskEndStackNum;
                }
