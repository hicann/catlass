...
public:
    ...
    CATLASS_DEVICE
    uint32_t GetQNBlockTile(uint32_t qSeqlen, uint32_t groupSize, uint32_t curQSBlockTile)
    {
        uint32_t qNBlockTile = curQSBlockTile / qSeqlen;
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        AscendC::GlobalTensor<ElementOTmp> gO;
        gO.SetGlobalBuffer((__gm__ ElementOTmp *)params.o);
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            ...
            for (uint32_t kvSIdx = 0; kvSIdx < kvSLoopNumNoMask + preKVNum; kvSIdx += blockStackNum) {
                if (kvSIdx < kvSLoopNumNoMask) {
                    ...
                    if constexpr (PAGED_CACHE_FLAG) {
                        blockMmadQK(gQ[gmQOffset], gK, gS[gmSOffset],
                            gBlockTable, layoutQTemp, layoutKTemp,
                ...
                if (kvSIdx >= preKVNum) {
                    ...
                    if constexpr (PAGED_CACHE_FLAG) {
                        blockMmadPV(gP[gmPOffset], gV, gOTmp[gmOTmpOffset],
                            gBlockTable, layoutPTemp, layoutVTemp,
            ...
            //         LayoutS layOutSTemp(rowNum, stackSeqTile, stackSeqTileRound);
            //         if constexpr (PAGED_CACHE_FLAG) {
            //             if (kvSIdx == kvSLoopNumNoMask) {
            ...
            //             }
            //             blockMmadQK(gQ[gmQOffset], gK, gS[gmSOffset],
            //                 gBlockTable, layoutQTemp, layoutKTemp, layOutSTemp,
            ...
            //         uint32_t gmPOffset = coreIdx * WORKSPACE_BLOCK_SIZE_DB * (preLaunch + 1) + PVWorkSpacePingPongFlag * WORKSPACE_BLOCK_SIZE_DB;
            //         uint32_t gmOTmpOffset = coreIdx * WORKSPACE_BLOCK_SIZE_DB * (preLaunch + 1) + PVWorkSpacePingPongFlag * WORKSPACE_BLOCK_SIZE_DB;
            //         LayoutP layoutPTemp(rowNum, stackSeqTile, stackSeqTileRound);
            //         LayoutOTmp layoutOTmpTemp(rowNum, embed, embedRound);
            //         if constexpr (PAGED_CACHE_FLAG) {
            //             if (nowkvSIdx == kvSLoopNumNoMask) {
            //                 maskTailS = kvSLoopNumNoMask * pagedBlockSize - noMaskKvS;
            //             } else {
            //                 maskTailS = 0;
            //             }
            //             blockMmadPV(gP[gmPOffset], gV, gO[gmOTmpOffset],
            //                 gBlockTable, layoutPTemp, layoutVTemp, layoutOTmpTemp,
            //                 nowkvSIdx, kvSLoopNumNoMask, pagedBlockSize, noSkipKvS, strideKV, softmaxReady, maskTailS);
            //         } else {
            //             blockMmadPV(gP[gmPOffset], gV[gmVOffset], gO[gmOTmpOffset],
            //                 gBlockTable, layoutPTemp, layoutVTemp, layoutOTmpTemp,
                                ...
                                GM_ADDR tiling)
{
    ...
    using DispatchPolicyOnlineSoftmax = Epilogue::EpilogueAtlasA2MLASoftmax;
    ...
    using DispatchPolicyRescaleO = Epilogue::EpilogueAtlasA2MLARescaleO;
    using OType = Gemm::GemmType<ElementO, LayoutO>;
    using EpilogueRescaleO =
        Epilogue::Block::BlockEpilogue<DispatchPolicyRescaleO, OType, OType, OTmpType>;
