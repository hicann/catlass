...
{
    uint64_t mm1OutSize = blockDim * WORKSPACE_BLOCK_SIZE_DB * NUM4 * NUM2;
    uint64_t smOnlineOutSize = blockDim * WORKSPACE_BLOCK_SIZE_DB * NUM2 * NUM2;
    uint64_t mm2OutSize = blockDim * WORKSPACE_BLOCK_SIZE_DB * NUM4 * NUM2;
    uint64_t UpdateSize = blockDim * WORKSPACE_BLOCK_SIZE_DB * NUM4 * NUM2;
...
int32_t GetFATilingParam(const FAInfo &faInfo, uint32_t blockDim, FATilingData &faTilingData)
{
    ...
    for (int32_t batchIdx = 0; batchIdx < faInfo.batch; batchIdx++) {
        ...
        int64_t kvSeqlen = *(faInfo.kvSeqlenList + batchIdx);
        if (qSeqlen > kvSeqlen) {
            cerr << "[ERROR] qSeqlen > kvSeqlen is not supported." << endl;
        }
