...
def gen_data(groupCnt):
    ...
    for i in range(groupCnt):
        ...
        A = np.random.uniform(-0.125, 0.125, size=(K, M)).astype(NP_DATA_TYPE)
        B = np.random.uniform(-0.125, 0.125, size=(N, K)).astype(NP_DATA_TYPE)
        C = np.random.uniform(-0.125, 0.125, size=(N, M)).astype(NP_DATA_TYPE)
