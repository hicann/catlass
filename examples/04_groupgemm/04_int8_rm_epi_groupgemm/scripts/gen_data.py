...
def gen_data(groupCnt):
    ...
    for i in range(groupCnt):
        ...
        A = np.random.uniform(-2, 1, size=(M, K)).astype(NP_DATA_TYPE)
        B = np.random.uniform(-2, 1, size=(K, N)).astype(NP_DATA_TYPE)
        C = np.random.uniform(-2, 1, size=(M, N)).astype(DATA_TYPE)
