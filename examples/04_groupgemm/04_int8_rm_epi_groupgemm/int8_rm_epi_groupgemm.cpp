...
void INT8RMGroupGemm(
    ...
    layout::RowMajor layoutC{1024, 100};  //暂用
    typename EpilogueBlock::Params epilogueParams{1, 1, gmC, layoutC, gmC, layoutC}; // x只是传了一个地址
    // 实例化Gemm部分
...
void Run(Options& options){
    ...
    size_t sizeA = allMKCnt * sizeof(int8_t);
    int8_t* hostA;
    ...
    size_t sizeB = allKNCnt * sizeof(int8_t);
    int8_t* hostB;
    ...
    size_t sizeC = allMNCnt * sizeof(int32_t);
    int32_t* hostC;
    ...
    size_t sizeX = allMNCnt * sizeof(int32_t);
    ...
    INT8RMGroupGemm<LayoutA, LayoutB, LayoutC><<<aicCoreNum, nullptr, stream>>>(
        groupCnt,
        fftsAddr,
