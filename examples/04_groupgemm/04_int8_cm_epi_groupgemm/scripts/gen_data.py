...
def gen_data(groupCnt):
    ...
    for i in range(groupCnt):
        ...
        A = np.random.uniform(-0.5, 0.5, size=(K, M)).astype(NP_DATA_TYPE)
        B = np.random.uniform(-0.5, 0.5, size=(N, K)).astype(NP_DATA_TYPE)
        C = np.random.uniform(-0.5, 0.5, size=(N, M)).astype(DATA_TYPE)
        ...
        alpha = np.random.uniform(-2, 1, 1).astype(DATA_TYPE)
        beta = np.random.uniform(-2, 1, 1).astype(DATA_TYPE)
        ...
        expect_res = np.array(tmp * alpha + beta * C.astype(DATA_TYPE)).astype(DATA_TYPE)
