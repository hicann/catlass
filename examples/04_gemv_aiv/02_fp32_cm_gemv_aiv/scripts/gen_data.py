...
def calc_expect_func(M, N):
    ...
    # matrix_gm = np.random.randint(1, 2, (M, N)).astype(NP_DATA_TYPE)
    # vector_gm = np.random.randint(1, 2, (N, 1)).astype(NP_DATA_TYPE)
    # vector_y_gm = np.random.randint(1, 2, (M, 1)).astype(NP_DATA_TYPE)
