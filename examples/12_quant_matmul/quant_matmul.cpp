...
void Run(Options const &options) {
    ...
    size_t sizeScale = lenScale * sizeof(float16);
    size_t sizePerTokenScale = lenPerTokenScale * sizeof(float16);
    size_t sizeD = lenD * sizeof(float16);
    size_t sizeWorkspace;

    std::vector<int8_t> hostA(lenA);
    std::vector<int8_t> hostB(lenB);
    std::vector<float16> hostScale(lenScale);
    std::vector<float16> hostPerTokenScale(lenPerTokenScale);
