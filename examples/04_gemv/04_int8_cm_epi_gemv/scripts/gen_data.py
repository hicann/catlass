...
def calc_expect_func(M, N):
    ...
    SizeA = (N, M) # 首先全是行优先
    Sizey = (1, M)
    alpha = np.random.uniform(-10,10, 1).astype(DATA_TYPE)
    beta  = np.random.uniform(-10,10, 1).astype(DATA_TYPE)
    x = np.random.uniform(-10,10,size=Sizex).astype(NP_DATA_TYPE)
    A = np.random.uniform(-10,10,size=SizeA).astype(NP_DATA_TYPE)
    y = np.random.uniform(-10,10,size=Sizey).astype(DATA_TYPE)
    ...
    # D = alpha * A * x + beta * y
    expect_res = np.array(tmp * alpha.astype(DATA_TYPE) + beta.astype(DATA_TYPE) * y.astype(DATA_TYPE)).astype(DATA_TYPE)
