...
def calc_expect_func(M, N):
    ...
    SizeA = (M, N) # 首先全是行优先
    Sizey = (1, M)
    alpha = np.random.uniform(-5,5, 1).astype(DATA_TYPE)
    beta  = np.random.uniform(-5,5, 1).astype(DATA_TYPE)
    x = np.random.uniform(-5,5,size=Sizex).astype(NP_DATA_TYPE)
    A = np.random.uniform(-5,5,size=SizeA).astype(NP_DATA_TYPE)
    y = np.random.uniform(-5,5,size=Sizey).astype(DATA_TYPE)
