...
bool IsNeedPadding(Act::layout::RowMajor layout, uint32_t align)
{
...
bool IsNeedPadding(Act::layout::ColumnMajor layout, uint32_t align)
{
    // If the stride is greater than 65536, padding is required to reduce the stride.
...
ACT_DEVICE void LaunchMatmulDynamicSwizzle(Act::GemmCoord problemShape, GM_ADDR gmA, LayoutA layoutA, GM_ADDR gmB,
                                               ...
                                               LayoutWA layoutWA, GM_ADDR gmWB, LayoutWB layoutWB)
{
    if (problemShape.m() > problemShape.n()) {
        ...
        // kernel level
        using MatmulKernel = Act::Gemm::Kernel::OptimizedMatmul<BlockMmad, BlockEpilogue, BlockScheduler>;
                                     ...
                                     GM_ADDR gmWB)
{
    ...
    using L1TileShape =
        std::conditional_t<std::is_same_v<LayoutA, Act::layout::ColumnMajor> && std::is_same_v<LayoutB, Act::layout::ColumnMajor>,
                           Act::GemmShape<256, 128, 256>, Act::GemmShape<128, 256, 256>>;
    using L0TileShape =
        std::conditional_t<std::is_same_v<LayoutA, Act::layout::ColumnMajor> && std::is_same_v<LayoutB, Act::layout::ColumnMajor>,
                           Act::GemmShape<256, 128, 64>, Act::GemmShape<128, 256, 64>>;
    ;
    if (gmA == gmWA && gmB == gmWB) {
        // no need to padding A and B.
        using LayoutWA = LayoutA;
        using LayoutWB = LayoutB;
        using AType = Act::Gemm::GemmType<half, LayoutWA>;
        using BType = Act::Gemm::GemmType<half, LayoutWB>;
        ...
        using LayoutWA = LayoutA;
        using LayoutWB = std::conditional_t<std::is_same_v<LayoutB, Act::layout::RowMajor>, Act::layout::PaddingRowMajor,
                                            Act::layout::PaddingColumnMajor>;
        using AType = Act::Gemm::GemmType<half, LayoutWA>;
        using BType = Act::Gemm::GemmType<half, LayoutWB>;
        using CType = Act::Gemm::GemmType<half, LayoutC>;
        using BlockMmad = Act::Gemm::Block::BlockMmad<DispatchPolicy, L1TileShape, L0TileShape, AType, BType, CType>;
        ...
        using LayoutWA = std::conditional_t<std::is_same_v<LayoutA, Act::layout::RowMajor>, Act::layout::PaddingRowMajor,
                                            Act::layout::PaddingColumnMajor>;
        using LayoutWB = LayoutB;
        using AType = Act::Gemm::GemmType<half, LayoutWA>;
        using BType = Act::Gemm::GemmType<half, LayoutWB>;
