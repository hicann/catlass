...
void FAImpl(const uint32_t blockNum, aclrtStream stream, const FAKernelInfo &kernelInfo)
{
    ...
    int32_t kvSeqlen = kernelInfo.kvSeqlen;
    int32_t numHeads = kernelInfo.numHeads;
    int32_t kvHeads = kernelInfo.kvHeads;
    int32_t embeddingSize = kernelInfo.embeddingSize;
    int32_t blockSize = kernelInfo.blockSize;
    int32_t maskType = kernelInfo.maskType;
    // aclDataType dataType = DType;
    ...
    uint64_t kvSize =
        (uint64_t)numBlocks * (uint64_t)blockSize * (uint64_t)kvHeads * (uint64_t)embeddingSize * sizeof(DType);
    uint64_t maskSize = 1024 * 1024 * sizeof(DType);
    ...
    uint8_t *oDevice = kernelInfo.outputAddr.at(0);
...
void FlashAttentionInfer(uint32_t blockNum, aclrtStream stream, ConvKernelInfo kernelInfo)
{
    ...
    } else if (kernelInfo.dataType == ACL_BF16) {
        FAImpl<bfloat16_t>(blockNum, stream, kernelInfo);
    }
}
} // namespace CatlassKernel