...
struct Options {
    const std::string HELPER = "70_950_grouped_matmul_slice_m_fixpipe_dequant group_count m n k quant_mode [device_id]";
    ...
    int Parse(int argc, const char **argv)
    {
        ...
        quantMode = std::atoi(argv[QUANT_MODE_INDEX]);
        if (argc == ARGS_MAX) {
...
void RunTest(Options const &options)
{
    ...
    double low = 0.0;
    double high = 1.0;
    double randomNum = low + (static_cast<double>(rand()) / static_cast<double>(RAND_MAX)) * (high - low);
    float scalePerTensor = static_cast<float>(randomNum);
    ...
    // kernel level
    using MatmulKernel = Gemm::Kernel::GroupedMatmulSliceMAswtTla<BlockMmadTla, BlockEpilogue, BlockScheduler, int64_t>;
