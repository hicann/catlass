...
void ComputeMatmul(const GemmCoord &problemShape,
                   const std::vector<ElementA> &dataA,
                   const LayoutA &layoutA,
                   const std::vector<ElementB> &dataB,
...
template <typename Element,
          ...
          class ElementGolden,
          class LayoutGolden>
void ComputeGemm(const GemmCoord &problemShape,
                 Element alpha,
                 Element beta,
                 const std::vector<ElementA> &dataA,
                 const LayoutA &layoutA,
                 const std::vector<ElementB> &dataB,
                 const LayoutB &layoutB,
                 const std::vector<ElementC> &dataC,
                 const LayoutC &layoutC,
                 std::vector<ElementGolden> &dataGolden,
                 const LayoutGolden &layoutGolden) {
    for (uint32_t i = 0; i < problemShape.m(); ++i) {
        for (uint32_t j = 0; j < problemShape.n(); ++j) {
            ...
            dataGolden[offsetGolden] = static_cast<ElementGolden>(beta)
                                           * static_cast<ElementGolden>(dataC[offsetGolden])
...
template <typename Element,
          ...
          class ElementGolden,
          class LayoutGolden>
void ComputeGemv(const Catlass::GemvCoord &problemShape,
                 Element alpha,
                 Element beta,
                 const std::vector<ElementA> &dataA,
                 const LayoutA &layoutA,
                 const std::vector<ElementX> &dataX,
                 const LayoutX &layoutX,
                 const std::vector<ElementY> &dataY,
...
template <typename Element,
          ...
          class ElementGolden,
          class LayoutGolden>
void ComputeGroupGemm(uint32_t problemCount,
                      const std::vector<GemmCoord> &problemShapeList,
                      const std::vector<Element> &alphaList,
                      const std::vector<Element> &betaList,
                      const std::vector<ElementA> &dataA,
                      const std::vector<LayoutA> &layoutAList,
                      const std::vector<ElementB> &dataB,
                      const std::vector<LayoutB> &layoutBList,
                      const std::vector<ElementC> &dataC,
                      const std::vector<LayoutC> &layoutCList,
                      std::vector<ElementGolden> &dataGolden,
    ...
    for (uint32_t inGroupId = 0; inGroupId < problemCount; ++inGroupId) {
        ...
        for (uint32_t i = 0; i < problemShape.m(); ++i) {
            for (uint32_t j = 0; j < problemShape.n(); ++j) {
                ...
                dataGolden[offsetGolden] = static_cast<ElementGolden>(beta) * static_cast<ElementGolden>(dataC[offsetC])
                                           + static_cast<ElementGolden>(accumulator);
            }
        }
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementGolden, class LayoutGolden>
void ComputeBatchedMatmul(const uint32_t batchedCount,
                          const GemmCoord &problemShape,
                          const std::vector<ElementA> &dataA,
                          const LayoutA &layoutA,
                          const std::vector<ElementB> &dataB,
                          const LayoutB &layoutB,
                          std::vector<ElementGolden> &dataC,
                          const LayoutGolden &layoutGolden) {
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementGolden, class LayoutGolden>
void ComputeGroupedMatmul(uint32_t problemCount,
                          const std::vector<GemmCoord> &problemShapeList,
                          const std::vector<ElementA> &dataA,
                          const std::vector<LayoutA> &layoutAList,
                          const std::vector<ElementB> &dataB,
...
template <class ElementA,
          ...
          class ElementGolden,
          class LayoutGolden>
void ComputeMatmulElemWiseAdd(const GemmCoord &problemShape,
                              const std::vector<ElementA> &dataA,
                              const LayoutA &layoutA,
                              const std::vector<ElementB> &dataB,
                              const LayoutB &layoutB,
...
void ComputeGroupedMatmulPerTokenDequant(const GemmCoord &problemShape,
                                         uint32_t problemCount,
                                         const std::vector<ElementGroupList> &groupList,
                                         const std::vector<int8_t> &dataA,
                                         const layout::RowMajor &layoutA,
                                         const std::vector<int8_t> &dataB,
                                         const LayoutB &layoutB,
                                         const std::vector<ElementScale> &dataScale,
                                         const LayoutScale &,
                                         const std::vector<ElementScale> &dataPerTokenScale,
                                         const LayoutPerTokenScale &,
                                         std::vector<float> &dataGolden,
...
void QuantMatmul(const GemmCoord &problemShape,
                 const std::vector<int8_t> &dataA,
                 const LayoutA &layoutA,
                 const std::vector<int8_t> &dataB,
                 const LayoutB &layoutB,
                 const std::vector<ElementScale> &dataScale,
                 const layout::VectorLayout &layoutScale,
                 const std::vector<ElementScale> &dataPerTokenScale,
                 const layout::VectorLayout &layoutPerTokenScale,
                 std::vector<float> &dataGolden,
                 const layout::RowMajor &layoutGolden) {
    for (uint32_t i = 0; i < problemShape.m(); ++i) {
        for (uint32_t j = 0; j < problemShape.n(); ++j) {
...
void ComputeGroupedMatmulSliceKPerTokenDequant(const GemmCoord &problemShape,
                                               uint32_t problemCount,
                                               const std::vector<ElementGroupList> &groupList,
                                               const std::vector<int8_t> &dataA,
                                               const layout::ColumnMajor &layoutA,
                                               const std::vector<int8_t> &dataB,
                                               const layout::RowMajor &layoutB,
                                               const std::vector<ElementScale> &dataScale,
                                               const LayoutScale &,
                                               const std::vector<ElementScale> &dataPerTokenScale,
                                               const LayoutPerTokenScale &,
        ...
        for (uint32_t i = 0; i < problemShape.m(); ++i) {
            for (uint32_t j = 0; j < problemShape.n(); ++j) {
                ...
                dataGolden[groupOffsetD + offsetGolden] = static_cast<float>(accumulator)
                                                          * static_cast<float>(dataScale[groupOffsetScale + j])
                                                          * static_cast<float>(
                                                              dataPerTokenScale[groupOffsetPerTokenScale + i]);
            }
        }
...
template <class ElementA,
          ...
          class LayoutGolden,
          class ElementBias>
void ComputeMatmulBias(const GemmCoord &problemShape,
                       const std::vector<ElementA> &dataA,
                       const LayoutA &layoutA,
                       const std::vector<ElementB> &dataB,
                       const LayoutB &layoutB,
...
template <class ElementA,
          ...
          class LayoutGolden,
          class Activation>
void ComputeMatmulElemWiseActivation(const GemmCoord &problemShape,
                                     const std::vector<ElementA> &dataA,
                                     const LayoutA &layoutA,
                                     const std::vector<ElementB> &dataB,
                                     const LayoutB &layoutB,
...
template <class ElementT> struct Relu {
    ElementT operator()(ElementT val) const { return val < ElementT(0) ? ElementT(0) : val; }
};
...
template <class ElementT> struct Swish {
    ElementT operator()(ElementT val) const { return val * Sigmoid<ElementT>{}(val); }
};
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementGolden, class LayoutGolden>
void ComputeMatmulElemWiseRelu(const GemmCoord &problemShape,
                               const std::vector<ElementA> &dataA,
                               const LayoutA &layoutA,
                               const std::vector<ElementB> &dataB,
                               const LayoutB &layoutB,
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementGolden, class LayoutGolden>
void ComputeMatmulElemWiseGelu(const GemmCoord &problemShape,
                               const std::vector<ElementA> &dataA,
                               const LayoutA &layoutA,
                               const std::vector<ElementB> &dataB,
                               const LayoutB &layoutB,
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementGolden, class LayoutGolden>
void ComputeMatmulElemWiseSwish(const GemmCoord &problemShape,
                                const std::vector<ElementA> &dataA,
                                const LayoutA &layoutA,
                                const std::vector<ElementB> &dataB,
                                const LayoutB &layoutB,
