...
#ifndef CATLASS_EXAMPLES_X_ATTENTION_TILING_HPP
#define CATLASS_EXAMPLES_X_ATTENTION_TILING_HPP
...
inline uint64_t GetTiling(const Context& context, XAttentionTilingData& tilingData, uint64_t& workspaceSize)
{
    Validate(context);
    ...
    uint32_t groupCountPerLoop =
        std::min(UNSHARED_Q_TILE / tilingData.groupSize, UNSHARED_KV_TILE / context.maxDecodeStep);
    ...
    tilingData.mm1OutSize = (static_cast<uint64_t>(tilingData.sharedCoreNum) * WORKSPACE_BLOCK_SIZE_DB +
    ...
    workspaceSize = tilingData.mm1OutSize + tilingData.smOnlineOutSize + tilingData.mm2OutSize + tilingData.updateSize +
                    tilingData.sharedWorkspaceSize + unsharedWorkspaceSize;
...
#endif // CATLASS_EXAMPLES_X_ATTENTION_TILING_HPP
