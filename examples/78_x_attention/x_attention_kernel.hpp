...
#ifndef CATLASS_EXAMPLES_X_ATTENTION_KERNEL_HPP
#define CATLASS_EXAMPLES_X_ATTENTION_KERNEL_HPP
...
template <class BlockMmadQK, class BlockMmadPV, class EpilogueFAUnsharedSoftmax, bool PAGED_CACHE_FLAG>
...
public:
    ...
    CATLASS_DEVICE
    UnsharedFAInferKernel(__gm__ XAttentionTilingData* tilingDataPtr) : faTilingData(tilingDataPtr)
    {}
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(XAttnKernelParams const& params)
    {
        ...
        uint32_t taskStartIdx = relativeCoreIdx < unsharedFullCoreNum ?
                                    relativeCoreIdx * unsharedTaskNumHead :
                                    (unsharedFullCoreNum * unsharedTaskNumHead +
                                     (relativeCoreIdx - unsharedFullCoreNum) * unsharedTaskNumTail);
        uint32_t taskEndIdx =
            taskStartIdx + (relativeCoreIdx >= unsharedFullCoreNum ? unsharedTaskNumTail : unsharedTaskNumHead);
        ...
        uint64_t gmSOffset =
            (coreNumShared * WORKSPACE_BLOCK_SIZE_DB + relativeCoreIdx * UNSHARED_WORKSPACE_BLOCK_SIZE_DB) * NUM3;
        ...
        for (uint32_t taskIdx = taskStartIdx, nextTaskIdx; taskIdx < taskEndIdx + PRE_LAUNCH; ++taskIdx) {
            if (taskIdx < taskEndIdx) {
                ...
                blockMmadQK(
                    gQ[gmQOffset], gUnsharedK[gmKOffset], gS[actualSOffset], layoutQTemp, layoutKTemp, layoutSTemp,
                    actualBlockShapeQK);
                ...
                if (nextTaskIdx % unsharedLoopCountPerBatch == 0) {
                    GetKVOffset(
                        nextTaskIdx, unsharedLoopCountPerBatch, kvBaseBlockSize, gUnsharedBlockTable, gmKOffset);
                } else {
            ...
            if (taskIdx >= taskStartIdx + PRE_LAUNCH) {
                ...
                blockMmadPV(
                    gP[actualPOffset], gUnsharedV[gmVOffset], gUnsharedO[gmOOffset], layoutPTemp, layoutVTemp,
                    layoutOTemp, actualBlockShapePV, softmaxReady);
                gmOOffset += qOBaseBlockSize;
                nextTaskIdx = taskIdx + 1 - PRE_LAUNCH;
                if (nextTaskIdx % unsharedLoopCountPerBatch == 0) {
                    GetKVOffset(
                        nextTaskIdx, unsharedLoopCountPerBatch, kvBaseBlockSize, gUnsharedBlockTable, gmVOffset);
                } else {
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(XAttnKernelParams const& params)
    {
        ...
        EpilogueFAUnsharedSoftmax epilogueFAUnsharedSoftmax(
            resource, scaleValue, unsharedKvSeqLen, maxDecodeStep, unshareGroupCountPerLoop, groupSize);
        ...
        LayoutS layoutSTemp(qLen, kvLen, kvLenAlign);
        LayoutP layoutPTemp(qLen, kvLen, kvLenAlign);
        GemmCoord actualBlockShapeQK{
            unshareGroupCountPerLoop * groupSize, unshareGroupCountPerLoop * maxDecodeStep, embeddingSize};
        for (uint32_t taskIdx = taskStartIdx; taskIdx < taskEndIdx; ++taskIdx) {
            ...
            epilogueFAUnsharedSoftmax(
                ...
                gUnsharedGl[gmUnsharedGmGlOffset], layoutPTemp, layoutSTemp, actualBlockShapeQK,
                unshareGroupCountPerLoop);
...
private:
    CATLASS_DEVICE void GetKVOffset(
        uint32_t taskIdx, uint32_t unsharedLoopCountPerBatch, uint64_t kvBaseBlockSize,
...
private:
    ...
    __gm__ XAttentionTilingData* faTilingData;
};
...
*/
template <
    class BlockMmadQK, class BlockMmadPV, class EpilogueOnlineSoftmax, class EpilogueRescaleO,
...
public:
    ...
    // Methods
    CATLASS_DEVICE
    SharedFAInferKernelShort(__gm__ XAttentionTilingData* tilingDataPtr) : faTilingData(tilingDataPtr)
    {}
    ...
    struct TaskQue {
        ...
        CATLASS_DEVICE
        void SetValue(
            uint32_t taskIdx_, uint64_t gmOffsetV_, uint64_t gmOffsetO_, uint32_t rowNum_, uint32_t stackSeqTile_,
            uint32_t kvSIdx_, uint32_t blockBOffset_, uint32_t qSeqlen_, uint32_t qSBlockSize_, uint32_t qNBlockSize_,
            uint32_t kvSLoopNumTotal_)
        {
            taskIdx = taskIdx_;
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum + PRE_LAUNCH * coreNum; taskIdx += uint32_t(coreNum)) {
            ...
            for (uint32_t kvSIdx = 0; kvSIdx < kvSLoopNumTotal; kvSIdx += blockStackNum) {
                if (taskIdx < totalTaskNum) {
                    ...
                    blockMmadQK(
                        gQ[gmOffsetQ], gK[gmOffsetK], gS[gmOffsetS], gBlockTable[blockBOffset], layoutQTemp,
                        layoutKTemp, layOutS, actualBlockShapeQK, kvSIdx, pagedBlockSize, strideKV, l1KPPingPongFlag,
                        l0ABPingPongFlag, l0CPingPongFlag);
                    Arch::CrossCoreSetFlag<0x2, PIPE_FIX>(qkReady);

                    taskQue[workspaceStagesFlag].SetValue(
                        taskIdx, gmOffsetV, gmOffsetO, rowNum, stackSeqTile, kvSIdx, blockBOffset, qSeqlen, qSBlockSize,
                        qNBlockSize, kvSLoopNumTotal);
                }
                if (loopCnt >= PRE_LAUNCH) {
                    ...
                    uint64_t gmOffsetP = coreIdx * WORKSPACE_BLOCK_SIZE_DB * (PRE_LAUNCH + 1) +
                                         nowWorkspaceStagesFlag * WORKSPACE_BLOCK_SIZE_DB;
                    blockMmadPV(
                        gP[gmOffsetP], gV[taskQue[nowWorkspaceStagesFlag].gmOffsetV], gOTmp[gmOffsetOTmp],
                        gBlockTable[taskQue[nowWorkspaceStagesFlag].blockBOffset], layoutPTemp, layoutVTemp, layoutOTmp,
                        actualBlockShapePV, nowkvSIdx, pagedBlockSize, strideKV, softmaxReady, l1KPPingPongFlag,
                        l0ABPingPongFlag, l0CPingPongFlag);
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(XAttnKernelParams const& params)
    {
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum + PRE_LAUNCH * coreNum; taskIdx += uint32_t(coreNum)) {
            ...
            uint32_t gSharedOffset = curBatch * qSeqlen * qHeads * SOFTMAX_BROAD_SIZE +
                                     qSBlockIdx * curQSBlockTile * qHeads * SOFTMAX_BROAD_SIZE +
                                     qStartNIdx * SOFTMAX_BROAD_SIZE;
            ...
            for (uint32_t kvSIdx = 0; kvSIdx < kvSLoopNumTotal; kvSIdx += blockStackNum) {
                if (taskIdx < totalTaskNum) {
                    ...
                    epilogueOnlineSoftmax(
                        gP[gmOffsetP], gS[gmOffsetS], gSharedMax[gSharedOffset], gSharedSum[gSharedOffset], layOutP,
                        layOutS, actualBlockShapeQK, (kvSIdx == 0), isLastStackTile, qSBlockSize, qNBlockSize,
                        workspaceStagesFlag, qHeads);
                    Arch::CrossCoreSetFlag<0x2, PIPE_MTE3>(softmaxReady);
                    taskQue[workspaceStagesFlag].SetValue(
                        taskIdx, 0, gmOffsetO, rowNum, stackSeqTile, kvSIdx, 0, qSeqlen, qSBlockSize, qNBlockSize,
                        kvSLoopNumTotal);
                }
                if (loopCnt >= PRE_LAUNCH) {
                    uint32_t nowWorkspaceStagesFlag =
                    ...
                    // rescale O
                    epilogueRescaleO(
                        gO[taskQue[nowWorkspaceStagesFlag].gmOffsetO], gOTmp[gmOffsetOTmp],
                        gSharedO[taskQue[nowWorkspaceStagesFlag].gmOffsetO], layoutO, layoutOTmp, actualBlockShapePV,
                        taskQue[nowWorkspaceStagesFlag].qSBlockSize, taskQue[nowWorkspaceStagesFlag].qNBlockSize,
                        (nowkvSIdx == 0), nowkvSIdx + blockStackNum >= taskQue[nowWorkspaceStagesFlag].kvSLoopNumTotal,
...
public:
    ...
    CATLASS_DEVICE
    CombineScaleKernel(__gm__ XAttentionTilingData* tilingDataPtr) : faTilingData(tilingDataPtr)
    {}
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(XAttnKernelParams const& params)
    {
        ...
        if (coreIdx < combineFormerCoreNum) {
            ...
            gmOffsetUnSharedGl =
                combineFormerCoreNum * combineFormerRowNum + (coreIdx - combineFormerCoreNum) * combineTailRowNum;
            gmOffsetUnSharedGm = gmOffsetUnSharedGl;
        ...
        gSharedGl.SetGlobalBuffer(
            (__gm__ ElementInput*)params.shared_workspace + attnOutputOffset + sharedSumMaxGmOffset);
        ...
        gUnsharedGl.SetGlobalBuffer(
            (__gm__ ElementInput*)params.unshared_workspace + attnOutputOffset + unsharedSumMaxGmOffset);
        ...
        epilogueCombineScale(
            gSharedGm[gmOffsetSharedGm], gUnsharedGm[gmOffsetUnSharedGm], gSharedGl[gmOffsetSharedGl],
            gUnsharedGl[gmOffsetUnSharedGl], gSharedOut[gmOffsetO], gUnsharedOut[gmOffsetO], gFinalOut[gmOffsetO],
            actualBlockShape);
...
#endif // CATLASS_EXAMPLES_X_ATTENTION_KERNEL_HPP
