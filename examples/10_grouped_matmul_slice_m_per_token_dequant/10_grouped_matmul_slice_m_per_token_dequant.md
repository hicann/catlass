...
- 设置[L1TileShape和L0TileShape](../../docs/zh/2_Design/01_kernel_design/03_dispatch_policies.md)，用于基本块分核和L1/L0切块。需要注意TileShape和DispatchPolicy的设置有约束关系，例如当前L1::M=128、L1::K=128时，l0AStages不能超过4，以免超出L0A大小、在样例编译时被BlockMmad侧静态校验拦截。TileShape计算可参考[TileShape约束](../../docs/zh/1_Practice/10_matmul_optimization.md)
...
- [适配器初始化算子](../10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L208)，特别注意，对于涉及核间同步的算子，需要[初始化fftsAddr](../10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L138)并在MatmulAdapter执行时调用传入
