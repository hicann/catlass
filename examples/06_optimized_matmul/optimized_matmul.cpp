...
using LayoutPaddingB = std::conditional_t<std::is_same_v<LayoutB, layout::RowMajor>,
            layout::PaddingRowMajor, layout::PaddingColumnMajor>;
using AType = Act::Gemm::GemmType<ElementA, LayoutA>;
using BType = Act::Gemm::GemmType<ElementB, LayoutB>;
using CType = Act::Gemm::GemmType<ElementC, LayoutC>;
using ATypePadding = Act::Gemm::GemmType<ElementA, LayoutPaddingA>;
using BTypePadding = Act::Gemm::GemmType<ElementB, LayoutPaddingB>;
using DispatchPolicy = Act::Gemm::MmadAtlasA2Preload<ENABLE_UNIT_FLAG, ENABLE_SHUFFLE_K>;
...
using BlockScheduler30 = typename Act::Gemm::Block::GemmIdentityBlockSwizzle<3, 0>;
using BlockScheduler31 = typename Act::Gemm::Block::GemmIdentityBlockSwizzle<3, 1>;
...
void Run(Options const &options)
{
    ...
    if (m > n) {
        if (isNeedPaddingA && isNeedPaddingB) {
            using BlockMmadOpt = Act::Gemm::Block::BlockMmad<
                DispatchPolicy, L1TileShape, L0TileShape, ATypePadding, BTypePadding, CType>;
            using MatmulKernel = Act::Gemm::Kernel::OptimizedMatmul<BlockMmadOpt, BlockEpilogue, BlockScheduler30>;
        ...
        if (isNeedPaddingA && isNeedPaddingB) {
            using BlockMmadOpt = Act::Gemm::Block::BlockMmad<
                DispatchPolicy, L1TileShape, L0TileShape, ATypePadding, BTypePadding, CType>;
            using MatmulKernel = Act::Gemm::Kernel::OptimizedMatmul<BlockMmadOpt, BlockEpilogue, BlockScheduler31>;
            LayoutPaddingA layoutWA = LayoutPaddingA(
                layoutA.shape(0), layoutA.shape(1), L1TileShape::M, L1TileShape::K);
            LayoutPaddingB layoutWB = LayoutPaddingB(
                layoutB.shape(0), layoutB.shape(1), L1TileShape::K, L1TileShape::N);
    ...
    std::vector<ElementWorkspace> hostGolden(lenC);
