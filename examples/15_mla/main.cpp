...
void GetNdMLATiling(const MLAInfo &mlaInfo, uint32_t &blockDim, uint32_t *tilingHost)
{
    int32_t maxKVSeqlen = 0;
...
int32_t GetQNBlockTile(const MLAInfo &mlaInfo, int32_t qSeqLen)
{
...
void GetTilingHead(const MLAInfo &mlaInfo, uint32_t *tilingHost, const uint32_t *torPtr, int32_t maxQseqlen)
{
    ...
    int32_t curQNBlockTile = GetQNBlockTile(mlaInfo, maxQseqlen);
...
int32_t GetMLATilingParam(const MLAInfo &mlaInfo, uint32_t &blockDim, uint32_t *tilingHost)
{
    ...
    GetNdMLATiling(mlaInfo, blockDim, tilingHost);
    GetTilingHead(mlaInfo, tilingHost, torPtr, maxQseqlen);
...
struct Options {
    static constexpr auto HELPER =
        "Usage: mla batch qSeqlen kvSeqlen numHeads numBlocks blockSize [--datapath DATA_PATH --device DEVICE_ID]\n";
...
void Run(const Options &options)
{
    ...
    uint64_t qoSize = (uint64_t)numTokens * (uint64_t)numHeads *
                      (uint64_t)embeddingSize * sizeof(fp16_t);
    uint64_t qRopeSize = (uint64_t)numTokens * (uint64_t)numHeads *
                         (uint64_t)embeddingSizeRope * sizeof(fp16_t);
    uint64_t kvSize = (uint64_t)numBlocks * (uint64_t)blockSize * (uint64_t)kvHeads *
                      (uint64_t)embeddingSize * sizeof(fp16_t);
    uint64_t kRopeSize = (uint64_t)numBlocks * (uint64_t)blockSize * (uint64_t)kvHeads *
                         (uint64_t)embeddingSizeRope * sizeof(fp16_t);
    uint64_t maskSize = (uint64_t)numTokens * (uint64_t)maxKvSeqlen * sizeof(fp16_t);
    uint64_t blockTableSize = static_cast<uint64_t>(batch * ((maxKvSeqlen + blockSize - 1) / blockSize) * sizeof(int32_t));
    ...
    uint8_t *sDevice;
    ACL_CHECK(
        aclrtMalloc((void **)(&sDevice), aicCoreNum * MLATiling::WORKSPACE_BLOCK_SIZE_DB * sizeof(float),
                    ACL_MEM_MALLOC_HUGE_FIRST));

    uint8_t *pDevice;
    ACL_CHECK(
        aclrtMalloc((void **)(&pDevice), aicCoreNum * MLATiling::WORKSPACE_BLOCK_SIZE_DB * sizeof(fp16_t),
                    ACL_MEM_MALLOC_HUGE_FIRST));

    uint8_t *oTmpDevice;
    ACL_CHECK(
        aclrtMalloc((void **)(&oTmpDevice), aicCoreNum * MLATiling::WORKSPACE_BLOCK_SIZE_DB * sizeof(float) * 2,
                    ACL_MEM_MALLOC_HUGE_FIRST));

    uint8_t *globaloDevice;
    ACL_CHECK(
        aclrtMalloc((void **)(&globaloDevice), aicCoreNum * MLATiling::WORKSPACE_BLOCK_SIZE_DB * sizeof(float),
                    ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    ACL_CHECK(
        aclrtMalloc((void **)(&oCoreTmpDevice), oFdSize, ACL_MEM_MALLOC_HUGE_FIRST));

    uint8_t *lDevice;
    ACL_CHECK(
        aclrtMalloc((void **)(&lDevice), lSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    // Compare the result
    vector<uint64_t> errorIndices = golden::CompareData(oHost, goldenHost, kvSeqlen);
