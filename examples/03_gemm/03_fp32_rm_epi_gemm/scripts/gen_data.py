...
def calc_expect_func(M, N, K):
    ...
    alpha = np.random.uniform(-2,2, 1).astype(NP_DATA_TYPE)
    beta  = np.random.uniform(-2,2, 1).astype(NP_DATA_TYPE)
    A = np.random.uniform(-2,2,size=matSizeA).astype(NP_DATA_TYPE)
    B = np.random.uniform(-2,2,size=matSizeB).astype(NP_DATA_TYPE)
    C = np.random.uniform(-2,2,size=matSizeC).astype(NP_DATA_TYPE)
