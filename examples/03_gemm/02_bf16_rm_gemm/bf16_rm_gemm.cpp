...
void BF16RMGemm(
    ...
    using AType = matmul::MatmulType<bfloat16_t, LayoutA>;
    using BType = matmul::MatmulType<bfloat16_t, LayoutB>;
    using CType = matmul::MatmulType<bfloat16_t, LayoutC>;
    ...
    constexpr uint32_t computeLength = 12288; // 128 * 128 / 2 开启双缓冲机制
    // 后处理部分
...
typedef struct Options{
    const std::string HELPER = "03_gemm/02_bf16_rm_gemm m n k [device_id]";
...
void Run(Options options){
    ...
    size_t sizeWA = GetWorkspaceLen(layoutWA) * sizeof(bfloat16_t);
    size_t sizeWB = GetWorkspaceLen(layoutWB) * sizeof(bfloat16_t);
    ...
    BF16RMGemm<<<aicCoreNum, nullptr, stream>>>(
        fftsAddr,
        alpha[0], beta[0],
