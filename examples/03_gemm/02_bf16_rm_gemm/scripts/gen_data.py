...
def calc_expect_func(M, N, K):
    ...
    A = np.random.uniform(-0.25,0.25,size=matSizeA).astype(NP_DATA_TYPE)
    B = np.random.uniform(-0.25,0.25,size=matSizeB).astype(NP_DATA_TYPE)
    C = np.random.uniform(-0.25,0.25,size=matSizeC).astype(NP_DATA_TYPE)
    ...
    # print(B)
    # print(C)
    tmp = np.matmul(A.astype(DATA_TYPE),B.astype(DATA_TYPE)).astype(DATA_TYPE) # 中间过程结果
    # D = alpha * B * A + beta * C
