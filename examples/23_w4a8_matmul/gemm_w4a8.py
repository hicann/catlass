...
def accuracy_check(deviceId: int = 0) -> None:
    ...
    for csv in csv_list:
        ...
        for _, testcase in data.iterrows():
            ...
            cmd = f"cd {WORKSPACE}/build/examples/23_w4a8_matmul && ./23_w4a8_matmul {deviceId} {param.b} {param.m} {param.k} {param.n} {int(param.transA)} {int(param.transB)} | tee -a {WORKSPACE}/log/accur.log"
            print(cmd)
            os.system(cmd)
            compare(param)
