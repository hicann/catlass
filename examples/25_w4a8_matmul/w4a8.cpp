...
void Run(Options const &options)
{
    ...
    for (uint32_t i = 0; i < (verifyLevel == 0 ? loopTimes : 1); ++i) {
        ...
        using SrcType = Gemm::GemmType<int8_t, layout::RowMajorInt4>;
        using DstType = Gemm::GemmType<int8_t, layout::RowMajor>;

        using PrologueA = void;
        using PrologueB = Gemm::Tile::TileCastInt4ToInt8<ArchTag, SrcType, DstType>;
