...
class MXFP4MatrixQuantizer:
    ...
    def matrix_fp32_to_fp4(self, matrix: torch.Tensor) -> torch.Tensor:
        # 原始行列
        rows, cols = matrix.shape
        total_elems = rows * cols

        total_uint8 = (total_elems + 1) // 2

        packed_flat = torch.zeros(total_uint8, dtype=torch.uint8, device=matrix.device)

        global_idx = 0
        for row in range(rows):
            for col in range(cols):
                uint8_pos = global_idx // 2
                bit_pos = global_idx % 2
                ...
                if bit_pos == 0:
                    packed_flat[uint8_pos] = (packed_flat[uint8_pos] & 0xF0) | fp4_val
                else:
                    packed_flat[uint8_pos] = (packed_flat[uint8_pos] & 0x0F) | (fp4_val << 4)

                global_idx += 1

        out_cols = (cols + 1) // 2
        out_rows = (total_uint8 + out_cols - 1) // out_cols

        final_matrix = torch.zeros(out_rows, out_cols, dtype=torch.uint8, device=matrix.device)
        final_matrix.view(-1)[:total_uint8] = packed_flat
