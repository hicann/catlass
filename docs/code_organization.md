...
- scripts包含模板库样例的构建脚本
## include
CATLASS是一套基于AscendC开发的算子模板库，提供昇腾硬件Gemm类算子定制化开发的极致性能。模板库的分层对应硬件的不同层级展开。block层对应于NPU的单核单基块的层级，tile层对应于分片粒度的数据搬运和计算的层级，basic对应于基础API的层级。这些组件可以在相应的算子内的不同运算层级被使用。
...
|                |── block_epilogue_fa_rescale_o.hpp        // fa的rescale_o后融合操作
...
|                |── block_epilogue_mla_fd_rescale_o.hpp    // mla_fd的rescale_o后融合操作
|                |── block_epilogue_mla_rescale_o.hpp       // mla的rescale后融合操作
|                |── block_epilogue_mla_softmax.hpp         // mla的softmax后融合操作
|                |── block_epilogue_mla_tp1_rescale_o.hpp   // mla_tp1的rescale_o后融合操作
|                |── block_epilogue_mla_tp1_softmax.hpp     // mla_tp1的softmax后融合操作
...
|            |── dispatch_policy.hpp        // EpilogueDispatchPolicy定义
|        |── gemm
|            |── block
...
|                |── block_mmad_mla_pv_tp1_spec.hpp  // block层mla pv实现(针对headNum128特殊优化)
...
|                |── batched_matmul.hpp           // kernel层batched_matmul实现
|                |── gemm.hpp                     // kernel层gemm实现
|                |── group_gemm.hpp               // kernel层group_gemm实现
|                |── grouped_matmul.hpp           // kernel层grouped_matmul实现
|                |── grouped_matmul_slice_k.hpp   // kernel层k轴切分groupedMatmul实现
...
|            |── device
|                |── device_gemv.hpp          // gemv的device层实现
|                |── kernel_adapter.hpp       // gemv的通用内核模板定义
|            |── kernel
