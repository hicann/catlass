...
def _npdtype_to_torch(np_dtype):
    m = {
    ...
    return m.get(np_dtype, torch.float32)
...
class TestPagedMLAttention():
    ...
    # NOTE: same as https://github.com/vllm-project/vllm/blob/v0.10.0/tests/kernels/attention/test_attention.py
    def ref_single_query_cached_kv_attention(self, attention_inputs: AttentionInputs, output, true_out) -> None:
        ...
        for i in range(batch):
            ...
            # print("i, block_table: ", i, block_table)
            keys = []
            values = []
    ...
    def calc_data(self, gen_data_params, device, save_file=True, calc_ref=False):
        torch_dtype = _npdtype_to_torch(gen_data_params.dtype)
        ...
        num_tokens = int(sum(gen_data_params.q_seqlen_list))
        batch_size = int(len(gen_data_params.q_seqlen_list))

        query = torch.empty(
            (num_tokens, gen_data_params.num_heads, head_size_qk),
        ...
        max_k_seqlen = int(max(gen_data_params.k_seqlen_list))
        max_num_blocks_per_seq = (max_k_seqlen + gen_data_params.block_size - 1) // gen_data_params.block_size

        block_tables_t = torch.arange(
            batch_size * max_num_blocks_per_seq, device=device, dtype=torch.int32
        ).view(batch_size, max_num_blocks_per_seq)

        mask = None
        if gen_data_params.mask_type == 1:
            pre_mask_factor = -10000.0
            # mask shape: (num_tokens, max_k_seqlen)
            mask = torch.zeros((num_tokens, max_k_seqlen), dtype=torch_dtype, device=device)
            pre_qseqlen = 0
            for i in range(batch_size):
                qseqlen = int(gen_data_params.q_seqlen_list[i])
                kseqlen = int(gen_data_params.k_seqlen_list[i])
                tri = torch.triu(torch.ones((qseqlen, qseqlen), device=device, dtype=torch_dtype), diagonal=1)
                tri = tri * pre_mask_factor
        ...
        ref_output_t = torch.zeros(shape_out, dtype=torch_dtype, device=device)
        true_out_t = torch.zeros(shape_out, dtype=torch.float32, device=device)
        ...
        if calc_ref:
            attention_inputs = self.AttentionInputs(
                ...
                block_tables=(block_tables_t.cpu().numpy().tolist()),
                q_seqlen_list=np.array(gen_data_params.q_seqlen_list, dtype=np.int32),
                k_seqlen_list=np.array(gen_data_params.k_seqlen_list, dtype=np.int32),
                mask=(None if mask is None else mask.cpu().numpy()),
                mask_type=gen_data_params.mask_type,
            )

            ref_output_np = ref_output_t.cpu().numpy()
            true_out_np = true_out_t.cpu().numpy()
            ...
            ref_output_t = torch.from_numpy(ref_output_np).to(device=device, dtype=torch.float32)
            true_out_t = torch.from_numpy(true_out_np).to(device=device, dtype=torch.float32)
        else:
            pass
            # print("WARNING: Reference is not being calculated during data generation for MLA! (To generate faster)"
            #     " If you are going to compare your output with golden.bin, you need to set calc_ref == True")

        if save_file:
            torch.tensor([num_tokens], dtype=torch.int32).cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "q_ntokens.bin"))

            query_nope.cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "q.bin"))
            query_rope.cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "q_rope.bin"))

            kv_nope_cache.cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "k.bin"))
            kv_rope_cache.cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "k_rope.bin"))

            bt_cpu = block_tables_t.cpu().numpy().astype(np.int32)
            token2batch = np.empty((num_tokens,), dtype=np.int32)
            idx = 0
            for b, qlen in enumerate(gen_data_params.q_seqlen_list):
                token2batch[idx:idx+qlen] = b
                idx += qlen
            block_tables_expanded = bt_cpu[token2batch] 
            block_tables_expanded.tofile(os.path.join(WORKSPACE, "data", "block_table.bin"))

            np.array(gen_data_params.q_seqlen_list, dtype=np.int32).tofile(os.path.join(WORKSPACE, "data", "q_seqlen.bin"))
            np.array(gen_data_params.k_seqlen_list, dtype=np.int32).tofile(os.path.join(WORKSPACE, "data", "kv_seqlen.bin"))

            if mask is not None:
                mask.cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "mask.bin"))

            ref_output_t.to(torch.float32).cpu().numpy().tofile(os.path.join(WORKSPACE, "data", "golden.bin"))
        else:
            bt_cpu = block_tables_t.cpu().numpy().astype(np.int32)
            token2batch = np.empty((num_tokens,), dtype=np.int32)
            idx = 0
            ...
            return (
                query_nope.cpu().numpy(),             # (num_tokens, num_heads, head_size_vo)  [device]
                query_rope.cpu().numpy(),             # (num_tokens, num_heads, head_size_rope)[device]
                kv_nope_cache.cpu().numpy(),          # (num_blocks, block_size, kv_heads, head_size_vo)[device]
                kv_rope_cache.cpu().numpy(),          # (num_blocks, block_size, kv_heads, head_size_rope)[device]
                block_tables_expanded,         # (batch_size, max_num_blocks_per_seq)   [device]
                ref_output_t.to(torch.float32).cpu().numpy()  # (num_tokens, num_heads, head_size_vo) [device]
            )


if __name__ == "__main__":
    if len(sys.argv) != 8:
        logging("[ERROR] args: batchSize, qSeqlen, kvSeqlen, qheadNum, numBlock, blockSize, dtype")
        logging("[ERROR] Example: 1024 1 2048 128 20000 128 half")
        sys.exit()
    ...
    testObj.calc_data(gen_data_params, device)

    meta_data = dict(
        num_heads=gen_data_params.num_heads,
        kv_heads=gen_data_params.kv_heads,
        head_size=gen_data_params.head_size,
        head_size_rope=gen_data_params.head_size_rope,
        num_blocks=gen_data_params.num_blocks,
        block_size=gen_data_params.block_size,
        # dtype=gen_data_params.dtype
    )
    print(meta_data)

    with open(os.path.join(WORKSPACE, "data", "meta_data.json"), 'w') as f:
