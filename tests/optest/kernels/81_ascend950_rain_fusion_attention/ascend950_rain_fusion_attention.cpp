...
template <typename T>
struct RfaTypeTag { using type = T; };
...
public:
    ...
    __aicore__ inline
    void operator()(RfaKernelParamsArch35 const &params)
    {
        __gm__ RfaTilingData *tilingData =
            reinterpret_cast<__gm__ RfaTilingData *>(params.tiling);
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum_; taskIdx += coreNum) {
            ...
            uint32_t lastSelectIdx = static_cast<int32_t>(
                gSelectIdx.GetValue(curSelectNumIdx * maxKvBlockNum_ + curSelectNum - 1));
            ...
            uint32_t selectKvSeqLen = (lastSelectIdx == curKvYBlockNum - 1 && kvSeqlen % blockShapeY_ != 0) ?
                blockShapeY_ * (curSelectNum - 1) + kvSeqlen % blockShapeY_ : blockShapeY_ * curSelectNum;
            ...
            uint32_t qSTileSizeAct = (qXBlockIdx < curQXBlockNum - 1) ?
                ((qXBlockInnerIdx == qSTileNumInXBlock - 1) ? blockShapeX_ - qXBlockInnerIdx * qBaseTile_ : qBaseTile_) :
                ((qXBlockInnerIdx == CeilDiv(xTailBlockLen, qBaseTile_) - 1) ? xTailBlockLen - qXBlockInnerIdx * qBaseTile_ : qBaseTile_);
            ...
            auto gmKLayoutTla = tla::MakeLayout(
                tla::MakeShape(kvShapeCol, kvBaseTile_),
                tla::MakeStride(tla::Int<1>{}, (int64_t)kvShapeCol),
            ...
            auto gmVLayoutTla = tla::MakeLayout(
                tla::MakeShape(kvBaseTile_, kvShapeCol),
                tla::MakeStride((int64_t)kvShapeCol, tla::Int<1>{}),
            ...
            for (uint32_t kvSTileIdx = 0; kvSTileIdx < kvSLoopNumTotal + PRE_LAUNCH; kvSTileIdx++) {
                if (kvSTileIdx < kvSLoopNumTotal) {
                    ...
                    blockMmadQK(
                        gmKTensorTla, ubSTensorTla, gSelectIdx[curSelectNumIdx * maxKvBlockNum_],
                        actualBlockShapeQK, kvSTileIdx, kvSeqlen, kvBaseTile_, blockShapeY_, curSelectNum,
                        curKvYBlockNum, prefixSumL0AStages, prefixSumL0BStages, mm1ToSmFlag);
                    if (kvSTileIdx == kvSLoopNumTotal - 1) {
                ...
                if (kvSTileIdx >= PRE_LAUNCH) {
                    ...
                    auto ubOTmpTensorTla = tla::MakeTensor(ubOTmpTensor[ubOTmpBufId], ubOTmpLayoutTla, Arch::PositionUB{});
                    ...
                    blockMmadPV(
                        gmVTensorTla, ubOTmpTensorTla, gSelectIdx[curSelectNumIdx * maxKvBlockNum_],
                        actualBlockShapePV, kvSIdxActual, kvSeqlen, kvBaseTile_, blockShapeY_, curSelectNum,
                        curKvYBlockNum, prefixSumL0AStages, prefixSumL0BStages, smToMm2Flag, mm2ToReFlag);
#endif
#ifdef __DAV_VEC__
                    // Stage 4: rescale O
                    ...
                    epilogueRescaleO(
                        ...
                        curTileMod, kvSIdxActual,
                        (kvSIdxActual == 0),
                        (kvSIdxActual == kvSLoopNumTotal - 1),
                        mm2ToReFlag);
#endif
                }
    ...
    __aicore__ inline
    void GetTilingData(__gm__ RfaTilingData *tilingData)
    {
    ...
    __aicore__ inline
    uint64_t CalcMm1Mm2PrefixSumL0ABStages(uint32_t kvSTileIdx, uint32_t mm1L0Stages,
        uint32_t mm2L0Stages, uint32_t kvSLoopNum, bool isMm1)
    {
        ...
        AscendC::LocalTensor<ElementOTmp> (&ubOTmpTensor)[UB_S_OTMP_BUF_STAGES])
    {
        ...
        for (uint32_t i = 0; i < UB_S_OTMP_BUF_STAGES; i++) {
            ...
            ubOTmpTensor[i] = resource.ubBuf.template GetBufferByByte<ElementOTmp>(
                ...
                spElemNum * sizeof(ElementP) * UB_S_OTMP_BUF_STAGES +
                rescaleElemNum * sizeof(ElementOTmp) * i);
        }
    }
    ...
    GM_ADDR tiling)
{
    ...
    using EpilogueRescaleO = Epilogue::Block::BlockEpilogue<
        DispatchPolicyRescaleO, ElementO, ElementOTmp, ElementK, TileCopyRescaleO>;
    ...
    RfaKernelParamsArch35 params{q, k, v, mask, blockTable, actualQseqlen, actualKvseqlen,
...
void RainFusionAttention(uint32_t blockNum, aclrtStream stream, const RainFusionAttentionParams& params)
{
    auto aicCoreNum = blockNum;

    uint32_t batch = params.batch;
    uint32_t numHeads = params.numHeads;
    uint32_t kvHeads = params.kvHeads;
    uint32_t embeddingSize = params.embeddingSize;
    uint32_t blockShapeX = params.blockShapeX;
    uint32_t blockShapeY = params.blockShapeY;
    uint32_t maxKvBlockNum = params.maxKvBlockNum;
    uint32_t isVariedLen = params.isVariedLen;
    uint32_t qInputLayout = params.qInputLayout;
    uint32_t kvInputLayout = params.kvInputLayout;
    ...
    uint8_t* selectIdxDevice = params.inputAddr.at(5);
    uint8_t* selectNumIdxDevice = params.inputAddr.at(6);
    uint8_t* oDevice = params.outputAddr.at(0);

    uint8_t* maskDevice{nullptr};
    uint8_t* blockTableDevice{nullptr};
    uint8_t* lseDevice{nullptr};
    uint8_t* blockShapeDevice{nullptr};

    // ---- Tiling (host side) ----
    RfaContext rfaContext;
    rfaContext.batch = static_cast<int32_t>(batch);
    rfaContext.numHeads = static_cast<int32_t>(numHeads);
    rfaContext.kvHeads = static_cast<int32_t>(kvHeads);
    rfaContext.maxQSeqlen = static_cast<int64_t>(params.maxQSeqlen);
    rfaContext.maxKvSeqlen = static_cast<int64_t>(params.maxKvSeqlen);
    rfaContext.embeddingSize = static_cast<int32_t>(embeddingSize);
    rfaContext.blockShapeX = static_cast<int64_t>(blockShapeX);
    rfaContext.blockShapeY = static_cast<int64_t>(blockShapeY);
    rfaContext.maxKvBlockNum = maxKvBlockNum;
    rfaContext.maxNumBlocksPerBatch = maxKvBlockNum * numHeads;
    rfaContext.dataType = (dataType == ACL_BF16) ? DataType::BF16 : DataType::FP16;
    rfaContext.qInputLayout = (qInputLayout == 1) ? "BNSD" : "TND";
    rfaContext.kvInputLayout = (kvInputLayout == 1) ? "BNSD" : "TND";
    rfaContext.isVarLen = (isVariedLen == 1);
    rfaContext.qSeqlenList = params.qSeqHost.data();
    rfaContext.kvSeqlenList = params.kvSeqHost.data();
    rfaContext.scaleValue = scaleValue;
    ...
    uint8_t* tilingDevice = g_catlassWorkspaceAllocFromHost(&rfaTilingData, tilingSize);

    // ---- Workspace ----
    uint32_t workSpaceSize = static_cast<uint32_t>(rfaTilingData.get_workSpaceSize());
    uint8_t* workspaceDevice = workSpaceSize > 0 ? g_catlassWorkspaceAlloc(workSpaceSize) : nullptr;

    uint32_t blockDim = aicCoreNum;

    auto callRfaKernel = [&](auto dtypeTag) {
        using T = typename decltype(dtypeTag)::type;
        if (qInputLayout == 0 && kvInputLayout == 0) {
            RainFusionAttention950<T, T, Epilogue::LseMode::NONE, 0, 0>
                <<<blockDim, nullptr, stream>>>(
                    qDevice, kDevice, vDevice,
                    selectIdxDevice, selectNumIdxDevice, blockShapeDevice,
                    qSeqDevice, kvSeqDevice,
                    maskDevice, blockTableDevice,
                    oDevice, lseDevice, workspaceDevice, tilingDevice);
        } else {
            RainFusionAttention950<T, T, Epilogue::LseMode::NONE, 1, 1>
                <<<blockDim, nullptr, stream>>>(
                    qDevice, kDevice, vDevice,
                    selectIdxDevice, selectNumIdxDevice, blockShapeDevice,
                    qSeqDevice, kvSeqDevice,
