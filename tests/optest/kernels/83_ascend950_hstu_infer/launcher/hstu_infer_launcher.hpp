 ...
 */
#ifndef OPTEST_HSTU_INFER_LAUNCHER_HPP
#define OPTEST_HSTU_INFER_LAUNCHER_HPP
...
void RunHSTUKernel(
    ...
    aclrtStream stream, const uint32_t aicCoreNum)
{
    ...
    if (sizeWorkspace > 0) {
        HSTU_ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceWorkspace), sizeWorkspace, ACL_MEM_MALLOC_HUGE_FIRST));
    }
    ...
    // 同步后释放 workspace，避免异步执行期间释放导致 UAF
    HSTU_ACL_CHECK(aclrtSynchronizeStream(stream));
    if (deviceWorkspace != nullptr) {
        HSTU_ACL_CHECK(aclrtFree(deviceWorkspace));
    }
}
