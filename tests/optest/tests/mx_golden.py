...
def prepare_fp4_mx_quant_matmul_inputs(
    m: int, n: int, k: int, device: str = "npu", seed: int = None, vary_mx_scales: bool = False
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Build inputs for example 85 FP4 MX matmul + per-token/per-channel scales."""
    if k % 2 != 0:
        raise ValueError(f"K must be even for FP4 packing: k={k}")
    if n % 2 != 0:
        raise ValueError(f"N must be even for FP4 packing: n={n}")

    torch.manual_seed(7900 + m + n + k if seed is None else seed)
    ...
    mx_scale_b = torch.full((k0 // 2, n, 2), _E8M0_ONE, dtype=torch.uint8).view(torch.float8_e8m0fnu)

    if vary_mx_scales:
        exponent_a = torch.randint(-2, 3, (m, k0))
        exponent_b = torch.randint(-2, 3, (n, k0))
        mx_scale_a = (exponent_a + _E8M0_ONE).to(torch.uint8).reshape(m, k0 // 2, 2).view(torch.float8_e8m0fnu)
        # RowMajor B stores each pair of K-group scales before advancing N.
        mx_scale_b = (
            (exponent_b + _E8M0_ONE).to(torch.uint8).reshape(n, k0 // 2, 2)
            .permute(1, 0, 2).contiguous().view(torch.float8_e8m0fnu)
        )
        scale_a = torch.exp2(exponent_a.float()).repeat_interleave(32, dim=1)[:, :k]
        scale_b = torch.exp2(exponent_b.float()).repeat_interleave(32, dim=1)[:, :k].t()
        expected = ((a_val * scale_a) @ (b_val * scale_b)) * a_scale_f32.view(-1, 1) * b_scale_f32.view(1, -1)
...
def prepare_fp8_e4m3_quant_matmul_inputs(
    m: int, n: int, k: int, device: str = "npu", seed: int = None
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Build inputs for example 86 FP8 matmul + per-token/per-channel scales."""
    torch.manual_seed(8100 + m + n + k if seed is None else seed)
    a_fp32 = torch.randn((m, k), dtype=torch.float32) * 2
    b_fp32 = torch.randn((k, n), dtype=torch.float32) * 2
