...
from catlass_test.golden import grouped_matmul_golden, grouped_matmul_gen_case
...
class CatlassTest(TestCase):
    ...
    def test_grouped_matmul_list_m(self):
        ...
        a = torch.randn((m_sum, k), device="npu").to(dtype)
        b = torch.randn((g, k, n), device="npu").to(dtype)
        ...
        # )
        golden = grouped_matmul_golden(a, b, group_list, dtype, False, True, "m")
        self.assertRtolEqual(result, golden)
    ...
    def test_grouped_matmul_list_k(self):
        ...
        # )
        golden = grouped_matmul_golden(a, b, group_list, dtype, True, False, "k")
