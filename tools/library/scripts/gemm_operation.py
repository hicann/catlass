...
class GemmOperation:
    def __init__(
        ...
        self.kernel_instance_generators = {
            ...
            'grouped_matmul_slice_m': GroupedMatmulSliceMKernelInstance,
            'optimized_matmul_without_padding': OptimizedMatmulWithoutPaddingKernelInstance,
            'optimized_matmul_padding_ab': OptimizedMatmulPaddingAPaddingBKernelInstance,
            'optimized_matmul_padding_a_only': OptimizedMatmulPaddingAOnlyKernelInstance,
            'optimized_matmul_padding_b_only': OptimizedMatmulPaddingBOnlyKernelInstance,
        }
...
class GroupedMatmulSliceMKernelInstance:
    ...
    def gen_src(self, gemm_operation):
        src = self.template.format(
            ...
            epilogue=gemm_operation.epilogue,
            scheduler=gemm_operation.block_swizzle,
            element_group_list=gemm_operation.custom_args['element_group_list']
        )
        return src
...
class OptimizedMatmulPaddingAPaddingBKernelInstance:
    def __init__(self):
        ...
        {device_api}<
            {kernel_api}<
                Gemm::Kernel::PaddingMatrixBlockND<{arch}, {element_a}, {layout_a}, {layout_padding_a}, {compute_length_a}>,
                Gemm::Kernel::PaddingMatrixBlockND<{arch}, {element_b}, {layout_b}, {layout_padding_b}, {compute_length_b}>,
                {block_api}<
                    {dispatch},
                    GemmShape<{l1_m}, {l1_n}, {l1_k}>,
                    GemmShape<{l0_m}, {l0_n}, {l0_k}>,
                    Gemm::GemmType<{element_a}, {layout_padding_a}>,
                    Gemm::GemmType<{element_b}, {layout_padding_b}>,
                    Gemm::GemmType<{element_c}, {layout_c}>,
                    void,
                    TileCopyOpt<
                        {arch},
    ...
    def gen_src(self, gemm_operation):
        src = self.template.format(
            ...
            layout_padding_a=gemm_operation.custom_args['layout_padding_a'],
            layout_padding_b=gemm_operation.custom_args['layout_padding_b'],
            compute_length_a=gemm_operation.custom_args['compute_length_a'],
            compute_length_b=gemm_operation.custom_args['compute_length_b']
        )
        return src
