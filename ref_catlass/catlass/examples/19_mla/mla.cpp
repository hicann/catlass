...
void pickValid(const std::vector<float>& lHost,
               std::vector<float>& validElements,
               std::vector<size_t>& validIndices, int32_t kv_seq_len) {
    validElements.clear();
    validIndices.clear();

    validElements.reserve(lHost.size());
    validIndices.reserve(lHost.size());

    int32_t n = kv_seq_len / 128;
    int32_t jump_n = (n - 1);

    for (size_t i = 0, cycle = 0; i < lHost.size(); /* i is advanced inside */) {
        const bool shortBlock = (cycle % 11 == 10); // every 11th block uses 8/24
        const size_t blockSize = shortBlock ? 8 : 12;
        const size_t jumpSize  = blockSize * jump_n;

        // Take phase
        const size_t end = std::min(i + blockSize, lHost.size());
        for (size_t k = i; k < end; ++k) {
            validElements.push_back(lHost[k]);
            validIndices.push_back(k);
        }
        if (end == lHost.size()) break; // reached the end of input
...
void Run(const Options &options)
{
    ...
    // Init lDevice memory with by copying it from hostLdevice
    vector<float> hostLdevice(numTokens * numHeads * kvSplitCoreNum, 0.0f);
    ACL_CHECK(aclrtMemcpy(lDevice, lSize, hostLdevice.data(), lSize, ACL_MEMCPY_HOST_TO_DEVICE));
