...
def _render_tensor(
    tlv: memoryview, shape_tlv: memoryview, logical_block: int
) -> None:
...
def _validate_tensor_tlv(
    tlv: memoryview, total: int, shape_tlv: memoryview
) -> None:
    """Mirror RuntimeWrapper.cpp ``validate_tensor_tlv`` checks."""
    if total < _PRINT_TENSOR_TLV_SIZE:
        raise AscDebugFifoError(
            "malformed tensor print FIFO: truncated tensor header"
        )
    ...
    if dtype is None:
        raise AscDebugFifoError(
            "malformed tensor print FIFO: unsupported tensor dtype"
        )
    _name, width = dtype
    ...
    if (
        ...
        raise AscDebugFifoError(
            "malformed tensor print FIFO: invalid tensor dump size"
        )
