...
module {
  func.func @vector_load_us_b8(
    ...
    %src = tla.tensor_desc %src_memref[%src_c0, %src_c0, %src_c1, %src_c1, %src_c1, %src_c1, %src_c1, %src_c1] : (memref<1xi8, #hivm.address_space<ub>>, index, index, index, index, index, index, index, index) -> !ivec
    %dst_c0 = arith.constant 0 : index
    %dst_c1 = arith.constant 1 : index
    %dst = tla.tensor_desc %dst_memref[%dst_c0, %dst_c0, %dst_c1, %dst_c1, %dst_c1, %dst_c1, %dst_c1, %dst_c1] : (memref<1xi8, #hivm.address_space<ub>>, index, index, index, index, index, index, index, index) -> !ivec
    "tla.vec.func"() ({
