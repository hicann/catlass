...
DEFAULT_CACHE_DIR = DEMO_DIR / "artifacts" / "runtime-cache" / "basic_mixed_store_zN"
...
def dump_tlair() -> str:
    return basic_mixed_store_zN.dump_mlir(type_args=_compile_only_type_args())
...
def _require_torch_npu(device_id: int) -> Any:
    try:
        ...
        raise SystemExit("basic_mixed_store_zN --run requires PyTorch.") from exc
    try:
        import torch_npu
    except ImportError as exc:
        raise SystemExit("basic_mixed_store_zN --run requires torch_npu.") from exc
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compile and run a minimal mixed kernel, matrix A gm->ub(row->zN)->l1.")
