...
def streamk_mmad_kernel(
    ...
    aiv_out_ptr = tla.allocate(
        aiv_ub_tile_elems, dtype_gm_c, tla.AddressSpace.ub, 256
    )
    ...
    with tla.cube():
        ...
        for loop_idx in loop_range:
            ...
            swizzle_tb_loop = (loops_m + _swizzle.SWIZZLE_OFFSET - 1) // _swizzle.SWIZZLE_OFFSET
            ...
            if is_sk_now:
                ...
                block_row = block_tb_idx * _swizzle.SWIZZLE_OFFSET + block_in_tile % block_n_row
                ...
                if k_tile_idx % loops_k + cur_k_tile_num > loops_k:
                    ...
                    if streamk_tb_idx == (swizzle_tb_loop - 1):
                        streamk_n_row = loops_m - _swizzle.SWIZZLE_OFFSET * streamk_tb_idx
                    streamk_block_row = (
    ...
    with tla.vector():
        ...
        for aiv_sk_id in tla.range(c0, streamk_blocks, c1):
            ...
            aiv_tb_loop = (loops_m + _swizzle.SWIZZLE_OFFSET - 1) // _swizzle.SWIZZLE_OFFSET
            ...
            if aiv_id >= aiv_start_core:
                if aiv_id < aiv_end_core_raw:
                    aiv_loop_start = (
                        aiv_global - aiv_start_core * AIV_SUB_BLOCK_NUM
                    )
                                    ...
                                    for _aiv_rn in tla.range(
                                        ...
                                        aiv_acc_chunk.store(
                                            tla.add(
                                                aiv_acc_chunk.load(),
                                                aiv_temp_chunk.load(),
                                                mask=tla.create_mask(pattern=tla.mask.ALL, dtype=DTYPE_C)
                                            ), 
                                            mask=tla.create_mask(pattern=tla.mask.ALL, dtype=DTYPE_C)
                                        )
...
def main() -> int:
    ...
    parser.add_argument("--atol", type=float, default=1e-3, help="Comparison tolerance.")
