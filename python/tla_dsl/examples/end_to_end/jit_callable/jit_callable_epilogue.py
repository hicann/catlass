...
"""End-to-end: ``@tla.jit`` helper staged as ``tla.Constexpr`` Callable.

Same device launch path as ``constexpr_callable``, but the epilogue is a
``@tla.jit`` wrapper (``__wrapped__`` identity) rather than a bare Python def.
...
@tla.kernel
def transform_with_jit_epilogue(
    gm_src: tla.Tensor,
    gm_dst: tla.Tensor,
...
def run(args: argparse.Namespace) -> int:
    ...
    assert tla.is_jit_callable(abs_epilogue)

    torch.npu.set_device(args.device)
    block_num = get_block_num(args.block_num, args.device, kind="vector")
    print(f"--- jit_epilogue=abs n={n_ele} block_num={block_num} ---")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="@tla.jit Constexpr Callable epilogue on device."
    )
