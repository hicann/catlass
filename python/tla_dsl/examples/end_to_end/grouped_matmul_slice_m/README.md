...
| 文件 | 作用 |
|------|------|
| `grouped_matmul_slice_m.py` | Host + `@tla.kernel`（同文件）：Int32 `group_list` 前缀 + **单次 launch** + golden |
...
- 组内 MN：`GemmIdentityBlockSwizzle<3, dir>`；host 按 `m/G ≥ n` 设 Zn/Nz（`const_expr`，同 C++ ex60）
