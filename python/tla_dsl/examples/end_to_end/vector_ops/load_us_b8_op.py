...
def _print_skip(op_name: str, dtype_name: str, shape: tuple[int, ...]) -> None:
    ...
    print(
        ...
        "DIST_US_B8 currently requires i8"
    )
...
def _set_kernel_config(
    ...
    if dtype_name != "i8":
        raise SystemExit(
            f"DIST_US_B8 currently requires i8, got {dtype_name}"
        )
...
def _expected(op_name: str, inputs: tuple[Any, ...]) -> Any:
    ...
    dst = torch.full((VECTOR_ELE,), _KERNEL_SENTINEL, dtype=src.dtype, device=src.device)
...
HARNESS = DirectVectorOpHarness(
    DirectVectorOpConfig(
        ...
        env_compile_jobs="LOAD_US_B8_OP_COMPILE_JOBS",
