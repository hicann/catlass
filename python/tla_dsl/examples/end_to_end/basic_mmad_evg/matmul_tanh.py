                    ...
                    with tla.vec.func(mode="simd"):
                        for row_i in tla.range(c0, tile_rows, c1):
                            ...
                            for col_j in tla.range(c0, n_simd_strips, c1):
                                ...
                                e = tla.exp(tla.mul(clipped, 2.0, mask=tail), mask=tail)
                                result_chunk.store(
...
DESCRIPTION = "Matmul EVG tanh: D=tanh(A@B) via L0C→GM workspace + AIV; dynamic GM."
...
def run(args: argparse.Namespace) -> int:
    ...
    ref = torch.tanh(a.float() @ b.float())
