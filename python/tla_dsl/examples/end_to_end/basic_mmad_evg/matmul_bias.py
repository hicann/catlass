...
def _refresh_ub_derived() -> None:
    ...
    _ELEM_C_BYTES = dtype_size_bytes(DTYPE_GM_D.dtype)
...
DESCRIPTION = "Matmul EVG bias: D=A@B+bias via L0C→GM workspace + AIV; dynamic GM."


def golden(a, b, bias, out_dtype):
    import torch

    expected = a.to(torch.float32) @ b.to(torch.float32) + bias.to(torch.float32)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        artifact(ta, tb, td, tbias, tw, block_dim=block_dim)
