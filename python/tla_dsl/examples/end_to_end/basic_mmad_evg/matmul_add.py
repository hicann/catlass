...
DESCRIPTION = "Matmul EVG add: D=A@B+X via L0C→GM workspace + AIV; dynamic GM."


def golden(a, b, x, out_dtype):
    import torch

    expected = a.to(torch.float32) @ b.to(torch.float32) + x.to(torch.float32)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        c = torch.rand(mi, ni, dtype=torch_of[dc], device="npu") * 10.0 - 5.0
        workspace = torch.zeros((mi, ni), dtype=torch_of[dc], device="npu")
        expected = golden(a, b, c, torch_of[dc])

        ta, tb, tc, tw = (
            create_tla_tensor(a, la),
            create_tla_tensor(b, lb),
            create_tla_tensor(c, "row"),
