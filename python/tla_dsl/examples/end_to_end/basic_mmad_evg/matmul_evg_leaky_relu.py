                    ...
                    with tla.vec.func(mode="simd"):
                        for row_i in tla.range(c0, tile_rows, c1):
                            ...
                            for col_j in tla.range(c0, n_simd_strips, c1):
                                ...
                                # = max(x,0) + a*min(x,0)
                                xv = c_chunk.load()
                                pos = tla.max(xv, 0.0, mask=tail)
                                neg = tla.min(xv, 0.0, mask=tail)
                                result_chunk.store(
                                    tla.add(
                                        pos,
                                        tla.mul(neg, LEAKY_RELU_ALPHA, mask=tail),
                                        mask=tail,
                                    ),
                                    mask=tail,
                                )
...
def dump_tlair(
    ...
    return matmul_evg_leaky_relu_kernel.dump_mlir(
...
def _print_case_result(
    *,
    ...
    print(f"D equals expected leaky_relu(A@B, alpha=0.1)? {expected_match_all}")
...
def _run_one(
    ...
    *,
    ...
    # mem_d starts empty; workspace holds A×B for the epilogue.
    ...
    expected_f32 = torch.nn.functional.leaky_relu(mm, negative_slope=float(LEAKY_RELU_ALPHA))
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile/run matmul EVG leaky_relu: D=leaky_relu(A@B, alpha=0.1) via L0C→GM "
