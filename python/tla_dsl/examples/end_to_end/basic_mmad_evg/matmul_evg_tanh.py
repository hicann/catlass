...
# UB nodes counted for sizing: load Acc plus several compute temps (store reuses Out).
EVG_UB_NODES = 8
# Clip input before exp(2x) to avoid fp32 overflow in the tanh formula.
TANH_CLIP_MAX = 8.8
TANH_CLIP_MIN = -8.8
...
@tla.kernel
def matmul_evg_tanh_kernel(
    mem_a: tla.Tensor,
    mem_b: tla.Tensor,
    mem_d: tla.Tensor,
    mem_workspace: tla.Tensor,
) -> None:
    """Cube GEMM to GM workspace; vector epilogue D = tanh(Acc) via clipped (e^{2x}-1)/(e^{2x}+1)."""
                    ...
                    with tla.vec.func(mode="simd"):
                        for row_i in tla.range(c0, tile_rows, c1):
                            ...
                            for col_j in tla.range(c0, n_simd_strips, c1):
                                ...
                                # Tanh: clip to ±8.8, then (exp(2x)-1)/(exp(2x)+1)
                                xv = c_chunk.load()
                                clipped = tla.max(
                                    tla.min(xv, TANH_CLIP_MAX, mask=tail),
                                    TANH_CLIP_MIN,
                                    mask=tail,
                                )
                                e = tla.exp(
                                    tla.mul(clipped, 2.0, mask=tail), mask=tail
                                )
                                result_chunk.store(
                                    tla.div(e + (-1.0), e + 1.0, mask=tail),
                                    mask=tail,
                                )
...
def _validate_mmad_dtype_triple(dtype_a: ElemDType, dtype_b: ElemDType, dtype_c: ElemDType) -> None:
    ...
    # P0 tanh (GM workspace): epilogue is fp32; GM D narrowing not in P0.
    allowed = {
        ("f16", "f16", "f32"),
        ("bf16", "bf16", "f32"),
        ("f32", "f32", "f32"),
    ...
    if triple not in allowed:
        raise ValueError(
            ...
            "(L0C/UB compute is fp32; narrowed GM D is not supported on tanh P0)."
        )
...
def dump_tlair(
    ...
    return matmul_evg_tanh_kernel.dump_mlir(
...
def _print_case_result(
    *,
    ...
    print(f"D equals expected tanh(A@B)? {expected_match_all}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile/run matmul EVG tanh: D=tanh(A@B) via L0C→GM "
    ...
    parser.add_argument(
        ...
        type=_parse_elem_dtype,
        default="f32",
        help="GM element type for D (M×N): f32 (P0); narrowed f16/bf16 GM D not supported.",
    )
    parser.add_argument(
        "--all-mmad-dtypes",
        action="store_true",
