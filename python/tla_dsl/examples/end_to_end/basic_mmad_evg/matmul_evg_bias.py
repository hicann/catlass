...
DTYPE_GM_D = tla.Float32  # GM D / bias / workspace (follows dtype-c)
ENABLE_UNIT_FLAG = True

# UB nodes counted for sizing: load Acc, load bias row, compute (store reuses Out).
EVG_UB_NODES = 3
...
def _elem_c_bytes() -> int:
    return dtype_size_bytes(DTYPE_GM_D.dtype)
...
def matmul_evg_bias_kernel(
    ...
    """Cube GEMM to GM workspace; vector epilogue D = Acc + bias (bias is 1×N)."""
    ...
    # ---- UB allocates ----
    ub_acc_ptr0 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
    ub_aux_ptr0 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
    ub_out_ptr0 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
    ub_acc_ptr1 = ub_acc_ptr0
    ub_aux_ptr1 = ub_aux_ptr0
    ub_out_ptr1 = ub_out_ptr0
    if EVG_UB_STAGES >= 2:
        ub_acc_ptr1 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
        ub_aux_ptr1 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
        ub_out_ptr1 = tla.allocate(ub_slot_elems, DTYPE_GM_D, tla.AddressSpace.ub, 256)
    ...
    with tla.vector():
        ...
        for block_linear in block_range:
            ...
            # Bias is 1×N; take the N-column slice for this MN block.
            gm_bias_by_core = tla.tile_view(
                mem_bias, tla.make_shape(1, l1_tn), tla.make_coord(c0, block_col)
            )
            ...
            if half_n <= ub_slot_elems:
                ...
                for tile_i in tla.range(c0, n_epilogue_row_tiles, c1):
                    ...
                    # Load the 1×cols bias segment for this MN half-tile.
                    gm_bias_tile = tla.tile_view(
                        gm_bias_by_core,
                        tla.make_shape(1, half_n),
                        tla.make_coord(c0, c0),
                    )
                    tile_rows = gm_d_tile.origin_shape[0]
                    tile_cols = gm_d_tile.origin_shape[1]
                    ...
                    # Full MxN UB slot; bias is read from row 0 during compute.
                    ub_bias = tla.make_tensor(
                        ub_aux_ptr1 if use_ub_stage1 else ub_aux_ptr0, ub_layout
                    )
                    ub_result = tla.make_tensor(
                        ub_out_ptr1 if use_ub_stage1 else ub_out_ptr0, ub_layout
                    )
                    ub_bias_row0 = tla.tile_view(
                        ub_bias, tla.make_shape(1, tile_cols), tla.make_coord(c0, c0)
                    )

                    # Wait UB slot free → load accumulator and bias row → signal compute may start.
                    if use_ub_stage1:
                        tla.wait_flag(ub_load_ready_1)
                    else:
                        tla.wait_flag(ub_load_ready_0)
                    tla.copy(ub_acc, gm_c_tile)
                    tla.copy(ub_bias_row0, gm_bias_tile)
                    if use_ub_stage1:
                    ...
                    # Add Acc + bias, reading bias from UB row 0 each row
                    # (no full-row replicate into every UB row).
                    with tla.vec.func(mode="simd"):
                        for row_i in tla.range(c0, tile_rows, c1):
                            ...
                            for col_j in tla.range(c0, n_simd_strips, c1):
                                ...
                                bias_chunk = tla.tile_view(
                                    ub_bias,
                                    tla.make_shape(REG_M, simd_lanes),
                                    tla.make_coord(c0, col_j),
                                )
                                ...
                                tail, remaining = tla.update_mask(
                                    remaining, dtype=DTYPE_GM_D
                                )
                                result_chunk.store(
                                    c_chunk.load() + bias_chunk.load(), mask=tail
                                )
                ...
                for row_i in tla.range(c0, half_m, c1):
                    for col_tile in tla.range(c0, n_col_tiles, c1):
                        ...
                        gm_bias_tile = tla.tile_view(
                            gm_bias_by_core,
                            tla.make_shape(1, ub_slot_elems),
                            tla.make_coord(c0, col_tile),
                        )
                        tile_rows = gm_d_tile.origin_shape[0]
                        tile_cols = gm_d_tile.origin_shape[1]
                        ub_row_stride = (
                        ...
                        # COMPUTE: Add with bias broadcast-on-read from UB row 0.
                        with tla.vec.func(mode="simd"):
...
def _validate_mmad_dtype_triple(dtype_a: ElemDType, dtype_b: ElemDType, dtype_c: ElemDType) -> None:
    ...
    # bf16 output is not supported for this op.
    allowed = {
        ("f16", "f16", "f16"),
        ("f16", "f16", "f32"),
        ("bf16", "bf16", "f16"),
    ...
    with runtime_mod._eager_capture():
        return (
            ...
            tla.Tensor(
                tla.make_shape(1, n),
                tc,
                origin_shape=tla.make_shape(1, n),
                layout_tag=tla.arch.RowMajor,
            ).mark_layout_dynamic(),
...
def dump_tlair(
    ...
    return matmul_evg_bias_kernel.dump_mlir(
...
def _print_case_result(
    *,
    ...
    print(f"D still equals zeros? {unchanged_all}")
    print(f"D equals expected A@B+bias? {expected_match_all}")
    print(f"D changed count={changed_count}")
...
def _run_one(
    ...
    *,
    ...
    # Fill A/B/bias with uniform[-5, 5].
    ...
    # mem_d is output; mem_bias is 1×N; workspace holds A×B for the epilogue.
    torch_tensor_d = torch.zeros((m, n), dtype=torch_dtype_c, device=device)
    torch_bias = torch.empty(
        (1, n), dtype=torch.float32, device=device
    ).uniform_(-5.0, 5.0).to(torch_dtype_c)
    torch_workspace = torch.zeros((m, n), dtype=torch_dtype_c, device=device)
    initial_d = torch_tensor_d.detach().clone()
    expected_f32 = (
        torch_tensor_a.to(torch.float32) @ torch_tensor_b.to(torch.float32)
        + torch_bias.to(torch.float32)
    )
    ...
    tla_bias = _create_tla_tensor(torch_bias, "row")
    ...
    artifact(
        tla_tensor_a, tla_tensor_b, tla_tensor_d, tla_bias, tla_workspace, block=block
    )

    torch.npu.synchronize()
    actual = torch_tensor_d.to(torch.float32)
    unchanged = _compare_close_cpp(
        actual, initial_d.to(torch.float32), rtol=rtol
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile/run matmul EVG bias: D=A@B+rowbroadcast(X) via L0C→GM "
            "workspace + AIV. Kernel takes 5 GM tensors "
            "(A, B, D, bias 1×N, workspace). "
    ...
    parser.add_argument(
        ...
        type=_parse_elem_dtype,
        default="f32",
        help="GM element type for D/bias: f16 or f32 (bf16 output not supported).",
    )
    parser.add_argument(
        "--all-mmad-dtypes",
        action="store_true",
