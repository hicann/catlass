...
DTYPE_A = tla.Float32
DTYPE_B = tla.Float32
DTYPE_C = tla.Float32  # L0C accumulator / UB compute
DTYPE_GM_C = tla.Float32  # GM C: initial addend X, overwritten with D=A@B+X
...
# Host may rewrite before compile, e.g. ``EVG_UB_STAGES = 1``.
EVG_UB_STAGES = 2
...
def evg_compute_length(stages: int | None = None) -> int:
    ...
    budget = UB_SIZE - L0C_SIZE // 2
    raw = budget // EVG_UB_NODES // s // _elem_c_bytes()
...
def matmul_evg_add_ub_kernel(
    ...
    # Compile-time EVG sizing (host may have rewritten EVG_UB_STAGES before compile).
    ...
    fixpipe_free = tla.flag("fixpipe_free", tla.arch.FIX, tla.arch.CUBE)

    # ---- EVG/UB soft-flags ----
    #   V_MTE2  (ub_load_ready):  Vector frees X slot for MTE2 load (cross-tile)
    #   MTE2_V  (ub_loaded):      MTE2 load done → Vector may compute
    #   V_MTE3  (vec_done):       Vector compute done → MTE3 may store
    #   MTE3_V  (ub_store_done):  MTE3 store done → Vector may reuse D slot (cross-tile)
    ...
    with tla.vector():
        ...
        for block_linear in block_range:
            ...
            # SPLIT_M half-tile: request ceil(M/2)×N, let origin_shape clip the tail.
            actual_m = gm_c_by_core.origin_shape[0]
            actual_n = gm_c_by_core.origin_shape[1]
            half_m = (actual_m + 1) // 2
            gm_result = tla.tile_view(
                gm_c_by_core, tla.make_shape(half_m, actual_n), tla.make_coord(aiv_sub_idx, c0)
            )
...
# Fixed default for reproducible A/B/X fills (questboard acceptance uses the same).
...
def _validate_mmad_dtype_triple(dtype_a: ElemDType, dtype_b: ElemDType, dtype_c: ElemDType) -> None:
    ...
    # P0 add_ub: UB epilogue is fp32; GM C narrowing (f16/bf16) is not legalized for L0C→UB path.
    allowed = {
        ("f16", "f16", "f32"),
        ("bf16", "bf16", "f32"),
        ("f32", "f32", "f32"),
    ...
    if triple not in allowed:
        raise ValueError(
            ...
            "f16,f16,f32 | bf16,bf16,f32 | f32,f32,f32 "
            "(L0C/UB compute is fp32; narrowed GM C is not supported on add_ub P0)."
        )
...
def _launch_block(block: int, device: int, m_val: int, n_val: int) -> int:
    """Cube core count: min(available, task_count) when block<=0; else min(block, available)."""
    aic = int(tla.get_aicore_num(device))
    task_num = (
        (int(m_val) + l1_tm - 1) // l1_tm
        * ((int(n_val) + l1_tn - 1) // l1_tn)
    )
    if int(block) > 0:
        used = min(int(block), aic)
    else:
        used = min(aic, task_num)
    return max(1, used)
...
def dump_tlair(
    ...
    return matmul_evg_add_ub_kernel.dump_mlir(
...
def _print_case_result(
    *,
    ...
    print(f"C still equals initial addend X? {unchanged_all}")
    print(f"C equals expected A@B+X? {expected_match_all}")
...
def _run_one(
    ...
    *,
    ...
    # Three-arg in-place API: mem_c starts as addend X, then becomes D = A@B+X.
    torch_tensor_c = torch.empty(
        (m, n), dtype=torch.float32, device=device
    ).uniform_(-5.0, 5.0).to(torch_dtype_c)
    initial_x = torch_tensor_c.detach().clone()
    expected_f32 = (
        torch_tensor_a.to(torch.float32) @ torch_tensor_b.to(torch.float32)
        + initial_x.to(torch.float32)
    )
    ...
    block = _launch_block(args.block, args.device, m_val, n_val)
    ...
    unchanged = _compare_close_cpp(
        actual, initial_x.to(torch.float32), rtol=rtol
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile/run matmul EVG add_ub: D=A@B+X via L0C→UB. "
            "Kernel takes 3 GM tensors (A, B, C); C initially holds addend X and is "
            "overwritten with D = A@B+X. "
    ...
    parser.add_argument(
        ...
        help=f"RNG seed for A/B/X fills (default: {DEFAULT_RNG_SEED}).",
    )
    parser.add_argument(
        "--m",
        type=int,
