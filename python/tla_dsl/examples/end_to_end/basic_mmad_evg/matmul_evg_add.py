...
# ``DTYPE_GM_C``: GM C/X/D, GM workspace, and UB epilogue buffers.
DTYPE_A = tla.Float32
DTYPE_B = tla.Float32
DTYPE_C = tla.Float32  # L0C accumulator only
DTYPE_GM_C = tla.Float32  # ElementC: GM C/X/D, workspace, UB epilogue
ENABLE_UNIT_FLAG = True

# UB nodes counted for sizing: load Acc, load Aux(X), compute (store reuses Out).
EVG_UB_NODES = 3
...
@tla.kernel
def matmul_evg_add_kernel(
    mem_a: tla.Tensor,
    mem_b: tla.Tensor,
    mem_c: tla.Tensor,
    mem_workspace: tla.Tensor,
) -> None:
    """Cube GEMM to GM workspace; vector epilogue D = Acc + X (X from mem_c)."""
    ...
    # No per-tile aiv_finish, so FixPipe(N+1) can overlap Add(N).
        ...
        for block_linear in block_range:
            ...
            if half_n <= ub_slot_elems:
                ...
                for tile_i in tla.range(c0, n_epilogue_row_tiles, c1):
                    ...
                    # Wait UB slot free → load accumulator and addend → signal compute may start.
                    if use_ub_stage1:
...
def _validate_mmad_dtype_triple(dtype_a: ElemDType, dtype_b: ElemDType, dtype_c: ElemDType) -> None:
    ...
    # Supported: A==B in {f16,f32,bf16}, output C in {f16,f32}.
    allowed = {
        ("f16", "f16", "f16"),
        ("f16", "f16", "f32"),
        ("bf16", "bf16", "f16"),
    ...
    if triple not in allowed:
        raise ValueError(
            ...
            "(bf16 output may fail precision checks)."
        )
...
def _compile_only_type_args(
    ...
    with runtime_mod._eager_capture():
        return (
            ...
            # GM workspace for A×B (ElementC); FixPipe casts L0C fp32→ElementC.
            tla.Tensor(
                tla.make_shape(m, n),
                tc,
                origin_shape=tla.make_shape(m, n),
...
def dump_tlair(
    ...
    return matmul_evg_add_kernel.dump_mlir(
...
def _run_one(
    ...
    *,
    ...
    # mem_c initially holds addend X, then becomes D = A@B+X.
    # Workspace is m×n of the GM element type for A×B before the epilogue.
    ...
    artifact(
        tla_tensor_a, tla_tensor_b, tla_tensor_c, tla_workspace, block=block
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile/run matmul EVG add: D=A@B+X via L0C→GM workspace + AIV. "
            "Kernel takes 4 GM tensors (A, B, C, workspace); C initially holds "
            "addend X and is overwritten with D = A@B+X. "
