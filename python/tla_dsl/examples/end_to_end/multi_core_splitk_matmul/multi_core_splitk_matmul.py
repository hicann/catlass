...
def multi_core_splitk_mmad_kernel(
    ...
    if tla.const_expr(dtype_gm_c != tla.Float32):
        ub_cast_ptr = tla.allocate(_splitk.task_per_aiv, dtype_gm_c, tla.AddressSpace.ub, 256)
    ...
    with tla.cube():
        ...
        for task in task_range:
            ...
            gm_w_by_core = tla.tile_view(
                gm_ws_plane, tla.make_shape(_tiling.l1_tm, _tiling.l1_tn),
...
def compute_reduce_tiling(
    ...
    task = ((per_aiv + ELE_PER_VECTOR_BLOCK - 1) // ELE_PER_VECTOR_BLOCK) * ELE_PER_VECTOR_BLOCK
