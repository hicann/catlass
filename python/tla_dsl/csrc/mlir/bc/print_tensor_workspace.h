...
[aicore] __attribute__((always_inline)) inline uint32_t
EncodeDescriptor(uint64_t call_id, uint32_t subblock_tag = 0) {
  return kDescriptorNamespace | (subblock_tag << 16) |
         (static_cast<uint32_t>(call_id) & 0xffffU);
}
