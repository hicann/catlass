...
CATLASS_DEVICE void copyL0CToGM(
    memref_t<__cc__ ElementSrc, 1> *src, memref_t<__gm__ ElementDst, 2> *dst,
    const TensorDesc &srcDesc, const TensorDesc &dstDesc, uint8_t unitFlag) {
...
CATLASS_DEVICE void copyL0CToL1(
    memref_t<__cc__ ElementSrc, 1> *src, memref_t<__cbuf__ ElementDst, 1> *dst,
    const TensorDesc &srcDesc, const TensorDesc &dstDesc, uint8_t unitFlag) {
    auto srcTensor = makeL0CTensor<LayoutTag::L0C, ElementSrc>(src, srcDesc);
    auto dstTensor = makeL1Tensor<DstLayout, ElementDst>(dst, dstDesc);
    Catlass::Gemm::Tile::CopyL0CToL1Tla<ArchTag, decltype(srcTensor),
                                        decltype(dstTensor)>{}(dstTensor,
                                                               srcTensor, unitFlag);
}

template <class ArchTag, LayoutTag DstLayout,
    Catlass::Gemm::Tile::CopyL0CToUBMode Mode, typename ElementSrc, typename ElementDst>
CATLASS_DEVICE void copyL0CToUB(
    memref_t<__cc__ ElementSrc, 1> *src, memref_t<__ubuf__ ElementDst, 1> *dst,
    const TensorDesc &srcDesc, const TensorDesc &dstDesc, uint8_t unitFlag, uint8_t subBlockId) {
    auto srcTensor = makeL0CTensor<LayoutTag::L0C, ElementSrc>(src, srcDesc);
    ...
    } else if constexpr (Mode == Catlass::Gemm::Tile::CopyL0CToUBMode::SPLIT_M ||
...
void _mlir_ciface_copy_gm_##NameSrc##_to_l1_##NameDst##_##DType(                                             \
    memref_t<__gm__ DType, 2> *src, memref_t<__cbuf__ DType, 1> *dst,                                        \
...
[aicore] __attribute__((always_inline))                                                                      \
void _mlir_ciface_copy_l1_##NameSrc##_to_l0a_zN_##DType(                                                     \
    memref_t<__cbuf__ DType, 1> *src, memref_t<__ca__ DType, 1> *dst,                                        \
    DESC_ABI_PARAMS(src), DESC_ABI_PARAMS(dst)) {                                                            \
...
[aicore] __attribute__((always_inline))                                                                      \
void _mlir_ciface_copy_l1_##NameSrc##_to_l0b_nZ_##DType(                                                     \
    memref_t<__cbuf__ DType, 1> *src, memref_t<__cb__ DType, 1> *dst,                                        \
    DESC_ABI_PARAMS(src), DESC_ABI_PARAMS(dst)) {                                                            \
...
[aicore] __attribute__((always_inline))                                                                      \
void _mlir_ciface_copy_l0c_to_gm_row_major_##DTypeDst(                                                       \
    memref_t<__cc__ DTypeSrc, 1> *src, memref_t<__gm__ DTypeDst, 2> *dst,                                    \
    DESC_ABI_PARAMS(src), DESC_ABI_PARAMS(dst), uint8_t unitFlag) {                                          \
...
[aicore] __attribute__((always_inline))                                                                      \
void _mlir_ciface_copy_l0c_to_l1_zN_##DTypeDst(                                                              \
    memref_t<__cc__ DTypeSrc, 1> *src, memref_t<__cbuf__ DTypeDst, 1> *dst,                                  \
    DESC_ABI_PARAMS(src), DESC_ABI_PARAMS(dst), uint8_t unitFlag) {                                          \
...
[aicore] __attribute__((always_inline))                                                                      \
void _mlir_ciface_copy_l0c_to_ub_##NameDst##_##mode##_##DTypeDst(                                            \
    memref_t<__cc__ DTypeSrc, 1> *src, memref_t<__ubuf__ DTypeDst, 1> *dst,                                  \
    DESC_ABI_PARAMS(src), DESC_ABI_PARAMS(dst), uint8_t unitFlag, uint8_t subBlockId) {                      \
    copyL0CToUB<Catlass::Arch::Ascend950, LayoutTag::EnumDst,                                                \
                Catlass::Gemm::Tile::CopyL0CToUBMode::MODE, DTypeSrc, DTypeDst>(                            \
        src, dst, TENSOR_DESC_12(src), TENSOR_DESC_12(dst), unitFlag, subBlockId);                           \
}
