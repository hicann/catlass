...
void printTensor(uint64_t print_workspace, uint64_t tensor_address,
  ...
  AscendC::ShapeInfo shapeInfo =
      MakeShapeInfo(static_cast<uint32_t>(signedShape0),
  ...
  AscendC::DumpTensor(tensor[0],
                      tla::print_tensor::EncodeDescriptor(call_id, subblockTag),
...
template <typename ElementType>
[aicore] __attribute__((always_inline))
void printLocalTensor(uint64_t print_workspace, uint64_t tensor_address,
                      uint64_t count, uint64_t packed_shape,
                      uint64_t call_id) {
  int32_t signedShape0 = static_cast<int32_t>(packed_shape);
  int32_t signedShape1 = static_cast<int32_t>(packed_shape >> 32);
  if (signedShape0 <= 0 || signedShape1 < 0 || count == 0 ||
      count > kMaxFloat32Elements || (tensor_address & 31U) != 0U ||
      count > static_cast<uint64_t>(signedShape0) *
                  (signedShape1 == 0 ? 1U : static_cast<uint32_t>(signedShape1)))
    return;
...
#define TLA_PRINT_TENSOR_UB_WRAPPER(SUFFIX, TYPE)                           \
  [aicore] __attribute__((always_inline)) void                              \
      _mlir_ciface_tla_print_tensor_ub_##SUFFIX(                            \
          uint64_t workspace, uint64_t address, uint64_t count,             \
          uint64_t packed_shape, uint64_t call_id) {                        \
