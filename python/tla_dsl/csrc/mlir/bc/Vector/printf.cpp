...
[aicore] __attribute__((always_inline))
void tla_printf_write_format_record(const char *format,
                                    uint64_t formatLen,
                                    uint64_t argsNum,
                                    const uint64_t *args,
                                    uint64_t printWorkspace) {
    ...
    if (format == nullptr || argsNum > kMaxArgs ||
        (argsNum != 0 && args == nullptr)) {
        return;
    }
    ...
    __gm__ uint8_t *record =
        g_sysPrintFifoSpace + static_cast<uint64_t>(blockIdx) * kDebugBlockLength;
    __gm__ uint8_t *ring = record + kRingBufferOffset;
    __gm__ uint8_t *writeInfo = ring + kRingBufferBytes;
    __gm__ uint64_t *writeOffset =
        reinterpret_cast<__gm__ uint64_t *>(writeInfo + 8);
    uint64_t offset = *writeOffset;
    uint64_t rawTotal = kPrintTlvBytes + argsBytes + formatLen + 1;
    uint64_t total =
        (rawTotal + kPrintRecordAlignment - 1) &
        ~(kPrintRecordAlignment - 1);
    ...
    *reinterpret_cast<__gm__ uint32_t *>(tlv + 4) =
        static_cast<uint32_t>(total - 8);
    *reinterpret_cast<__gm__ uint32_t *>(tlv + 8) = blockIdx;
    *reinterpret_cast<__gm__ uint32_t *>(tlv + 12) = 0;
    *reinterpret_cast<__gm__ uint64_t *>(tlv + 16) =
        kPrintFmtOffset + argsBytes;
    for (uint64_t i = 0; i < argsNum; ++i) {
        *reinterpret_cast<__gm__ uint64_t *>(
            tlv + kPrintTlvBytes + i * sizeof(uint64_t)) = args[i];
    }
...
[aicore] __attribute__((always_inline))
void _mlir_ciface_tla_printf_format_string(const char *format,
                                           uint64_t formatLen,
                                           uint64_t printWorkspace) {
    tla_printf_write_format_record(format, formatLen, 0, nullptr,
                                   printWorkspace);
}

[aicore] __attribute__((always_inline))
void _mlir_ciface_tla_printf_format_values(const char *format,
                                           uint64_t formatLen,
                                           uint64_t argsNum,
                                           uint64_t arg0,
                                           uint64_t arg1,
    ...
    tla_printf_write_format_record(format, formatLen, argsNum, args,
                                   printWorkspace);
}
