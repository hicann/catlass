...
Type TlaDialect::parseType(DialectAsmParser& parser) const
{
    ...
    if (!parseResult.has_value()) {
        if (mnemonic == "flag" || mnemonic == "cross_flag")
            return OpaqueType::get(StringAttr::get(getContext(), getDialectNamespace()), parser.getFullSymbolSpec());
...
Attribute TlaDialect::parseAttribute(DialectAsmParser& parser, Type type) const
{
    ...
    if (attrTag == "store_dist") {
        if (parser.parseLess())
            return {};
        StringRef distKeyword;
        if (parser.parseKeyword(&distKeyword) || parser.parseGreater())
            return {};
        auto symbolized = ::symbolizeStoreDist(distKeyword);
        if (!symbolized) {
            parser.emitError(parser.getNameLoc()) << "invalid tla.store_dist value: " << distKeyword;
            return {};
        }
