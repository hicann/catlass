...
static bool isTlaTileType(Type type)
{
    return llvm::isa<::tla::TlaTensorType>(type);
}
...
static bool rewriteTlaTileTypesToLLVMPointer(ModuleOp module)
{
    ...
    for (func::CallOp callOp : calls) {
        ...
        auto newCall = builder.create<func::CallOp>(
            callOp.getLoc(), callOp.getCallee(), callOp.getResultTypes(), callOp.getOperands());
...
static llvm::Value* createRank1DescriptorAlloca(
    llvm::IRBuilder<>& builder, ArrayRef<llvm::Value*> fields, llvm::Twine name)
{
    assert(fields.size() == 5 && "expected 5 flattened fields for rank-1 memref");
...
static llvm::Value* createRank2DescriptorAlloca(
    llvm::IRBuilder<>& builder, ArrayRef<llvm::Value*> fields, llvm::Twine name)
{
    assert(fields.size() == 7 && "expected 7 flattened fields for rank-2 memref");
...
static llvm::Function* getOrCreateLoadWrapper(llvm::Module& module, llvm::Function& callee)
{
    ...
    SmallVector<llvm::Type*, 13> wrapperArgs = {gmPtrTy, gmPtrTy, i64Ty, i64Ty, i64Ty, ubPtrTy, ubPtrTy,
                                                i64Ty,   i64Ty,   i64Ty, i32Ty, f32Ty, i64Ty};
    auto* wrapperTy = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), wrapperArgs, false);
    auto* wrapper = llvm::Function::Create(wrapperTy, llvm::GlobalValue::PrivateLinkage, kWrapperName, module);
    ...
    llvm::Value* srcDesc =
        createRank1DescriptorAlloca(builder, {srcAllocated, srcAligned, srcOffset, srcSize, srcStride}, "src.desc");
    llvm::Value* dstDesc =
        createRank1DescriptorAlloca(builder, {dstAllocated, dstAligned, dstOffset, dstSize, dstStride}, "dst.desc");
...
static llvm::Function* getOrCreateVaddWrapper(llvm::Module& module, llvm::Function& callee)
{
    ...
    llvm::Type* i64Ty = llvm::Type::getInt64Ty(ctx);
    llvm::Type* ubPtrTy = llvm::PointerType::get(ctx, 6);
    SmallVector<llvm::Type*, 20> wrapperArgs = {ubPtrTy, ubPtrTy, i64Ty,   i64Ty,   i64Ty,   ubPtrTy, ubPtrTy,
                                                i64Ty,   i64Ty,   i64Ty,   ubPtrTy, ubPtrTy, i64Ty,   i64Ty,
                                                i64Ty,   ubPtrTy, ubPtrTy, i64Ty,   i64Ty,   i64Ty};
    auto* wrapperTy = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), wrapperArgs, false);
    auto* wrapper = llvm::Function::Create(wrapperTy, llvm::GlobalValue::PrivateLinkage, kWrapperName, module);
    wrapper->addFnAttr(llvm::Attribute::AlwaysInline);
    ...
    llvm::BasicBlock* entry = llvm::BasicBlock::Create(ctx, "entry", wrapper);
    llvm::IRBuilder<> builder(entry);
    llvm::Value* src0Desc = createRank1DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(0, 5), "src0.desc");
    llvm::Value* src1Desc = createRank1DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(5, 5), "src1.desc");
    llvm::Value* dstDesc = createRank1DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(10, 5), "dst.desc");
    llvm::Value* tmpDesc = createRank1DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(15, 5), "tmp.desc");
    builder.CreateCall(&callee, {src0Desc, src1Desc, dstDesc, tmpDesc});
    builder.CreateRetVoid();
    return wrapper;
}

static llvm::Function* getOrCreateRank2CopyWrapper(
    llvm::Module& module, llvm::Function& callee, llvm::StringRef wrapperName, unsigned srcAddrspace,
    unsigned dstAddrspace)
{
    ...
    SmallVector<llvm::Type*, 14> wrapperArgs = {srcPtrTy, srcPtrTy, i64Ty, i64Ty, i64Ty, i64Ty, i64Ty,
                                                dstPtrTy, dstPtrTy, i64Ty, i64Ty, i64Ty, i64Ty, i64Ty};
    ...
    auto* wrapperTy = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), wrapperArgs, false);
    auto* wrapper = llvm::Function::Create(wrapperTy, llvm::GlobalValue::PrivateLinkage, wrapperName, module);
    wrapper->addFnAttr(llvm::Attribute::AlwaysInline);
    ...
    llvm::BasicBlock* entry = llvm::BasicBlock::Create(ctx, "entry", wrapper);
    llvm::IRBuilder<> builder(entry);
    llvm::Value* srcDesc = createRank2DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(0, 7), "src.desc");
    llvm::Value* dstDesc = createRank2DescriptorAlloca(builder, ArrayRef<llvm::Value*>(args).slice(7, 7), "dst.desc");
...
static llvm::Function* getOrCreateCopyTileCbufToCaWrapper(llvm::Module& module, llvm::Function& callee)
{
...
static llvm::Function* getOrCreateCopyTileCbufToCbWrapper(llvm::Module& module, llvm::Function& callee)
{
...
static llvm::Value* resolveInsertedValueForIndices(llvm::Value* aggregate, ArrayRef<unsigned> indices)
{
...
static llvm::Value* peelDescriptorExtract(llvm::Value* value)
{
    ...
    if (!extract)
        return value;
    if (llvm::Value* inserted = resolveInsertedValueForIndices(extract->getAggregateOperand(), extract->getIndices()))
        return inserted;
    return value;
}
...
{
    auto rewriteCallUsers = [&](llvm::StringRef calleeName, auto wrapperFactory, unsigned droppedTrailingArgs) {
    ...
    if (!rewriteCallUsers(
            "_mlir_ciface_load_gm_to_ubuf_1d_float", getOrCreateLoadWrapper,
            /*droppedTrailingArgs=*/1))
        return false;
    if (!rewriteCallUsers(
            "_mlir_ciface_store_ubuf_to_gm_1d_float", getOrCreateStoreWrapper,
            /*droppedTrailingArgs=*/0))
        return false;
    if (!rewriteCallUsers(
            "_mlir_ciface_vadd_1d_float", getOrCreateVaddWrapper,
            /*droppedTrailingArgs=*/0))
        return false;
    if (!rewriteCallUsers(
        ...
        return false;
    if (!rewriteCallUsers(
            "_mlir_ciface_copy_tile_cbuf_zn_to_ca_zz_c310", getOrCreateCopyTileCbufToCaWrapper,
            /*droppedTrailingArgs=*/0))
        return false;
    if (!rewriteCallUsers(
            "_mlir_ciface_copy_tile_cbuf_zn_to_cb_nz_c310", getOrCreateCopyTileCbufToCbWrapper,
            /*droppedTrailingArgs=*/0))
        return false;
    if (!rewriteCallUsers(
...
static void addHivmKernelAnnotations(llvm::Module& module)
{
    ...
    llvm::Metadata* kernelValue = llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 1));
...
static void assignHivmDeclarationAttributes(llvm::Module& module)
{
    ...
    addAttrs(
        "llvm.hivm.SBITSET0", {llvm::Attribute::NoUnwind}, /*inaccessibleMemOnly=*/false,
        /*readNone=*/true);
...
static bool rewriteGMRawPointerABI(llvm::Module& module, const GMFunctionArgMap& gmArgInfoByFunc, std::string& error)
{
    llvm::LLVMContext& ctx = module.getContext();
    llvm::Type* i64Ty = llvm::Type::getInt64Ty(ctx);
    ...
    for (llvm::Function* func : functions) {
        ...
        auto* newFunc = llvm::Function::Create(
            newFuncTy, func->getLinkage(), func->getAddressSpace(), func->getName() + ".gm_raw", &module);
        ...
        for (auto [inputOrdinal, info] : llvm::enumerate(argInfo)) {
            ...
            if (info.isGMRank1Static) {
                ...
                func->getArg(oldStart + 3)->replaceAllUsesWith(llvm::ConstantInt::get(i64Ty, info.staticSize));
...
{
    registry.insert<
        arith::ArithDialect, cf::ControlFlowDialect, mlir::DLTIDialect, func::FuncDialect, scf::SCFDialect,
        LLVM::LLVMDialect, ::mlir::memref::MemRefDialect, vector::VectorDialect, ::tla::TlaDialect>();
    registry.insert<
...
bool runTlaCompilePipelines(ModuleOp module, llvm::StringRef emitMode, std::string& output, std::string& error)
{
    MLIRContext* context = module.getContext();
    PassManager tlaPm(context);
    PassManager llvmPm(context);
    buildTlaCompilePassManagers(*context, tlaPm, llvmPm);
    return runTlaCompilePipelinesWithManagers(
        module, emitMode, tlaPm, llvmPm, output, error,
        /*rewriteTileSignaturesToLLVMPointer=*/
        false);
}

bool runTlaCompilePipelinesWithManagers(
    ModuleOp module, llvm::StringRef emitMode, PassManager& tlaPm, PassManager& llvmPm, std::string& output,
    std::string& error, bool rewriteTileSignaturesToLLVMPointer)
{
    ...
    if (emitMode == "llvm") {
        ...
        if (rewriteTileSignaturesToLLVMPointer) {
            ...
            if (hasUnloweredTlaTileSignatures(module, tileDetail)) {
                error =
                    "Failed to run LLVM conversion pipeline: unresolved tla.tile_view types "
