...
public:
  ...
  static bool isTransparentMutexForward(Value value, Value source, DenseSet<Value> &visited) {
    if (value == source)
      return true;
    if (!value || !source || !isTlaMutexType(value.getType()) || !isTlaMutexType(source.getType()))
      return false;
    ...
    if (auto ifOp = value.getDefiningOp<scf::IfOp>()) {
      ...
      return isTransparentMutexForward(thenYield.getOperand(resultIndex), source, thenVisited) &&
             isTransparentMutexForward(elseYield.getOperand(resultIndex), source, elseVisited);
    }
    ...
    if (auto forOp = value.getDefiningOp<scf::ForOp>()) {
      ...
      return isTransparentMutexForward(forOp.getInitArgs()[resultIndex], source, initVisited) &&
             isTransparentMutexForward(yieldOp.getOperand(resultIndex),
                                       forOp.getRegionIterArg(resultIndex), yieldVisited);
    }
  ...
  static void emitMutexResolveError(Operation *diagnosticOp, bool *emittedDiagnostic,
                                    const Twine &message) {
    if (!diagnosticOp)
      return;
  ...
  static FailureOr<::tla::MutexOp> resolveMutexRootImpl(Value value, DenseSet<Value> &visited,
                                                        Operation *diagnosticOp,
    ...
    if (auto blockArg = dyn_cast<BlockArgument>(value)) {
      ...
      auto forOp = owner ? dyn_cast_or_null<scf::ForOp>(owner->getParentOp()) : scf::ForOp();
      ...
      FailureOr<::tla::MutexOp> initRoot = resolveMutexRootImpl(
          forOp.getInitArgs()[iterIndex], visited, diagnosticOp, emittedDiagnostic,
          verifyForBackedge);
      if (failed(initRoot))
        return failure();
      ...
      if (!sameMutexRoot(*initRoot, *yieldRoot)) {
        emitMutexResolveError(
            diagnosticOp, emittedDiagnostic,
            "cannot lower tla.mutex through scf.for iter_arg with different init/yield mutex operands");
        return failure();
      }
      return *initRoot;
    }
    ...
    if (auto forOp = value.getDefiningOp<scf::ForOp>()) {
      ...
      DenseSet<Value> initVisited = visited;
      FailureOr<::tla::MutexOp> initRoot = resolveMutexRootImpl(
          forOp.getInitArgs()[resultIndex], initVisited, diagnosticOp, emittedDiagnostic, true);
      DenseSet<Value> yieldVisited = visited;
      FailureOr<::tla::MutexOp> yieldRoot = resolveMutexRootImpl(
          yieldOp.getOperand(resultIndex), yieldVisited, diagnosticOp, emittedDiagnostic, false);
      if (failed(initRoot) || failed(yieldRoot))
        return failure();
      if (!sameMutexRoot(*initRoot, *yieldRoot)) {
        emitMutexResolveError(
            diagnosticOp, emittedDiagnostic,
            "cannot lower tla.mutex through scf.for result with different init/yield mutex operands");
        return failure();
      }
      return *initRoot;
    }
    ...
    if (auto ifOp = value.getDefiningOp<scf::IfOp>()) {
      ...
      DenseSet<Value> thenVisited = visited;
      FailureOr<::tla::MutexOp> thenRoot = resolveMutexRootImpl(
          thenYield.getOperand(resultIndex), thenVisited, diagnosticOp, emittedDiagnostic, true);
      DenseSet<Value> elseVisited = visited;
      FailureOr<::tla::MutexOp> elseRoot = resolveMutexRootImpl(
          elseYield.getOperand(resultIndex), elseVisited, diagnosticOp, emittedDiagnostic, true);
      if (failed(thenRoot) || failed(elseRoot))
        return failure();
      if (!sameMutexRoot(*thenRoot, *elseRoot)) {
        emitMutexResolveError(
            diagnosticOp, emittedDiagnostic,
            "cannot lower tla.mutex through scf.if result with different branch mutex operands");
        return failure();
      }
      return *thenRoot;
    }
  ...
  static FailureOr<::tla::MutexOp> resolveMutexRoot(Value value, Operation *diagnosticOp = nullptr,
                                                    bool *emittedDiagnostic = nullptr) {
    DenseSet<Value> visited;
    return resolveMutexRootImpl(value, visited, diagnosticOp, emittedDiagnostic, true);
  }
  ...
  static FailureOr<Value> materializeMutexIdImpl(PatternRewriter &rewriter, Value value,
    ...
    auto emitStaticMutexId = [&](::tla::MutexOp mutexOp) -> FailureOr<Value> {
      ...
      if (mutexId < 0) {
        diagnosticOp->emitError()
            << "mutex id auto allocation is not implemented for bitcode call lowering";
        return failure();
      }
      if (mutexId > 255) {
        diagnosticOp->emitError() << "mutex id must be in range 0..255 for bitcode call lowering";
        return failure();
      }
    ...
    if (auto ifOp = value.getDefiningOp<scf::IfOp>()) {
      ...
      auto newIfOp = rewriter.create<scf::IfOp>(ifOp.getLoc(), rewriter.getI8Type(),
...

      {
        ...
        FailureOr<Value> thenId = materializeMutexIdImpl(
            rewriter, thenYield.getOperand(resultIndex), diagnosticOp, getOrCreateConstant,
            thenVisited);
        if (failed(thenId))
          return failure();
      ...
      }
      {
        ...
        FailureOr<Value> elseId = materializeMutexIdImpl(
            rewriter, elseYield.getOperand(resultIndex), diagnosticOp, getOrCreateConstant,
            elseVisited);
        if (failed(elseId))
          return failure();
    ...
    if (auto blockArg = dyn_cast<BlockArgument>(value)) {
      ...
      if (!yieldOp || iterIndex >= yieldOp.getNumOperands() ||
        ...
        return failure();
      return materializeMutexIdImpl(rewriter, forOp.getInitArgs()[iterIndex], diagnosticOp,
    ...
    if (auto forOp = value.getDefiningOp<scf::ForOp>()) {
      ...
      if (!isTransparentMutexForward(yieldOp.getOperand(resultIndex), regionIterArg))
        return failure();
      return materializeMutexIdImpl(rewriter, forOp.getInitArgs()[resultIndex], diagnosticOp,
                                    getOrCreateConstant, visited);
    }
  ...
  static FailureOr<Value> materializeMutexId(PatternRewriter &rewriter, Value value,
                                             Operation *diagnosticOp,
                                             std::function<Value(Operation *, int64_t, unsigned)> getOrCreateConstant) {
    DenseSet<Value> visited;
    return materializeMutexIdImpl(rewriter, value, diagnosticOp, getOrCreateConstant, visited);
  }

  static bool shouldDropMutexForResult(scf::ForOp forOp, unsigned resultIndex,
  ...
  static bool cleanupMutexForOp(scf::ForOp forOp) {
    ...
    auto newForOp = builder.create<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(),
                                               forOp.getUpperBound(), forOp.getStep(), keptInitArgs);
    ...
    for (auto [newResultIndex, oldResultIndex] : llvm::enumerate(keptIndices))
      forOp.getResult(oldResultIndex).replaceAllUsesWith(newForOp.getResult(newResultIndex));
    forOp.erase();
    return true;
  }
  ...
  static bool shouldDropMutexIfResult(scf::IfOp ifOp, unsigned resultIndex,
    ...
    FailureOr<::tla::MutexOp> thenRoot = resolveMutexRootFromInit(thenYield.getOperand(resultIndex));
    FailureOr<::tla::MutexOp> elseRoot = resolveMutexRootFromInit(elseYield.getOperand(resultIndex));
    if (failed(thenRoot) || failed(elseRoot) || !sameMutexRoot(*thenRoot, *elseRoot))
      return false;
  ...
  static bool cleanupMutexIfOp(scf::IfOp ifOp) {
    ...
    for (auto [newResultIndex, oldResultIndex] : llvm::enumerate(keptIndices))
      ifOp.getResult(oldResultIndex).replaceAllUsesWith(newIfOp.getResult(newResultIndex));
    ifOp.erase();
    return true;
  }
  ...
  struct LowerTlaMutexAccessPattern : public OpRewritePattern<MutexOpT> {
    LowerTlaMutexAccessPattern(MLIRContext *ctx, ModuleOp module, StringRef calleePrefix,
                               std::function<Value(Operation *, int64_t, unsigned)> getOrCreateConstant,
                               SmallVectorImpl<Operation *> &toErase)
        : OpRewritePattern<MutexOpT>(ctx), module(module), calleePrefix(calleePrefix.str()),
          getOrCreateConstant(getOrCreateConstant), toErase(toErase) {}

    LogicalResult matchAndRewrite(MutexOpT op, PatternRewriter &rewriter) const override {
      if (op->getNumOperands() != 1) {
      ...
      FailureOr<Value> mutexId = TlaLowerToStdPass::materializeMutexId(
      ...
      FailureOr<std::string> pipeSuffix = TlaLowerToStdPass::getMutexPipeSuffix(op.getPipe());
  ...
  void runOnOperation() override {
    ...
    LowerTlaMutexAccessPattern<::tla::MutexLockOp> lowerMutexLock(
        &getContext(), module, "get_buf", getOrCreateConstant, toErase);
    LowerTlaMutexAccessPattern<::tla::MutexUnlockOp> lowerMutexUnlock(
        &getContext(), module, "rls_buf", getOrCreateConstant, toErase);
