...
struct LowerMutexPattern : public OpConversionPattern<::tla::MutexOp> {
  ...
  LogicalResult
  matchAndRewrite(::tla::MutexOp op, OpAdaptor,
...
struct LowerMutexAccessPattern : public OpConversionPattern<MutexOpT> {
  ...
  LowerMutexAccessPattern(TypeConverter &converter, MLIRContext *context,
                          ModuleOp module, StringRef calleePrefix)
      : Base(converter, context), module(module),
        calleePrefix(calleePrefix.str()) {}

  LogicalResult
  matchAndRewrite(MutexOpT op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
...
public:
  ...
  void runOnOperation() override {
    ...
    converter.addConversion([&](::tla::MutexType) -> Type {
    ...
    patterns.add<LowerMutexAccessPattern<::tla::MutexLockOp>>(
        converter, context, module, "get_buf");
    patterns.add<LowerMutexAccessPattern<::tla::MutexUnlockOp>>(
        converter, context, module, "rls_buf");
    ...
    target.addIllegalOp<::tla::MutexOp, ::tla::MutexLockOp,
                        ::tla::MutexUnlockOp>();
