...
This document describes the **TLA DSL Host-side APIs** (typically imported as `import catlass.tla as tla`). It covers the `@tla.kernel` decorator, Host `@dataclass` packing, `tla.compile` / `JitCompiledFunction` launch, and Host tensors. Environment variables are in `docs/zh/kernel_development/core_concepts/env_vars.md`. Kernel-side ops live in `docs/en/api/kernel_api_reference.md`.
