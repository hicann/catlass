...
AscendNPU-IR 的版本以 CATLASS 锁定的 revision 为准（即 `.gitmodules` 中 `python/tla_dsl/3rdparty/AscendNPU-IR` 指向的提交 `feature/regbase@a07821269…`）。具体的环境版本要求统一见[环境准备](../00_environment_setup.md#1-环境要求)。
...
然后返回 DSL 子项目并按照[编译与测试](../01_build_and_test.md)继续。
