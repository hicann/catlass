<!--
This file is generated by python/tla_dsl/tools/generate_api_reference.py.
Do not edit manually. Update catlass.core_api public exports, type annotations, or docstrings instead.
-->

# TLA DSL Core API Reference

本文档由 `catlass.core_api` 的运行时对象动态生成，当前只覆盖 Core API，暂不包含 launch / runtime / execution 等接口。

生成依据包括 `catlass.core_api.__all__`、`@dsl_user_op` 包装关系、`inspect.signature`、docstring 和源码位置。

## Core DSL Operations

### `make_shape`

Source: [`catlass.core_api.make_shape`](../catlass/core_api.py#L3476)

```python
make_shape(*components: IndexTree) -> TlaShape
```

Build a packed Tla shape from nested tuple components.

### `make_coord`

Source: [`catlass.core_api.make_coord`](../catlass/core_api.py#L3489)

```python
make_coord(*components: IndexTree) -> TlaCoord
```

Build a packed Tla coordinate from nested tuple components.

### `make_stride`

Source: [`catlass.core_api.make_stride`](../catlass/core_api.py#L3502)

```python
make_stride(*components: IndexTree) -> TlaStride
```

Build a packed Tla stride from nested tuple components.

### `make_layout`

Source: [`catlass.core_api.make_layout`](../catlass/core_api.py#L3515)

```python
make_layout(shape: _Shape, stride: _Stride, *, origin_shape: _Shape | None = None, layoutTag: Any | None = None) -> TlaLayout
```

Combine packed :func:`make_shape` and :func:`make_stride` into ``!tla.layout`` (``tla.make_layout``).

### `tile_view`

Source: [`catlass.core_api.tile_view`](../catlass/core_api.py#L3643)

```python
tile_view(source: Any, shape: _Shape, coord: _Coord) -> TlaTensor
```

Create a tile view using tile-coordinate granularity on a ``!tla.tensor`` source.

### `make_tensor`

Source: [`catlass.core_api.make_tensor`](../catlass/core_api.py#L3670)

```python
make_tensor(ptr: Any, layout: TlaLayout, coord: CoordLike | None = None) -> TlaTensor
```

Construct a ``!tla.tensor`` from an explicit pointer, layout, and coord.

### `make_tensor_like`

Source: [`catlass.core_api.make_tensor_like`](../catlass/core_api.py#L3857)

```python
make_tensor_like(ptr: Any, like: TileLike, layoutTag: Any | None = None) -> TlaTensor
```

Create a tensor over ``ptr`` from ``like`` using structured Tla tensor metadata.

### `copy`

Source: [`catlass.core_api.copy`](../catlass/core_api.py#L4043)

```python
copy(dst: TileLike, src: TileLike, params: CopyParams | None = None) -> None
```

Copy between Tla tensor/view values.

### `flag`

Source: [`catlass.core_api.flag`](../catlass/core_api.py#L4164)

```python
flag(name: str, src_pipe: PipeLike | None = None, dst_pipe: PipeLike | None = None) -> TlaFlag
```

Materialize a synchronization flag. Legacy one-arg form is supported.

### `cross_flag`

Source: [`catlass.core_api.cross_flag`](../catlass/core_api.py#L4211)

```python
cross_flag(name: str, *, mode: int = 2) -> TlaCrossFlag
```

Materialize a named cross-core synchronization flag. Source and destination pipes are specified by the corresponding set and wait operations. Mode 4 selects 1:1 AIC-to-AIV synchronization, addressing AIV0 and AIV1 independently.

### `cross_core_set_flag`

Source: [`catlass.core_api.cross_core_set_flag`](../catlass/core_api.py#L4269)

```python
cross_core_set_flag(cross_flag_value: CrossFlagLike, pipe: PipeLike, aiv_id: int | None = None) -> None
```

Set a cross-core synchronization flag from ``pipe``.

### `cross_core_wait_flag`

Source: [`catlass.core_api.cross_core_wait_flag`](../catlass/core_api.py#L4295)

```python
cross_core_wait_flag(cross_flag_value: CrossFlagLike, pipe: PipeLike, aiv_id: int | None = None) -> None
```

Wait on a cross-core synchronization flag on ``pipe``.

### `set_flag`

Source: [`catlass.core_api.set_flag`](../catlass/core_api.py#L4321)

```python
set_flag(flag_value: FlagLike) -> None
```

Set a synchronization flag.

### `wait_flag`

Source: [`catlass.core_api.wait_flag`](../catlass/core_api.py#L4330)

```python
wait_flag(flag_value: FlagLike) -> None
```

Wait on a synchronization flag.

### `pipe_barrier`

Source: [`catlass.core_api.pipe_barrier`](../catlass/core_api.py#L4339)

```python
pipe_barrier(pipe: PipeLike) -> None
```

Insert a pipe barrier for a specific pipe.

### `mutex`

Source: [`catlass.core_api.mutex`](../catlass/core_api.py#L4357)

```python
mutex(resource: str, id: int = -1) -> TlaMutex
```

Materialize a mutex associated with a semantic resource.

### `mutex_guard`

Source: [`catlass.core_api.mutex_guard`](../catlass/core_api.py#L4388)

```python
mutex_guard(*mutexes: MutexLike) -> _MutexGuard
```

Create a context manager that wraps a block with inferred mutex access.

### `mutex_lock`

Source: [`catlass.core_api.mutex_lock`](../catlass/core_api.py#L4412)

```python
mutex_lock(mutex_value: MutexLike, *, pipe: PipeLike) -> None
```

Acquire a mutex from the specified pipe.

### `mutex_unlock`

Source: [`catlass.core_api.mutex_unlock`](../catlass/core_api.py#L4425)

```python
mutex_unlock(mutex_value: MutexLike, *, pipe: PipeLike) -> None
```

Release a mutex from the specified pipe.

### `range`

Source: [`catlass.core_api.range`](../catlass/core_api.py#L4468)

```python
range(start: IndexLike, end: IndexLike | None = None, step: IndexLike | None = None) -> _ast_helpers.FrontendRange
```

Create a frontend Tla dynamic range. Supports Python range arities.

### `range_constexpr`

Source: [`catlass.core_api.range_constexpr`](../catlass/core_api.py#L4499)

```python
range_constexpr(start: int, end: int | None = None, step: int | None = None) -> Any
```

Create a frontend-time static range for unrolled Python loops.

### `cube`

Source: [`catlass.core_api.cube`](../catlass/core_api.py#L4527)

```python
cube() -> TlaRegion
```

Create a cube region stub for lowering-only usage.

### `vector`

Source: [`catlass.core_api.vector`](../catlass/core_api.py#L4533)

```python
vector() -> TlaRegion
```

Create a vector region stub for lowering-only usage.

### `mmad`

Source: [`catlass.core_api.mmad`](../catlass/core_api.py#L4567)

```python
mmad(acc: TileLike, lhs: TileLike, rhs: TileLike, init_c: bool | Bool | None = None, unit_flag: IndexLike | None = None, compute_order: ComputeOrder = <ComputeOrder.M_FIRST: 0>, **extra_kwargs: Any) -> None
```

Emit a matrix-multiply-accumulate operation over Tla tiles. ``init_c`` accepts only a Python ``bool`` or an SSA value of type ``i1``. ``compute_order`` selects the M/N compute-direction priority, defaulting to ``M_FIRST``.

### `full`

Source: [`catlass.core_api.full`](../catlass/core_api.py#L4640)

```python
full(value: Any, dtype: Any) -> VectorSSA
```

Create a 1-D vector SSA filled with a Python scalar literal.

### `arange`

Source: [`catlass.core_api.arange`](../catlass/core_api.py#L4696)

```python
arange(base: Any = 0, *, order: str = increase, dtype: Any) -> VectorSSA
```

Create a 1-D vector SSA with monotonically increasing or decreasing values.

### `exp`

Source: [`catlass.core_api.exp`](../catlass/core_api.py#L5091)

```python
exp(operand: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `log`

Source: [`catlass.core_api.log`](../catlass/core_api.py#L5091)

```python
log(operand: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `sqrt`

Source: [`catlass.core_api.sqrt`](../catlass/core_api.py#L5091)

```python
sqrt(operand: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `abs`

Source: [`catlass.core_api.abs`](../catlass/core_api.py#L5091)

```python
abs(operand: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `neg`

Source: [`catlass.core_api.neg`](../catlass/core_api.py#L5091)

```python
neg(operand: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `interleave`

Source: [`catlass.core_api.interleave`](../catlass/core_api.py#L5129)

```python
interleave(src0: VectorSSA, src1: VectorSSA) -> tuple[VectorSSA, VectorSSA]
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `deinterleave`

Source: [`catlass.core_api.deinterleave`](../catlass/core_api.py#L5165)

```python
deinterleave(src0: VectorSSA, src1: VectorSSA) -> tuple[VectorSSA, VectorSSA]
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `bitwise_not`

Source: [`catlass.core_api.bitwise_not`](../catlass/core_api.py#L5201)

```python
bitwise_not(operand: Any, *, mask: MaskSSA | None = None) -> MaskSSA | VectorSSA
```

> TODO: 补充 API 说明。该占位由生成器发现 docstring 缺失后自动生成。

### `add`

Source: [`catlass.core_api.add`](../catlass/core_api.py#L5217)

```python
add(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise add for loaded vector SSA values.

### `sub`

Source: [`catlass.core_api.sub`](../catlass/core_api.py#L5238)

```python
sub(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise subtract for loaded vector SSA values.

### `mul`

Source: [`catlass.core_api.mul`](../catlass/core_api.py#L5252)

```python
mul(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise multiply for loaded vector SSA values.

### `max`

Source: [`catlass.core_api.max`](../catlass/core_api.py#L5273)

```python
max(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise maximum for loaded vector SSA values.

### `min`

Source: [`catlass.core_api.min`](../catlass/core_api.py#L5294)

```python
min(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise minimum for loaded vector SSA values.

### `div`

Source: [`catlass.core_api.div`](../catlass/core_api.py#L5315)

```python
div(lhs: Any, rhs: Any, *, mask: MaskSSA | None = None) -> VectorSSA
```

Emit element-wise divide for loaded vector SSA values.

### `where`

Source: [`catlass.core_api.where`](../catlass/core_api.py#L5376)

```python
where(mask: MaskSSA, x: VectorSSA, y: VectorSSA) -> VectorSSA
```

Emit element-wise select for loaded vector SSA values.

### `squeeze`

Source: [`catlass.core_api.squeeze`](../catlass/core_api.py#L5417)

```python
squeeze(src: VectorSSA, mask: MaskSSA) -> VectorSSA
```

Pack lanes of ``src`` selected by ``mask`` into low indices of the result.

### `cmp`

Source: [`catlass.core_api.cmp`](../catlass/core_api.py#L5558)

```python
cmp(lhs: VectorSSA, rhs: Any, mode: str, *, mask: MaskSSA | None = None) -> MaskSSA
```

Return a mask for element-wise vector compare.

### `bitwise_and`

Source: [`catlass.core_api.bitwise_and`](../catlass/core_api.py#L5617)

```python
bitwise_and(src0_reg: Any, src1_reg: Any, *, mask: Any | None = None) -> MaskSSA | VectorSSA
```

Emit element-wise bitwise AND for MaskSSA or VectorSSA values.

### `bitwise_or`

Source: [`catlass.core_api.bitwise_or`](../catlass/core_api.py#L5636)

```python
bitwise_or(src0_reg: Any, src1_reg: Any, *, mask: Any | None = None) -> MaskSSA | VectorSSA
```

Emit element-wise bitwise OR for MaskSSA or VectorSSA values.

### `bitwise_xor`

Source: [`catlass.core_api.bitwise_xor`](../catlass/core_api.py#L5655)

```python
bitwise_xor(src0_reg: Any, src1_reg: Any, *, mask: Any | None = None) -> MaskSSA | VectorSSA
```

Emit element-wise bitwise XOR for MaskSSA or VectorSSA values.

### `gather`

Source: [`catlass.core_api.gather`](../catlass/core_api.py#L5674)

```python
gather(x: TileLike, y: VectorSSA, *, mask: MaskSSA | None = None) -> VectorSSA
```

Gather elements from a UB tensor according to vector indices.

### `allocate`

Source: [`catlass.core_api.allocate`](../catlass/core_api.py#L5838)

```python
allocate(shape: ShapeLike, dtype: type[Numeric], mem_scope: AddressSpace, byte_alignment: int) -> PointerTypeHint
```

Allocate static on-chip scratch memory and return a typed ``!tla.ptr``.

### `make_ptr`

Source: [`catlass.core_api.make_ptr`](../catlass/core_api.py#L5882)

```python
make_ptr(dtype: type[Numeric] | None, value: int | mlir_ir.Value | Numeric, mem_space: AddressSpace = <AddressSpace.gm: 1>, *, assumed_align: int | None = None) -> PointerTypeHint
```

Build a :class:`Pointer` from an integer bit pattern via ``tla.inttoptr``.

### `recast_ptr`

Source: [`catlass.core_api.recast_ptr`](../catlass/core_api.py#L5916)

```python
recast_ptr(ptr: PointerTypeHint, *, dtype: type[Numeric]) -> Any
```

Change the logical pointee type of a ``!tla.ptr`` (dtype-only; no swizzle).

### `create_mask`

Source: [`catlass.core_api.create_mask`](../catlass/core_api.py#L6108)

```python
create_mask(*, pattern: Any | None = None, dtype: Any = <class catlass.base_dsl.typing.Float32>) -> MaskSSA
```

Create a vector mask inside a vector region from a fixed pattern.

### `update_mask`

Source: [`catlass.core_api.update_mask`](../catlass/core_api.py#L6140)

```python
update_mask(true_shape: Any, dtype: Any = <class catlass.base_dsl.typing.Float32>) -> tuple[MaskSSA, Any]
```

Create a tail mask and the remaining element count.

## Core Support Objects

### `arch`

Source: `catlass.core_api.arch`

```python
_Namespace object
```

Core namespace exported by `catlass.core_api`; available members: `CUBE`, `ColumnMajor`, `FIX`, `L0A`, `L0B`, `L0C`, `L0Clayout`, `L1`, `MTE1`, `MTE2`, `MTE3`, `RowMajor`, `SCALAR`, `UB`, `VECTOR`, `block_idx`, `block_num`, `nN`, `nZ`, `sub_block_idx`, `thread_block_dim`, `thread_idx`, `zN`, `zNUnAlign`, `zZ`.

### `TlaCoreAPIError`

Source: [`catlass.runtime.TlaCoreAPIError`](../catlass/runtime.py#L48)

```python
class TlaCoreAPIError(RuntimeError)
```

Raised when a user-facing Tla API call violates preconditions.
