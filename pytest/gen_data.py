...
class TestFlashAttentionInfer():
    ...
    def calc_data(self, gen_data_params: GenDataParams):
        ...
        query = np.random.uniform(q_min_range, q_max_range,  # query
        ...
        layout = 'TND'  # input_layout
        key_cache = None  # key
        value_cache = None  # value
        if gen_data_params.kv_dtype == 1:
            ...
            for i in range(batch_size):
                ...
                block_tables.append(block_table)  # block_table
        ...
        if gen_data_params.mask_type == 1:  # sparse_mode
            mask = np.zeros(shape=(num_tokens, max_k_seqlen)).astype(gen_data_params.dtype)  # atten_mask
            pre_qseqlen = 0
            for i in range(batch_size):
                qseqlen = gen_data_params.q_seqlen_list[i]  # actual_seq_lengths
                kseqlen = gen_data_params.k_seqlen_list[i]  # actual_seq_lengths_kv
        ...
        fai_outputs.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "generate.bin"))
